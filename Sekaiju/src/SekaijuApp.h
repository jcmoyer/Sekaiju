...
#ifndef _SEKAIJUAPP_H_
#define _SEKAIJUAPP_H_
...
typedef struct {
	int m_nOn;                          // メトロノームオン
	int m_nOnWhilePlaying;              // 演奏中メトロノームオン
...
typedef struct {
	...
	int m_nAutoDeleteOldAutoSaveFile;   // 自動保存した場合は古い自動保存ファイルを削除する
} AutoSaveDlgStatus;
...
public:
	...
	BOOL m_bStepInputMode; // ステップ入力モード(0=リアルタイム入力モード, 1=ステップ入力モード)
	long m_lStepTimeIndex; // ステップタイムインデックス
	...
	BOOL m_bStepUpdatable;
	long m_lStepKeyDownCount;
...
public:
	...
	virtual void SendMinimumEachNoteOff (); // 20201213追加:SendAllNoteOffが効かない音源用
	virtual void SendMinimumEachNoteOffEx (long lPort, long lChannel); // 20201213追加:SendAllNoteOffExが効かない音源用
...
protected:
	...
	afx_msg void OnControlStepInputMode ();
	afx_msg void OnUpdateControlStepInputModeUI (CCmdUI* pCmdUI);
	afx_msg void OnControlStepAdvance ();
	afx_msg void OnUpdateControlStepAdvanceUI (CCmdUI* pCmdUI);
	afx_msg void OnControlStepTime (UINT nID);
	afx_msg void OnUpdateControlStepTimeUI (CCmdUI* pCmdUI);
	...
	afx_msg void OnControlSendPanic ();
