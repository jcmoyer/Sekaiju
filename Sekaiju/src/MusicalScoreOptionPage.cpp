...
// 状態が変更されたとき
void CMusicalScoreOptionPage::OnChange () {
	SetModified (TRUE);
}
...
BEGIN_MESSAGE_MAP (CMusicalScoreOptionPage, CPropertyPage)
	ON_EN_CHANGE (IDC_MUSICALSCOREOPTION_DEFTRACKZOOM, OnChange)
	ON_EN_CHANGE (IDC_MUSICALSCOREOPTION_DEFTIMEZOOM, OnChange)
	ON_BN_CLICKED (IDC_MUSICALSCOREOPTION_ENABLETRACKZOOMKEY, OnChange)
	ON_BN_CLICKED (IDC_MUSICALSCOREOPTION_ENABLETIMEZOOMKEY, OnChange)
	ON_BN_CLICKED (IDC_MUSICALSCOREOPTION_SPEAKERMODEALLTRACK, OnChange)
	ON_BN_CLICKED (IDC_MUSICALSCOREOPTION_SPEAKERMODEVISIBLETRACK, OnChange)
END_MESSAGE_MAP ()
