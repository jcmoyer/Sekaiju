...
int CSelectVoiceDlg::TStrInclude (TCHAR* psz1, const TCHAR* psz2) {
	TCHAR* p = psz1;
	int nRet = 0;
	int nLen1 = (int)CLIP (0, _tcslen (psz1), 0x7FFFFFFF);
	int nLen2 = (int)CLIP (0, _tcslen (psz2), 0x7FFFFFFF);
	int i;
	for (i = 0; i < nLen2; i++) {
...
int CSelectVoiceDlg::Search () {
	...
	// 検索テキストの正当性のチェック
	int nBuf1Len = (int)CLIP (0, _tcslen (szBuf1), 0x7FFFFFFF);
	...
	for (i = 0; i < 16384; i++) {
		...
		for (j = 0; j < 128; j++) {
			...
			if (lRet) {
				...
				_tcsncpy_s (szBuf2, szName, TSIZEOF (szBuf2) - 1);
				if (nBuf1Len > 0) {
					if (!nCaseSensitive) {
						_tcslwr_s (szBuf2);
					}
					if (nExactMatch) {
						nRet = _tcsstr (szBuf2, szBuf1) != NULL ? 1 : 0; //20210704修正
					}
					else {
						nRet = (TStrInclude (szBuf2, szBuf1) >= nBuf1Len) ? 1 : 0; //20210704修正
					}
				}
				if (nBuf1Len == 0 || nBuf1Len > 0 && nRet) {
					_sntprintf_s (szBuf, TSIZEOF (szBuf) - 1, _T("%3d   %3d   %3d%c%s\0"),
						i >> 7, (i & 0x7F), j,
						i > 0 ? _T('-') : _T('-'), szName);
