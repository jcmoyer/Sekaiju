...
(CString& strFileName, UINT nIDSTitle, DWORD lFlags, 
 BOOL bOpenFileDialog, CDocTemplate* pTemplate) {
	...
	strInitialDir = pSekaijuApp->m_strFileOpenPath;
	...
	VERIFY (strFilterMIDI[0].LoadString (IDS_STANDARD_MIDI_FILES_AD_MIDI));
	...
	VERIFY (strFilterMIDMIDI[2].LoadString (IDS_D_MIDI));
	...
	theFileDlg.m_ofn.lpstrInitialDir = NULL; //strInitialDir;
	...
	CString strExt5 = strFileName.Right (5); // 20221025修正
	if (bOpenFileDialog) {
		...
		else if (strExt.CompareNoCase (strFilterMMML[1].Right(5)) == 0) { // *.mmml
	...
	::SetCurrentDirectory (pSekaijuApp->m_strFileOpenPath);*/
	BOOL bResult = theFileDlg.DoModal() == IDOK ? TRUE : FALSE;
	/*pSekaijuApp->m_strFileOpenPath = _T("");
	TCHAR* pszNewDirectory = pSekaijuApp->m_strFileOpenPath.GetBuffer (_MAX_PATH);
	::GetCurrentDirectory (_MAX_PATH, pszNewDirectory);
	 pSekaijuApp->m_strFileOpenPath.ReleaseBuffer ();
	...
	if (bOpenFileDialog == FALSE) {
		if (theFileDlg.m_ofn.nFilterIndex == 1 &&
			...
			(strFileName.Right (5)).CompareNoCase (strFilterMIDI[2]) != 0)) { // .mid
