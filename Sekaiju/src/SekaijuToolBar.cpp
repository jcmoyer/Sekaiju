...
BEGIN_MESSAGE_MAP (CSekaijuToolBar, CToolBar)
	...
	ON_CBN_SELCHANGE (IDC_STEPTIMECOMBO, OnStepTimeChange)
	ON_WM_DRAWITEM ()
	ON_WM_ERASEBKGND ()
...
void CSekaijuToolBar::DrawBorders (CDC* pDC, CRect& rect) {
	...
	if (m_hReBarTheme) {
		if (::IsThemeBackgroundPartiallyTransparent
			...
			::DrawThemeParentBackground (m_hWnd, pDC->m_hDC, &rectWindow);
		}
...
BOOL CSekaijuToolBar::AdjustToHighResolution () {
	...
	for (int i = 0; i < nCount; i++) {
		if (this->GetButtonStyle (i) == TBBS_BUTTON) {
			CRect temp;
			this->GetItemRect (i, &temp);
			temp.MoveToXY (0, 0);
			temp.right = (int)(ceil (temp.right * fx));
			temp.bottom = (int)(ceil ((temp.bottom) * fy));
			CSize bmp_size (16, 15);
			CToolBar::SetSizes (temp.Size(), bmp_size);
			return TRUE;
		}
	}
	return FALSE;
...
// ステップタイムコンボの値が変更されたとき
void CSekaijuToolBar::OnStepTimeChange () {
	CMainFrame* pMainFrame = (CMainFrame*)AfxGetMainWnd ();
	CComboBox* pStepTimeCombo = &(pMainFrame->m_wndStepTimeCombo);
	int nCurSel = pStepTimeCombo->GetCurSel ();
	if (nCurSel == CB_ERR) {
		nCurSel = 0;
	}
	pMainFrame->PostMessage (WM_COMMAND, ID_CONTROL_STEPTIME00 + nCurSel);
}
