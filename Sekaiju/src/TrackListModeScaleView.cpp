...
void CTrackListModeScaleView::OnDraw (CDC* pDC) {
	...
	for (int j = 0; j < TRACKLISTFRAME_NUMCOLUMN; j ++) {
		...
		theRect.right = theRect.right + pTrackListFrame->GetColumnBaseWidth (j) * lColumnZoom * lLogPixelsX / 96;
...
void CTrackListModeScaleView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	for (j = 0; j < TRACKLISTFRAME_NUMCOLUMN; j++) {
		lBorderX += pTrackListFrame->GetColumnWidth (j, lLogPixelsX);
		if (lBorderX - 2 * lLogPixelsX / 96 <= point.x && point.x <= lBorderX + 2 * lLogPixelsX / 96) {
			break;
		}
	}
	// カーソルが境界上にあった場合
...
void CTrackListModeScaleView::OnLButtonUp (UINT nFlags, CPoint point) {
	...
	if (GetCapture () == this) {
		...
		pTrackListFrame->RecalcColumnScrollInfo (lLogPixelsY);
		pTrackListFrame->m_pModeScaleView->Invalidate (lLogPixelsX);
		pTrackListFrame->m_pTrackModeView->Invalidate (lLogPixelsX);
	}
}
...
// マウスホイールが回された時
BOOL CTrackListModeScaleView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CTrackListFrame* pTrackListFrame = (CTrackListFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
