...
void CEventListScaleView::OnUpdate (CView* pSender, LPARAM lHint, CObject* pHint) {
	...
	if (lHint & SEKAIJUDOC_MIDITRACKCHANGED) {
		...
		pEventListFrame->UpdateControlChangeList ();
	}
	...
	if ((lHint & SEKAIJUDOC_MIDIDATACHANGED) || 
		...
		pEventListFrame->MakeVisibleEventArray (lLogPixelsX, lLogPixelsY);
		pEventListFrame->RecalcRowScrollInfo (lLogPixelsY);
	}
...
// マウスホイールが回された時
BOOL CEventListScaleView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
