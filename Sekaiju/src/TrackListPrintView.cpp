...
CString CTrackListPrintView::GetCellString (long lRow, long lColumn) {
	...
	if (pMIDITrack) {
		...
		case TRACKLISTFRAME_TRACKNAME:
			...
			codestr2str (szBuf1, (long) CLIP (0, _tcslen (szBuf1), 0x7FFFFFFF), szBuf2, TSIZEOF (szBuf2) - 1);
			strText = szBuf2;
			break;
		// 可視(未使用)
		...
		forEachEvent (pMIDITrack, pMIDIEvent) {
			...
			if (0 <= lTime && lTime <= lEndTime) {
				...
				// マーカー
					...
					TCHAR szBuf[256];
					memset (szBuf, 0, sizeof (szBuf));
					MIDIEvent_GetText (pMIDIEvent, szBuf, TSIZEOF (szBuf) - 1); // 20230528 バグ修正 sizeof→TSIZEOF
					if (lOldTime != lTime) {
						if (strText2 != _T("")) {
							DrawFlagAndText (pDC, lOldTime, strText2, lColorTrack1);
						}
...
BOOL CTrackListPrintView::OnPreparePrinting (CPrintInfo* pInfo) {
	...
	return CSekaijuView::OnPreparePrinting(pInfo);
}
...
void CTrackListPrintView::OnBeginPrinting (CDC* pDC, CPrintInfo* pInfo) {
	...
	pSekaijuDoc->m_theCriticalSection.Unlock ();
	::Sleep(10);
	//return CSekaijuView::OnBeginPrinting(pDC, pInfo);
}

// 印刷終了時
void CTrackListPrintView::OnEndPrinting (CDC* pDC, CPrintInfo* pInfo) {
	return CSekaijuView::OnEndPrinting (pDC, pInfo);
}
