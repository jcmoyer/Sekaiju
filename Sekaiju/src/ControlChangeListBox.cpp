...
// メッセージマップ
IMPLEMENT_DYNCREATE (CControlChangeListBox, CCheckListBox)

BEGIN_MESSAGE_MAP (CControlChangeListBox, CCheckListBox)
	ON_WM_RBUTTONDOWN ()
END_MESSAGE_MAP ()
...
// コンストラクタ
CControlChangeListBox::CControlChangeListBox () {
	m_pDocument = NULL;
	m_lMenuID = 0;
...
// コンストラクタ
CControlChangeListBox::CControlChangeListBox (CDocument* pDocument, long lMenuID) {
...
CSekaijuDoc* CControlChangeListBox::GetDocument () {
	return (CSekaijuDoc*)m_pDocument;
}

long CControlChangeListBox::GetLastRButtonDownIndex () {
...
// マウス右ボタン押された時  
void CControlChangeListBox::OnRButtonDown (UINT nFlags, CPoint point) {
	this->SetFocus ();
	if (m_lMenuID == 0) {
		return;
	}
