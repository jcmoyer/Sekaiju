...
#define MUSICALSCOREFRAME_TRACKLISTWIDTH 200
...
// y座標をトラックインデックスに変換
long CMusicalScoreFrame::YtoTrackIndex (long y, long lLogPixelsY) {
	long lNumTrackInfo = INT_PTRtolong (GetTrackInfoCount ());
	for (long i = 0; i < lNumTrackInfo; i++) {
		MusicalScoreTrackInfo* pTrackInfo = GetTrackInfo (i);
		if (pTrackInfo->m_lTop * m_lTrackZoom * lLogPixelsY / 96 <= y && 
			y < (pTrackInfo->m_lTop + pTrackInfo->m_lHeight) * lLogPixelsY / 96 * m_lTrackZoom) {
			return i;
		}
	}
...
// y座標をノートキーに変換
long CMusicalScoreFrame::TrackIndexYtoKey (long lTrackIndex, long y, long lKeySignature, long lLogPixelsY) {
	long lNumTrackInfo = INT_PTRtolong (GetTrackInfoCount ());
	...
	for (lKey = 0; lKey < 128; lKey++) {
		long lTempY = TrackIndexKeytoY (lTrackIndex, lKey, lKeySignature, lLogPixelsY);
		if (lTempY < y) {
...
long CMusicalScoreFrame::TrackIndexLineNotoY (long lTrackIndex, long lLineNo, long lLogPixelsY) {
	...
	case 1: // ト音記号
		lKey000Y = (lTrackTop + lTrackHeight / 2) * m_lTrackZoom * lLogPixelsY / 96 + 41 * ry;
		break;
	case 2: // へ音記号
		lKey000Y = (lTrackTop + lTrackHeight / 2) * m_lTrackZoom * lLogPixelsY / 96 + 29 * ry;
		break;
	case 3: // 大譜表
		lKey000Y = (lTrackTop + lTrackHeight / 2) * m_lTrackZoom * lLogPixelsY / 96 + 35 * ry;
		break;
	default:
		lKey000Y = (lTrackTop + lTrackHeight / 2) * m_lTrackZoom * lLogPixelsY / 96 + 35 * ry;
		break;
...
long CMusicalScoreFrame::TrackIndexKeytoY (long lTrackIndex, long lKey, long lKeySignature, long lLogPixelsY) {
	long lLineNo = KeytoLineNo (lKey, lKeySignature);
	return TrackIndexLineNotoY (lTrackIndex, lLineNo, lLogPixelsY);
}



// x座標[pixel]からタイム[tick]を取得
long CMusicalScoreFrame::XtoTime (long x, long lLogPixelsX) {
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	MIDIData* pMIDIData = pSekaijuDoc->m_pMIDIData;
	INT_PTR lMeasureCount = m_theMeasureInfoArray.GetSize ();
	INT_PTR j = 0;
	for (j = 0; j < lMeasureCount - 1; j++) {
		if (GetMeasureLeft (j + 1) * m_lTimeZoom * lLogPixelsX / 96 > x) {
			break;
		}
	}
	...
	long lDeltaX_TimeZoom = x - 
		...
		pMeasureInfo->m_lPreWidth) * m_lTimeZoom * lLogPixelsX / 96;
	lDeltaX_TimeZoom = MAX (0, lDeltaX_TimeZoom);
	long lTimeResolution = MIDIData_GetTimeResolution (pMIDIData);
	return pMeasureInfo->m_lTime + lDeltaX_TimeZoom * lTimeResolution / 8 / m_lTimeZoom * 96 / lLogPixelsX;
	// 注：ズーム倍率1倍のとき4分音符の長さを8ピクセルと定義している。

}

// タイム[tick]をx座標[pixel]に変換
long CMusicalScoreFrame::TimetoX (long lTime, long lLogPixelsX) {
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	MIDIData* pMIDIData = pSekaijuDoc->m_pMIDIData;
	INT_PTR lMeasureCount = m_theMeasureInfoArray.GetSize ();
	...
	return 
		pMeasureInfo->m_lLeft * m_lTimeZoom * lLogPixelsX / 96 + 
		pMeasureInfo->m_lSignatureWidth * m_lTimeZoom * lLogPixelsX / 96 +
		pMeasureInfo->m_lPreWidth * m_lTimeZoom * lLogPixelsX / 96 +
 		(lDeltaTime * 8 * m_lTimeZoom / lTimeResolution * lLogPixelsX / 96);
	// 注：ズーム倍率1倍のとき4分音符の長さを8ピクセルと定義している。
}

// 小節をx座標[pixel]に変換(小節境界線位置)
long CMusicalScoreFrame::MeasuretoX (long lMeasure, long lLogPixelsX) {
	INT_PTR lMeasureCount = GetMeasureInfoCount ();
	ASSERT (0 <= lMeasure && lMeasure < lMeasureCount);
	MusicalScoreMeasureInfo* pMeasureInfo = NULL;
	VERIFY (pMeasureInfo = GetMeasureInfo (lMeasure));
	return pMeasureInfo->m_lLeft * m_lTimeZoom * lLogPixelsX / 96;
}

// 小節をx座標[pixel]に変換(小節境界線+拍子調整記号位置)
long CMusicalScoreFrame::MeasuretoX2 (long lMeasure, long lLogPixelsX) {
	INT_PTR lMeasureCount = GetMeasureInfoCount ();
	ASSERT (0 <= lMeasure && lMeasure < lMeasureCount);
	MusicalScoreMeasureInfo* pMeasureInfo = NULL;
	VERIFY (pMeasureInfo = GetMeasureInfo (lMeasure));
	return (pMeasureInfo->m_lLeft + pMeasureInfo->m_lSignatureWidth) * m_lTimeZoom * lLogPixelsX / 96;
}
...
// 表示されているタイムの左端を求める
long CMusicalScoreFrame::GetVisibleLeftTime (long lLogPixelsX) {
	return XtoTime (m_lTimeScrollPos - 120 * lLogPixelsX / 96, lLogPixelsX);
}

// 表示されているタイムの右端を求める
long CMusicalScoreFrame::GetVisibleRightTime (long lLogPixelsX) {
	CRect rcClient;
	m_pTimeScaleView->GetClientRect (&rcClient);
	return XtoTime (m_lTimeScrollPos + rcClient.Width () /*+ 120 * lLogPixelsX / 96*/, lLogPixelsX);
}

// 表示されているトラックの上限を計算
long CMusicalScoreFrame::GetVisibleTopTrack (long lLogPixelsY) {
	long lTrackCount = INT_PTRtolong (m_theTrackInfoArray.GetCount ());
	long lTrackIndex = 0;
	for (lTrackIndex = 0; lTrackIndex < lTrackCount - 1; lTrackIndex++) {
		if (GetTrackInfo (lTrackIndex + 1)->m_lTop * m_lTrackZoom * lLogPixelsY / 96 >= m_lTrackScrollPos) {
			break;
		}
	}
	return lTrackIndex;
...
// 表示されているトラックの下限を計算
long CMusicalScoreFrame::GetVisibleBottomTrack (long lLogPixelsY) {
	CRect rcClient;
	m_pTrackScaleView->GetClientRect (&rcClient);
	long lTrackCount = INT_PTRtolong (m_theTrackInfoArray.GetCount ());
	long lTrackIndex = 0;
	for (lTrackIndex = 0; lTrackIndex < lTrackCount; lTrackIndex++) {
		if (GetTrackInfo (lTrackIndex)->m_lTop * m_lTrackZoom * lLogPixelsY / 96 >= m_lTrackScrollPos + rcClient.Height ()) {
			break;
		}
	}
	return lTrackIndex;
...
// スプリッターキャプターの描画
void CMusicalScoreFrame::DrawSplitterCaptor (CDC* pDC, CPoint pt, long lLogPixelsX, long lLogPixelsY) {
	CRect rcClient;
	GetClientRect (&rcClient);
	CPen pen;
	CPen* pOldPen;
	pen.CreatePen (PS_SOLID, MUSICALSCOREFRAME_SPLITTERWIDTH * lLogPixelsX / 96, ::GetSysColor(COLOR_BTNSHADOW));
...
// キースクロールバー(縦)のデザイン設定
void CMusicalScoreFrame::RecalcTrackScrollInfo (long lLogPixelsY) {
	INT_PTR lTrackInfoCount = GetTrackInfoCount ();
	...
	si.nMin = 0;
	si.nMax = (lLastTrackTop + lLastTrackHeight) * m_lTrackZoom * lLogPixelsY / 96;
...
void CMusicalScoreFrame::RecalcTimeScrollInfo (long lLogPixelsX) {
	INT_PTR lMeasureInfoCount = m_theMeasureInfoArray.GetSize ();
	...
	SCROLLINFO si;
	si.fMask = SIF_RANGE | SIF_PAGE;
	si.nMin = lStartX * m_lTimeZoom * lLogPixelsX / 96;
	si.nMax = lEndX * m_lTimeZoom * lLogPixelsX / 96;
...
BOOL CMusicalScoreFrame::DeleteTrackInfoArray () {
	INT_PTR lNumTrackInfo = GetTrackInfoCount ();
	for (INT_PTR i = 0; i < lNumTrackInfo; i++) {
		MusicalScoreTrackInfo* pTrackInfo = GetTrackInfo (i);
...
BOOL CMusicalScoreFrame::DeleteNoteInfoArray (INT_PTR lTrackIndex) {
	INT_PTR lNumTrackInfo = GetTrackInfoCount ();
	ASSERT (0 <= lTrackIndex && lTrackIndex < lNumTrackInfo);
	MusicalScoreTrackInfo* pTrackInfo = GetTrackInfo (lTrackIndex);
	ASSERT (pTrackInfo);
	INT_PTR lNumNoteInfo = pTrackInfo->m_theNoteInfoArray.GetSize ();
	for (INT_PTR j = 0; j < lNumNoteInfo; j++) {
...
BOOL CMusicalScoreFrame::DeleteNoteGroupInfoArray (INT_PTR lTrackIndex) {
	INT_PTR lNumTrackInfo = GetTrackInfoCount ();
	ASSERT (0 <= lTrackIndex && lTrackIndex < lNumTrackInfo);
	MusicalScoreTrackInfo* pTrackInfo = GetTrackInfo (lTrackIndex);
	ASSERT (pTrackInfo);
	INT_PTR lNumNoteGroupInfo = pTrackInfo->m_theNoteGroupInfoArray.GetSize ();
	for (INT_PTR j = 0; j < lNumNoteGroupInfo; j++) {
...
BOOL CMusicalScoreFrame::DeleteTripletGroupInfoArray (INT_PTR lTrackIndex) {
	INT_PTR lNumTrackInfo = GetTrackInfoCount ();
	ASSERT (0 <= lTrackIndex && lTrackIndex < lNumTrackInfo);
	MusicalScoreTrackInfo* pTrackInfo = GetTrackInfo (lTrackIndex);
	ASSERT (pTrackInfo);
	INT_PTR lNumTripletGroupInfo = pTrackInfo->m_theTripletGroupInfoArray.GetSize ();
	for (INT_PTR j = 0; j < lNumTripletGroupInfo; j++) {
...
BOOL CMusicalScoreFrame::DeleteMeasureInfoArray () {
	INT_PTR lNumMeasureInfo = GetMeasureInfoCount ();
	for (INT_PTR j = 0; j < lNumMeasureInfo; j++) {
...
BOOL CMusicalScoreFrame::UpdateNoteInfoArray (long lTrackIndex, MIDITrack* pMIDITrack) {
	INT_PTR lNumTrackInfoArray = GetTrackInfoCount ();
...
void CMusicalScoreFrame::RecalcLayout (BOOL bNotify) {
	...
	if (rcClient.Height () - m_lToolBar1Height < 
		...
		MUSICALSCOREFRAME_SCALEHEIGHT + m_lHScrollBarHeight +
		MUSICALSCOREFRAME_BORDERHEIGHT * 2) {
		m_lScaleHeight = MUSICALSCOREFRAME_SCALEHEIGHT * lLogPixelsY / 96;
		m_lKeyHeight = 0;
	}
	else {
		m_lScaleHeight = MUSICALSCOREFRAME_SCALEHEIGHT * lLogPixelsY / 96;
	...
	// 幅方向の位置計算
	if (rcClient.Width () <
		MUSICALSCOREFRAME_SCALEWIDTH * lLogPixelsX / 96 + m_lVScrollBarWidth + 
		...
		MUSICALSCOREFRAME_SCALEWIDTH * lLogPixelsX / 96 + m_lVScrollBarWidth + 
		MUSICALSCOREFRAME_BORDERWIDTH * 4 + 
		MUSICALSCOREFRAME_SPLITTERWIDTH * lLogPixelsX / 96 + m_lTrackListWidth) {
		//m_lScaleWidth = rcClient.Width () - 
		//	m_lVScrollBarWidth - MUSICALSCOREFRAME_BORDERWIDTH * 2;
		m_lScaleWidth = MUSICALSCOREFRAME_SCALEWIDTH * lLogPixelsX / 96;
		m_lTimeWidth = 0;
		m_lTrackListWidth = rcClient.Width () - m_lScaleWidth -
			m_lVScrollBarWidth - MUSICALSCOREFRAME_BORDERWIDTH * 4 -
			MUSICALSCOREFRAME_SPLITTERWIDTH * lLogPixelsX / 96;
	...
	m_pTrackListBox->MoveWindow
		...
		m_lVScrollBarWidth + MUSICALSCOREFRAME_SPLITTERWIDTH * lLogPixelsX / 96, 
		m_lToolBar1Height + MUSICALSCOREFRAME_BORDERHEIGHT,
		m_lTrackListWidth,
	...
	RecalcTrackScrollInfo (lLogPixelsY);
	RecalcTimeScrollInfo (lLogPixelsX);
}
...
int CMusicalScoreFrame::OnCreate (LPCREATESTRUCT lpCreateStruct) {
	...
	long lLogPixelsY = ::GetDeviceCaps (hDC, LOGPIXELSY);
	::ReleaseDC (this->GetSafeHwnd (), hDC);
	m_lTrackListWidth = MUSICALSCOREFRAME_TRACKLISTWIDTH * lLogPixelsX / 96;
	...
	// トラックコンボの作成
	m_wndToolBar1.SetButtonInfo (5, IDC_TRACKCOMBO, TBBS_SEPARATOR, 80 * lLogPixelsX / 96);
	...
	m_wndTrackCombo.SetFont (&(pSekaijuApp->m_theDefaultFont));
	m_wndTrackCombo.SendMessage (CB_SETITEMHEIGHT, -1, 23 * lLogPixelsY / 96 - 8);
	
	// チャンネルコンボの作成
	m_wndToolBar1.SetButtonInfo (7, IDC_CHANNELCOMBO, TBBS_SEPARATOR, 40 * lLogPixelsX / 96);
	...
	m_wndChannelCombo.SetFont (&(pSekaijuApp->m_theDefaultFont));
	m_wndChannelCombo.SendMessage (CB_SETITEMHEIGHT, -1, 23 * lLogPixelsY / 96 - 8);

	// スナップコンボの作成
	m_wndToolBar1.SetButtonInfo (9, IDC_SNAPCOMBO, TBBS_SEPARATOR, 100 * lLogPixelsX / 96);
	...
	m_wndSnapCombo.SetFont (&(pSekaijuApp->m_theDefaultFont));
	m_wndSnapCombo.SendMessage (CB_SETITEMHEIGHT, -1, 23 * lLogPixelsY / 96 - 8);

	// ベロシティコンボの作成
	m_wndToolBar1.SetButtonInfo (11, IDC_VELOCITYCOMBO, TBBS_SEPARATOR, 50 * lLogPixelsX / 96);
	...
	m_wndVelocityCombo.SetFont (&(pSekaijuApp->m_theDefaultFont));
	m_wndVelocityCombo.SendMessage (CB_SETITEMHEIGHT, -1, 23 * lLogPixelsY / 96 - 8);

	// 長さコンボの作成
	m_wndToolBar1.SetButtonInfo (13, IDC_DURATIONCOMBO, TBBS_SEPARATOR, 50 * lLogPixelsX  / 96);
	...
	m_wndDurationCombo.SetFont (&(pSekaijuApp->m_theDefaultFont));
	m_wndDurationCombo.SendMessage (CB_SETITEMHEIGHT, -1, 23 * lLogPixelsY / 96 - 8);	

	// 表示精度コンボの作成
	m_wndToolBar1.SetButtonInfo (25, IDC_RESOLUTIONCOMBO, TBBS_SEPARATOR, 100 * lLogPixelsX / 96);
	...
	m_wndResolutionCombo.SetFont (&(pSekaijuApp->m_theDefaultFont));
	m_wndResolutionCombo.SendMessage (CB_SETITEMHEIGHT, -1, 23 * lLogPixelsY / 96 - 8);

	// フィルチェックボックス作成
	m_wndToolBar1.SetButtonInfo (27, IDC_FILLCHECKBOX, TBBS_SEPARATOR, 48 * lLogPixelsX / 96);
	...
	m_wndFillCheckBox.SetFont (&(pSekaijuApp->m_theDefaultFont));

	// トリムチェックボックスの作成
	m_wndToolBar1.SetButtonInfo (29, IDC_TRIMCHECKBOX, TBBS_SEPARATOR, 48 * lLogPixelsX / 96);
	...
	m_wndTrimCheckBox.SetFont (&(pSekaijuApp->m_theDefaultFont));
	...
	m_wndFillCheckBox.SetWindowText (strFill);

	// トリム
	CString strTrim;
	strTrim.LoadString (IDS_TRIM); //= LoadString (pSekaijuApp->m_hResourceDLL, IDS_TRIM);
	m_wndTrimCheckBox.SetWindowText (strTrim);
	...
	int x = this->MeasuretoX2 (0, lLogPixelsX) + 1;
...
void CMusicalScoreFrame::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	if (m_bSplitterMovingH || m_bSplitterMovingV) {
		...
		CDC* pDC = GetDC ();
		DrawSplitterCaptor (pDC, point, lLogPixelsX, lLogPixelsY);
		ReleaseDC (pDC);
		if (m_bSplitterMovingH && m_bSplitterMovingV) {
...
void CMusicalScoreFrame::OnLButtonUp (UINT nFlags, CPoint point) {
	...
	if (GetCapture () == this) {
		...
		pSekaijuDoc->m_theCriticalSection.Lock ();
		CDC* pDC = GetDC ();
		DrawSplitterCaptor (pDC, m_ptMouseMoveOld, lLogPixelsX, lLogPixelsY);
		ReleaseDC (pDC);
		ReleaseCapture ();
...
void CMusicalScoreFrame::OnMouseMove (UINT nFlags, CPoint point) {
	...
	// 非キャプター中
		...
		m_bSplitterMovingH =
			...
			MUSICALSCOREFRAME_SPLITTERHEIGHT * lLogPixelsY / 96;
		// カーソルが垂直スプリッター上にあるか
		m_bSplitterMovingV =
