...
(CPoint pt, long* pRow, long* pColumn, long lLogPixelsX, long lLogPixelsY) {
	...
	_ASSERT (pRow);
	_ASSERT (pColumn);
	*pRow = pTrackListFrame->YtoRow (pt.y, lLogPixelsY);
	*pColumn = pTrackListFrame->XtoColumn (pt.x, lLogPixelsY);
	return TRUE;
}
...
BOOL CTrackListTrackModeView::AutoScrolltoShowCell (long lRow, long lColumn) {
	...
	if (pTrackListFrame->GetColumnLeft (lColumn, lLogPixelsX) < rcClient.left) {
		lNewPos = pTrackListFrame->GetColumnLeft (lColumn, lLogPixelsX);
		pTrackListFrame->SetColumnScrollPos (lNewPos);
	}
	// 指定セルがビューの右にはみ出した場合の処理
	else if (pTrackListFrame->GetColumnLeft (lColumn, lLogPixelsX) + 
		pTrackListFrame->GetColumnWidth (lColumn, lLogPixelsX) > rcClient.right) {
		lNewPos = pTrackListFrame->GetColumnLeft (lColumn, lLogPixelsX) + 
			pTrackListFrame->GetColumnWidth (lColumn, lLogPixelsX) - rcClient.Width ();
		pTrackListFrame->SetColumnScrollPos (lNewPos);
	}
...
BOOL CTrackListTrackModeView::BeginValueUpDown () {
	...
	case TRACKLISTFRAME_CONTROLCHANGE:
		...
		pTempEvent = pSekaijuDoc->GetTrackFirstControlChange (pTempTrack, lNumber); // 20220103修正:lChannel==n/a(-1)の時の対策。
		while (pTempEvent) {
	...
	case TRACKLISTFRAME_PROGRAMCHANGE:
		...
		pTempEvent = pSekaijuDoc->GetTrackFirstProgramChange (pTempTrack); // 20220103修正:lChannel==n/a(-1)の時の対策。
	...
	if (pMIDITrack) {
		...
		case TRACKLISTFRAME_PROGRAMCHANGE:
			...
			if (0 <= lValue && lValue <= 127 && pMIDIEvent) {
				...
				long lBank = ((lBankMSB & 0x7F) << 7) | (lBankLSB & 0x7F); //MIDIEvent_GetBank (pMIDIEvent);
...
BOOL CTrackListTrackModeView::SetCellString (long lRow, long lColumn, CString strText) {
	...
	case TRACKLISTFRAME_CONTROLCHANGE:
		...
		if (pTempEvent == NULL) {
			long lChannel = MIDITrack_GetOutputChannel (pMIDITrack);
			lChannel = CLIP (0, lChannel, 15); // 20220103修正:lChannel==n/a(-1)の時の対策。
...
// 現在のセルの長方形を描画
BOOL CTrackListTrackModeView::DrawCurFrame (CDC* pDC, long lLogPixelsX, long lLogPixelsY) {
...
// セルの長方形を取得
CRect CTrackListTrackModeView::GetRectFromCell (long lRow, long lColumn, long lLogPixelsX, long lLogPixelsY) {
	long j;
	CRect rcCell (0, 0, 0 ,0);
	CTrackListFrame* pTrackListFrame = (CTrackListFrame*)GetParentFrame ();
	for (j = 0; j < lColumn; j++) {
		rcCell.left += pTrackListFrame->GetColumnWidth (j, lLogPixelsX);
	}
	rcCell.right = rcCell.left + pTrackListFrame->GetColumnWidth (j, lLogPixelsX);
	rcCell.top = lRow * pTrackListFrame->GetRowZoom () * lLogPixelsY / 96;
	rcCell.bottom = (lRow + 1) * pTrackListFrame->GetRowZoom () * lLogPixelsY / 96;
	return rcCell;
}
...
BOOL CTrackListTrackModeView::MoveTextBox (long lRow, long lColumn) {
	...
	m_theTextBox.MoveWindow 
		(rcNewCell.left, rcNewCell.top + pTrackListFrame->GetRowZoom () * lLogPixelsY / 96 / 2 - lHeight/ 2, 
		rcNewCell.Width () - lButtonWidth * lLogPixelsX / 96, lHeight);
...
void CTrackListTrackModeView::OnDraw (CDC* pDC) {
	...
	// 行の背景色描画
	long lTrackCount = MIDIData_CountTrack (pMIDIData);
	long lVisibleTopRow = pTrackListFrame->GetVisibleTopRow (lLogPixelsY);
	long lVisibleBottomRow = pTrackListFrame->GetVisibleBottomRow (lLogPixelsY);
	for (i = lVisibleTopRow; i <= lVisibleBottomRow; i++) {
		...
		pDC->FillSolidRect 
			(rcClient.left, i * lRowZoom * lLogPixelsY / 96, 
			rcClient.Width (), (i + 1) * lRowZoom * lLogPixelsY / 96, 
			lBackColor);
	}
	// 横線の描画
	...
	for (j = 0; j < TRACKLISTFRAME_NUMCOLUMN; j++) {
		...
		x += pTrackListFrame->GetColumnBaseWidth (j) * lColumnZoom * lLogPixelsX / 96;
	}
	pDC->SelectObject (pOldPen);
	...
	for (i = lStartRow; i < lEndRow; i++) {
		...
		if (pMIDITrack) {
			...
			// 各列について
			for (j = 0; j < TRACKLISTFRAME_NUMCOLUMN; j++) {
				CRect rcCell = GetRectFromCell (i, j, lLogPixelsX, lLogPixelsY);
				CRect rcText = rcCell;
				...
				switch (lColumnContent & 0xFFFF) {
				case TRACKLISTFRAME_FORECOLOR:
					rcColor.left = rcCell.left + 2 * lLogPixelsX / 96;
					rcColor.right = rcCell.right - 3 * lLogPixelsY / 96;
					rcColor.top = rcCell.top + rcCell.Height () / 2 - 6 * lLogPixelsX / 96;
					rcColor.bottom = rcCell.top + rcCell.Height () / 2 + 6 * lLogPixelsX / 96;
...
BOOL CTrackListTrackModeView::OnCreate (LPCREATESTRUCT lpcs) {
	...
	m_theTextBox.Create (WS_CHILD /*| WS_VISIBLE | WS_BORDER*/ | ES_AUTOHSCROLL, 
		...
		pTrackListFrame->GetColumnWidth (0, lLogPixelsX), 13), this, IDC_TEXTBOX);
	m_theTextBox.SetFont (&(pSekaijuApp->m_theFont[0]));
...
void CTrackListTrackModeView::OnKillFocus (CWnd* pNewWnd) {
	_RPTF1 (_CRT_WARN, "CTrackListTrackModeView::OnKillFocus (pNewWnd=0x%p)\n", pNewWnd);
	CTrackListFrame* pTrackListFrame = (CTrackListFrame*)GetParent ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	pSekaijuDoc->m_theCriticalSection.Lock ();
		...
		if (point.x > rcNewCell.right - lButtonWidth) {
			...
			// 現在のセルを操作可能な場合
				...
				if (m_lCurButtonState == 0x0000) {
					BeginValueUpDown ();
					// 上のボタンが押された場合
					if (point.y < rcNewCell.top + rcNewCell.Height () / 2) {
...
void CTrackListTrackModeView::OnRButtonDown (UINT nFlags, CPoint point) {
	...
	if ((lColumnContent & 0xFFFF) == TRACKLISTFRAME_INPUTPORT ||
		...
		// ボタンは押されていない場合
			...
			long lTrackIndex = pTrackListFrame->YtoRow (point.y, lLogPixelsY);
			pSekaijuDoc->m_lTempTime = pTrackListFrame->XtoTime (point.x, lLogPixelsX);
...
// マウスホイールが回された時
BOOL CTrackListTrackModeView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CTrackListFrame* pTrackListFrame = (CTrackListFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	// ユニバーサルスクロール停止
