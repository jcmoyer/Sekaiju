...
void CPianoRollVelScaleView::OnDraw (CDC* pDC) {
	...
	for (i = 0; i <= 8; i++) {
		...
		long y = pPianoRollFrame->VeltoY (i * 16, lLogPixelsY);
...
void CPianoRollVelScaleView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	m_lOldTempoBPM = pPianoRollFrame->YtoTempoBPM (m_ptMouseDown.y, lLogPixelsY);
	m_lDownTempoBPM = pPianoRollFrame->YtoTempoBPM (point.y, lLogPixelsY);
	m_lCurTempoBPM = pPianoRollFrame->YtoTempoBPM (point.y, lLogPixelsY);
	m_lOldVel = pPianoRollFrame->YtoVel (m_ptMouseDown.y, lLogPixelsY);
	m_lDownVel = pPianoRollFrame->YtoVel (point.y, lLogPixelsY);
	m_lCurVel = pPianoRollFrame->YtoVel (point.y, lLogPixelsY);
	m_lOldPitchBend = pPianoRollFrame->YtoPitchBend (m_ptMouseDown.y, lLogPixelsY);
	m_lDownPitchBend = pPianoRollFrame->YtoPitchBend (point.y, lLogPixelsY);
	m_lCurPitchBend = pPianoRollFrame->YtoPitchBend (point.y, lLogPixelsY);
...
void CPianoRollVelScaleView::OnMouseMove (UINT nFlags, CPoint point) {
	...
	if (GetCapture () == this) {
		pSekaijuDoc->m_theCriticalSection.Lock ();
		m_lCurTempoBPM = pPianoRollFrame->YtoTempoBPM (point.y, lLogPixelsY);
		m_lDownTempoBPM = pPianoRollFrame->YtoTempoBPM (m_ptMouseDown.y, lLogPixelsY);
		m_lOldTempoBPM = pPianoRollFrame->YtoTempoBPM (m_ptMouseMove.y, lLogPixelsY);
		m_lCurVel = pPianoRollFrame->YtoVel (point.y, lLogPixelsY);
		m_lDownVel = pPianoRollFrame->YtoVel (m_ptMouseDown.y, lLogPixelsY);
		m_lOldVel = pPianoRollFrame->YtoVel (m_ptMouseMove.y, lLogPixelsY);
		m_lCurPitchBend = pPianoRollFrame->YtoPitchBend (point.y, lLogPixelsY);
		m_lDownPitchBend = pPianoRollFrame->YtoPitchBend (m_ptMouseDown.y, lLogPixelsY);
		m_lOldPitchBend = pPianoRollFrame->YtoPitchBend (m_ptMouseMove.y, lLogPixelsY);
...
// マウスホイールが回された時
BOOL CPianoRollVelScaleView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	long lDelta = zDelta;
