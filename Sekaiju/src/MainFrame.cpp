...
// ステップコンボボックスの更新
BOOL CMainFrame::UpdateStepTimeCombo (long lTimeMode, long lTimeResolution) {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CString strText;
	long lCurSel = m_wndStepTimeCombo.GetCurSel ();
	m_wndStepTimeCombo.ResetContent ();
	if (lTimeMode == 0 && lTimeResolution == 0) {
		return FALSE;
	}
	// ステップコンボボックスの充満
	CString strFormat;
	...
	m_wndStepTimeCombo.AddString (strText);
...
//	m_wndStepTimeCombo.AddString (strText);
	// カレントセル設定
	if (m_wndStepTimeCombo.GetCount () >= 0) {
		m_wndStepTimeCombo.SetCurSel (pSekaijuApp->m_lStepTimeIndex);
	}
	return TRUE;
}
...
int CMainFrame::OnCreate (LPCREATESTRUCT lpCreateStruct) {
	...
	// デフォルトフォントの生成
	long lFontHeight = CLIP (6, 12 * lLogPixelsY / 96, 96);
	CString strFontName;
	...
	m_wndToolBar2.AdjustToHighResolution ();	
	...
	// 時:分:秒:ミリ秒の作成
	m_wndToolBar2.SetButtonInfo (0, IDC_MILLISECEDIT, TBBS_SEPARATOR, 100 * lLogPixelsX / 96);
	...
	m_wndMillisecEdit.SetFont (&(pSekaijuApp->m_theDefaultFont));


	// 小節:拍:ティックの作成
	m_wndToolBar2.SetButtonInfo (2, IDC_TIMEEDIT, TBBS_SEPARATOR, 100 * lLogPixelsX / 96);
	...
	m_wndTimeEdit.SetFont (&(pSekaijuApp->m_theDefaultFont));

	// 再生位置スクロールバーの作成
	m_wndToolBar2.SetButtonInfo (9, IDC_POSITIONSCROLL, TBBS_SEPARATOR, 120 * lLogPixelsX / 96);
	...
	// ステップティックの作成
	m_wndToolBar2.SetButtonInfo (20, IDC_STEPTIMECOMBO, TBBS_SEPARATOR, 100 * lLogPixelsX / 96);
	m_wndToolBar2.GetItemRect (20, &rcTemp);
	//rcTemp.bottom = 80;
	if (!m_wndStepTimeCombo.CreateEx (
		WS_EX_CLIENTEDGE, _T("COMBOBOX"), NULL,
		WS_VISIBLE | WS_TABSTOP | WS_CHILD | CBS_DROPDOWNLIST,
		rcTemp.left, rcTemp.top + 1, rcTemp.Width (), rcTemp.Height () - 2, 
		m_wndToolBar2.GetSafeHwnd (), (HMENU)IDC_STEPTIMECOMBO)) {
		TRACE0 ("Failed to create combo box\n");
		return -1;
	}
	//m_wndStepTimeCombo.SetFont (CFont::FromHandle ((HFONT)::GetStockObject (DEFAULT_GUI_FONT)));
	m_wndStepTimeCombo.SetFont (&(pSekaijuApp->m_theDefaultFont));
	m_wndStepTimeCombo.SendMessage (CB_SETITEMHEIGHT, -1, 23 * lLogPixelsY / 96 - 8);
	// CSekaijuApp::UpdateCurWndAndDocPtrが呼ばれてコンボボックス内を更新する。

	// キーシフトの作成
	m_wndToolBar2.SetButtonInfo (24, IDC_KEYSHIFTCOMBO, TBBS_SEPARATOR, 60 * lLogPixelsX / 96);
	m_wndToolBar2.GetItemRect (24, &rcTemp);
	//rcTemp.bottom = 80;
	...
	m_wndKeyShiftCombo.SetFont (&(pSekaijuApp->m_theDefaultFont));
	m_wndKeyShiftCombo.SendMessage (CB_SETITEMHEIGHT, -1, 23 * lLogPixelsY / 96 - 8);
	for (i = 12; i >= -12; i--) {
		CString str;
		if (i > 0) {
			str.Format (_T("+%d"), i);
	...
	// 拍子･調性の作成
	m_wndToolBar2.SetButtonInfo (26, IDC_MEASUREEDIT, TBBS_SEPARATOR, 60 * lLogPixelsX / 96);
	m_wndToolBar2.GetItemRect (26, &rcTemp);
	...
	m_wndMeasureEdit.SetFont (&(pSekaijuApp->m_theDefaultFont));

	// テンポ[BPM]の作成
	m_wndToolBar2.SetButtonInfo (28, IDC_TEMPOEDIT, TBBS_SEPARATOR, 60 * lLogPixelsX / 96);
	m_wndToolBar2.GetItemRect (28, &rcTemp);
	...
	m_wndTempoEdit.SetFont (&(pSekaijuApp->m_theDefaultFont));
	...
	m_wndStatusBar.SetPaneInfo (1, ID_INDICATOR_FORMAT, 0, 64 * lLogPixelsX / 96); 
	m_wndStatusBar.SetPaneInfo (2, ID_INDICATOR_NUMTRACKS, 0, 64 * lLogPixelsX / 96); 
	m_wndStatusBar.SetPaneInfo (3, ID_INDICATOR_TIMEBASE, 0, 96 * lLogPixelsX / 96); 
	m_wndStatusBar.SetPaneInfo (4, ID_INDICATOR_INPUTVELOCITY, 0, 156 * lLogPixelsX / 96);
	m_wndStatusBar.SetPaneInfo (5, ID_INDICATOR_OUTPUTVELOCITY, 0, 156 * lLogPixelsX / 96); 

	// ツールバーのテキスト更新用タイマー起動
	SetTimer (0x03, USER_TIMER_MINIMUM, NULL);

	// ステータスバーのゲージ更新用タイマー起動
	SetTimer (0x04, USER_TIMER_MINIMUM, NULL);
...
void CMainFrame::OnTimer (UINT_PTR nIDEvent) {
		...
		if (pActiveFrame == NULL || pActiveFrame == this) {
			...
			if (pSekaijuDoc) {
				...
				if (pSekaijuDoc->m_pMIDIData && pSekaijuDoc->m_pMIDIClock) {
					...
					_sntprintf_s (szText, 1023, _T("%02d:%02d:%02d:%03d"), lHour, lMinute, lSec, lMillisec);
					...
					if (lTimeMode == MIDIDATA_TPQNBASE) {
						MIDIData_BreakTimeEx (pSekaijuDoc->m_pMIDIData, lCurTime, &lMeasure, &lBeat, &lTick,
							&nn, &dd, &cc, &bb);
						memset (szText, 0, sizeof (szText));
						_sntprintf_s (szText, 1023, _T("%05d:%02d:%03d"), lMeasure + 1, lBeat + 1, lTick);
						...
						memset (szText, 0, sizeof (szText));
						_sntprintf_s (szText, 1023, _T("%08d:%03d"), lFrameNumber, lTick);
						SetWindowTextWhenDifferent (&m_wndTimeEdit, szText);
					}
					/* スクロールポジションの更新 */
					...
					memset (szText, 0, sizeof (szText));
					_sntprintf_s (szText, 1023, _T("%d/%d, %d%s"), nn, 1 << dd, abs(sf), 
					...
					memset (szText, 0, sizeof (szText));
					_sntprintf_s (szText, 1023, _T("%1.2lf"), (double)60000000 / (double)lCurTempo);
					...
					/* 現在の時：分：秒：ミリ秒表示 */
					memset (szText, 0, sizeof (szText));
					_sntprintf_s (szText, 1023, _T("%05d:%02d:%03d"), 1, 1, 0);
					SetWindowTextWhenDifferent (&m_wndTimeEdit, szText);
					/* 現在の小節：拍：ティック値表示 */
					memset (szText, 0, sizeof (szText));
					_sntprintf_s (szText, 1023, _T("%02d:%02d:%02d:%03d"), 0, 0, 0, 0);
					SetWindowTextWhenDifferent (&m_wndMillisecEdit, szText);
					/* スクロールポジションの更新 */
					m_wndPositionScroll.SetScrollPos (0, TRUE);
					/* 現在の拍子記号及び調性記号の表示 */
					memset (szText, 0, sizeof (szText));
					_sntprintf_s (szText, 1023, _T("%d/%d, %d%s"), 4, 4, 0, _T(" "));
					SetWindowTextWhenDifferent (&m_wndMeasureEdit, szText);
					/* 現在のテンポ[BPM]の表示 */
					memset (szText, 0, sizeof (szText));
					_sntprintf_s (szText, 1023, _T("%1.2lf"), (double)60000000 / (double)600000);
					SetWindowTextWhenDifferent (&m_wndTempoEdit, szText);
				}
			}
			else {
	...
	// ステータスバーのゲージ更新用タイマー(0x04)
		...
		rcLeft.left = rcItem.left + 32 * lLogPixelsX / 96, 
		rcLeft.top = rcItem.top + 2 * lLogPixelsY / 96;
		rcLeft.right = rcItem.left + (32 + ucMaxVelocity) * lLogPixelsX / 96;
		rcLeft.bottom = rcItem.bottom - 2 * lLogPixelsY / 96;
		rcRight.left = rcItem.left + (32 + ucMaxVelocity) * lLogPixelsX / 96;
		rcRight.top = rcItem.top + 2 * lLogPixelsY / 96;
		rcRight.right = rcItem.right - 2 * lLogPixelsX / 96;
		rcRight.bottom = rcItem.bottom - 2 * lLogPixelsY / 96;
	...
	if (pFrameWnd) {
		...
		if (pSekaijuDoc) {
			...
			case MIDIDATA_SMPTE25BASE:
				strText.Format (_T("SMPTE25/%d"), lTimeResolution); // 20230103:修正(24→25)
				break;
...
LRESULT CMainFrame::OnSALGetValue (WPARAM wParam, LPARAM lParam) {
	...
	if (pSekaijuApp->IsDocumentExist (pSekaijuDoc)) {
		...
		if (_tcscmp (pSALGetValue->m_szVarName, _T("TIMEBASE")) == 0) {
			...
			pSALGetValue->m_nLen = (int) CLIP (0, _tcslen (pSALGetValue->m_szValue), 0x7FFFFFFF);
			pSALGetValue->m_nValue = (int) CLIP (0, _tcslen (pSALGetValue->m_szValue), 0x7FFFFFFF);
		}
		else if (_tcscmp (pSALGetValue->m_szVarName, _T("SeqNum.Val")) == 0 &&
...
BOOL CMainFrame::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo) {
	...
	if ((nID == ID_FILE_PRINT || nID == ID_FILE_PRINT_DIRECT || nID == ID_FILE_PRINT_PREVIEW) &&
		...
		if (pChildFrame != NULL) {
			BOOL bRet = FALSE;
			if (pChildFrame->IsKindOf (RUNTIME_CLASS(CTrackListFrame))) {
				bRet = (((CTrackListFrame*)pChildFrame)->m_pPrintView)->OnCmdMsg(nID, nCode, pExtra, pHandlerInfo);
				return TRUE;
			}
