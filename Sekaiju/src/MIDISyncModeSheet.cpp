...
class CMIDISyncModeDialogTemplate : public CDialogTemplate {
	friend class CMIDISyncModeSheet;
};
...
void CMIDISyncModeSheet::BuildPropPageArray () {
	...
	for (int nPage = 0; nPage < m_pages.GetSize (); nPage++) {
		...
		dlgTemplate.m_dwTemplateSize = CMIDISyncModeDialogTemplate::GetTemplateSize (pResource);
