...
CAutoSaveDlg::CAutoSaveDlg () : CDialog (CAutoSaveDlg::IDD) {
	...
	m_nAutoDeleteOldAutoSaveFile = 0; // 20230114追加
}
...
// 『自動保存した場合は古い自動保存ファイルを削除する』を更新
BOOL CAutoSaveDlg::UpdateAutoDeleteOldAutoSaveFile () {
	int nOn = ((CButton*)GetDlgItem (IDC_AUTOSAVE_ON))->GetCheck ();
	GetDlgItem (IDC_AUTOSAVE_AUTODELETEOLDAUTOSAVEFILE)->EnableWindow (nOn);
	return TRUE;
}
...
void CAutoSaveDlg::DoDataExchange (CDataExchange* pDX) {
	...
	DDX_Check (pDX, IDC_AUTOSAVE_AUTODELETEOLDAUTOSAVEFILE, m_nAutoDeleteOldAutoSaveFile);
}
...
BOOL CAutoSaveDlg::OnInitDialog () {
	...
	UpdateAutoDeleteOldAutoSaveFile (); // 20230114追加
	SetIntervalRange ();
	return bRet;
}
