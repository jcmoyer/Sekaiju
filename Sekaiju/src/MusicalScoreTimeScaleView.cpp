...
(CDC* pDC, long lTime, LPCTSTR lpszText, long lColor, long lLogPixelsX, long lLogPixelsY) {
	...
	CRect rcBack (x, 0, x + 1024, 16 * lLogPixelsY / 96);
	CRect rcFlag (x, 4 * lLogPixelsY / 96, x + 4 * lLogPixelsX / 96, 12 * lLogPixelsY / 96);
	CRect rcText (x + 5 * lLogPixelsY / 96, 0, x + 1024, 16 * lLogPixelsY / 96);
	CPen pen;
	pen.CreatePen (PS_SOLID, 1, lColor);
	CPen* pOldPen = pDC->SelectObject (&pen);
	pDC->FillSolidRect (&rcBack, ::GetSysColor (COLOR_BTNFACE));
	pDC->FillSolidRect (&rcFlag, lColor);
	pDC->MoveTo (x, 4 * lLogPixelsX / 96);
	pDC->LineTo (x, 16 * lLogPixelsX / 96);
...
void CMusicalScoreTimeScaleView::OnDraw (CDC* pDC) {
		...
		forEachEvent (pMIDITrack, pMIDIEvent) {
			...
			if (0 <= lTime && lTime <= lVisibleRightTime) {
				...
				if (MIDIEvent_IsTempo (pMIDIEvent)) {
					...
					if (lOldTime != lTime) {
						if (strText2 != _T("")) {
							DrawFlagAndText (pDC, lOldTime, strText2, lColorTrack1, lLogPixelsX, lLogPixelsY);
						}
						strText2 = _T("");
					}
					if (strText2 != _T("")) {
	...
	if (lTimeMode == MIDIDATA_TPQNBASE) {
		...
		xold = pMusicalScoreFrame->MeasuretoX (lLeftMeasure, lLogPixelsX) - 1;
		CRect theRect;
		pDC->SetBkMode (TRANSPARENT);
		for (long i = lLeftMeasure; i <= lRightMeasure + 1; i++) {
			...
			pDC->MoveTo (x, 16 * lLogPixelsY / 96);
			pDC->LineTo (x, 32 * lLogPixelsY / 96);
			theRect = CRect (xold, 16 * lLogPixelsY / 96, x, 32 * lLogPixelsY / 96);
			pDC->Draw3dRect (xold, 16 * lLogPixelsY / 96, x - xold, 16 * lLogPixelsY / 96, lColorBtnHighlight, lColorBtnShadow);
			memset (szBuf, 0, sizeof (szBuf));
			_sntprintf_s (szBuf, 256, 255,  _T("%d"), i);
		...
		xold = pMusicalScoreFrame->TimetoX (lVisibleLeftTime, lLogPixelsX) - 1;
		CRect theRect;
		pDC->SetBkMode (TRANSPARENT);
	...
	if (GetCapture () == this) {
		long lDownX = pMusicalScoreFrame->TimetoX (m_lDownTime, lLogPixelsX);
		long lCurX = pMusicalScoreFrame->TimetoX (m_lCurTime, lLogPixelsX);
...
void CMusicalScoreTimeScaleView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	if (point.y < rcClient.Height () / 2) { // 20100103追加
		pSekaijuDoc->m_theCriticalSection.Lock ();
		long lTime = pMusicalScoreFrame->XtoTime (point.x, lLogPixelsX);
	...
	// 下半分(小節番号部)をクリックしたとき(小節選択)
		...
		m_lOldTime = pMusicalScoreFrame->XtoTime (m_ptMouseDown.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
		m_lDownTime = pMusicalScoreFrame->XtoTime (point.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
		m_lCurTime = pMusicalScoreFrame->XtoTime (point.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
...
void CMusicalScoreTimeScaleView::OnMouseMove (UINT nFlags, CPoint point) {
	...
	if (GetCapture () == this) {
		...
		m_lCurTime = pMusicalScoreFrame->XtoTime (point.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
		m_lDownTime = pMusicalScoreFrame->XtoTime (m_ptMouseDown.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
		m_lOldTime = pMusicalScoreFrame->XtoTime (m_ptMouseMove.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
...
// マウスホイールが回された時
BOOL CMusicalScoreTimeScaleView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CMusicalScoreFrame* pMusicalScoreFrame = (CMusicalScoreFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	// 画面のdpi取得
