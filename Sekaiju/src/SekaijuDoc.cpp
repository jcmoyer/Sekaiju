...
BOOL CSekaijuDoc::OnOpenDocument (LPCTSTR lpszPathName) {
	...
	// スタンダードMIDIファイル(*.mid)の場合
		...
		strExt5.CompareNoCase (_T(".midi")) == 0 || // 20220518追加
		...
		// TPQNベースで分解能がMAXTIMERESOLUTION TPQNを超える場合、MAXTIMERESOLUTION TPQNに修正する。
		if (MIDIData_GetTimeMode (m_pMIDIData) == MIDIDATA_TPQNBASE &&
			MIDIData_GetTimeResolution (m_pMIDIData) > MAXTIMERESOLUTION) {
			m_theCriticalSection.Unlock ();
			EndWaitCursor ();
			CString strFormat;
			VERIFY (strFormat.LoadString (IDS_THIS_MIDIDATA_TIMERESOLUTION_IS_TOO_HIGH_D_N_MODIFY_TO_D));
			CString strMsg;
			strMsg.Format (strFormat, MIDIData_GetTimeResolution (m_pMIDIData), MAXTIMERESOLUTION);
			AfxMessageBox (strMsg, MB_ICONEXCLAMATION | MB_OK);
			m_theCriticalSection.Lock ();
			VERIFY (MIDIData_SetTimeBase (m_pMIDIData, MIDIDATA_TPQNBASE, MAXTIMERESOLUTION));
		}
	...
	// MIDICSVファイル(*.csv)の場合
		...
		// TPQNベースで分解能がMAXTIMERESOLUTION TPQN以上の場合、MAXTIMERESOLUTION TPQNに修正する。
		if (MIDIData_GetTimeMode (m_pMIDIData) == MIDIDATA_TPQNBASE &&
			MIDIData_GetTimeResolution (m_pMIDIData) > MAXTIMERESOLUTION) {
			m_theCriticalSection.Unlock ();
			EndWaitCursor ();
	...
	if (MIDIData_GetFormat (m_pMIDIData) == 1) {
		...
		if (lNumErrorEvent > 0) {
			...
			if (lRet == IDYES) {
				...
				for (INT_PTR j = 0; j < theErrorEventArray.GetSize (); j++) {
		...
		if (lCount > 0) {
			...
			if (lRet == IDYES) {
				...
				forEachTrack (m_pMIDIData, pMIDITrack) {
					...
					for (INT_PTR j = 0; j < theEventArray.GetSize (); j++) {
...
BOOL CSekaijuDoc::ApplyAppCurSpeedIndex () {
	...
	BOOL bRunning = MIDIClock_IsRunning (m_pMIDIClock);
	if (bRunning) {
		MIDIClock_Stop (m_pMIDIClock);
	}
	// MIDIクロックのタイムモードと速度をアプリケーションに合わせる(20090625追加)
...
BOOL CSekaijuDoc::SetClipboardTextPrivate9 (CString& strData) {
	...
	_tcscpy_s (p, (strData.GetLength () + 1), (LPCTSTR)strData); // 20220309 修正
...
(CString& strData, long lBeginTrackIndex, long lBeginTime, CHistoryUnit* pHistoryUnit) {
	...
	long lKind;
	long lLen;
	BYTE byData[MIDIEVENT_MAXLEN]; // 20220310 256→MIDIEVENT_MAXLENに追加修正
	...
	while (1) {
		...
		if (strTextLine.Left (4) == _T("MDa2")) {
			...
			// MDat情報の読み取り
			_stscanf_s (strTextLine, _T("%s %x %x %x %x %x %x %x %x %x %x %x\n"), // TODO:20210523引数の数保留
				szType,
				256,
				&lFormat, 
				&lNumTrack, 
		...
		// MTr2行の場合
			...
			_stscanf_s (strTextLine, 
				_T("%s %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x\n"),
				szType,
				256,
				&lTrackIndex, 
				&lNumEvent,
		...
		// MEvt行の場合
			...
			_stscanf_s (p, _T("%s %x %x %x %x"),
				szType, 256, &lEventIndex, &lTime, &lKind, &lLen);
			...
			lLen = CLIP (0, lLen, MIDIEVENT_MAXLEN); // 20220310 256→MIDIEVENT_MAXLENに追加修正
			for (k = 0; k < lLen; k++) {
				_stscanf_s (p, _T("%hhx"), &byData[k]); //20210523 保留
				p = _tcschr (p, ' ');
				if (p == NULL) {
					break;
				}
...
CString CSekaijuDoc::GetKeyName (MIDITrack* pMIDITrack, long lTime, long lKey) {
	...
	// 20230204:lKeyが128～255のMIDIデータがあり得る。
	if (lKey < 0 || lKey >= 128) {
		return _T("");
	}							
	...
	// このトラックの表示モードは「ドラム」の場合
		...
		if (pMIDIInstDef) {
			...
			MIDINoteNameTable* pNoteNameTable = 
				...
				((lTrackCC0 & 0x7F) << 7) | (lTrackCC32 & 0x7F), lTrackPC);
...
long CSekaijuDoc::StringTimeToLongTime (MIDIData* pMIDIData, CString strText, long* pTime) {
	...
	if (lTimeMode == MIDIDATA_TPQNBASE) {
		...
		if (lBeat < 0 || lBeat >= 256) { // 20201101 最大値修正
		...
		// ティック値チェック
		lTick = _ttol (strText);
		if (lTick < 0 || lTick >= MAXTIMERESOLUTION * 4) { // 20201101 最大値修正(MAXTIMERESOLUTION*4=7680, N/1時)
...
BOOL CSekaijuDoc::AddHistoryUnit (CString strName) {
	...
	INT_PTR lNumHistoryUnit = m_theHistoryUnitArray.GetSize ();
	for (INT_PTR i = lNumHistoryUnit - 1; i > m_lCurHistoryPosition; i--) {
...
BOOL CSekaijuDoc::DeleteAllHistoryUnit () {
	INT_PTR lNumHistoryUnit = m_theHistoryUnitArray.GetSize ();
	for (INT_PTR i = lNumHistoryUnit - 1; i >= 0; i--) {
		CHistoryUnit* pHistoryUnit = (CHistoryUnit*)m_theHistoryUnitArray.GetAt (i);
		m_theHistoryUnitArray.RemoveAt (i);
		delete pHistoryUnit;
	}
...
void CSekaijuDoc::OnEditUndo () {
	...
	INT_PTR lHistoryRecordCount = pHistoryUnit->m_theHistoryRecordArray.GetSize ();
	INT_PTR i;
...
void CSekaijuDoc::OnEditTrack () {
	...
	if (theDlg.DoModal () == IDOK) {
		...
		INT_PTR lTempSelectedEventCount = theTempSelectedEventArray.GetSize ();
		for (INT_PTR j = 0; j < lTempSelectedEventCount; j++) {
			...
			if (lFormat == 1 && !MIDIEvent_IsMIDIEvent (pMIDIEvent) && lTargetTrackIndex == 0 ||
				...
				MIDIEvent* pNewEvent = NULL;
				long lTrackCount = MIDIData_CountTrack (m_pMIDIData);
				for (long i = lTrackCount; i <= lTargetTrackIndex; i++) { 
			...
			for (INT_PTR j = 0; j < lTempSelectedEventArrayCount; j++) {
				...
				if (theDlg.m_nUnit == 0) {
					...
					if (MIDIEvent_IsEndofTrack (pCloneEvent)) {
						MIDIEvent* pLastEvent = MIDITrack_GetLastEvent ((MIDITrack*)(pCloneEvent->m_pUser1));
...
void CSekaijuDoc::FlipSelectedNotesHorizontal() {
    CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp();
    m_theCriticalSection.Lock();
    MIDITrack* pMIDITrack = NULL;
    MIDIEvent* pMIDIEvent = NULL;
    ...
    forEachTrack(m_pMIDIData, pMIDITrack) {
        forEachEvent(pMIDITrack, pMIDIEvent) {
            if (this->IsEventSelected(pMIDIEvent) && MIDIEvent_IsNoteOn(pMIDIEvent) && MIDIEvent_IsCombined(pMIDIEvent)) {
    ...
    for (MIDIEvent* e : clone_events) {
        long t0 = MIDIEvent_GetTime(e);
        MIDIEvent_SetTime((e), max_time - (t0 - min_time));
        VERIFY(pCurHistoryUnit->AddHistoryRecord(HISTORYRECORD_INSERTEVENT, e));
    }
...
void CSekaijuDoc::OnEditBreakupAndTrill () {
	...
	if (theDlg.DoModal () == IDOK) {
		...
		forEachTrack (m_pMIDIData, pMIDITrack) {
			...
			for (INT_PTR i = 0; i < theEventArray.GetSize (); i++) {
	...
	if (theDlg.DoModal () == IDOK) {
			...
			for (i = 0; i < theEventArray.GetSize (); i++) { 
				...
				if (MIDIEvent_IsNote (pMIDIEvent)) {
					...
					// ビート時刻(j)とビート時刻(j+1)の間にあるイベント
						...
						VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVEEVENT, pMIDIEvent));
						VERIFY (pCloneEvent = ReplaceMIDIEvent (pMIDIEvent));
						MIDIEvent_SetTime (pCloneEvent, (long)lNewTime1);
						VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCloneEvent));
						pMIDIEvent = pCloneEvent;
						lMovedEventCount++;
					}
					...
					// ビート時刻(k)とビート時刻(k+1)の間にあるイベント
						...
						INT_PTR lNewDuration = lNewTime2 - lNewTime1;
						lNewDuration = CLIP (0, lNewDuration, 0xFFFF);
						VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVEEVENT, pMIDIEvent));
						VERIFY (pCloneEvent = ReplaceMIDIEvent (pMIDIEvent));
						MIDIEvent_SetDuration (pCloneEvent, (long)lNewDuration);
						VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCloneEvent));
						pMIDIEvent = pCloneEvent;
						lMovedEventCount++;
					}
				...
				// ノートイベント(ノートオン+ノートオフ)以外の場合
					...
					// ビート時刻(j)とビート時刻(j+1)の間にあるイベント
						...
						VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVEEVENT, pMIDIEvent));
						VERIFY (pCloneEvent = ReplaceMIDIEvent (pMIDIEvent));
						MIDIEvent_SetTime (pCloneEvent, (long)lNewTime);
						VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCloneEvent));
						pMIDIEvent = pCloneEvent;
						lMovedEventCount++;
					}
		...
		if (theDlg.m_nInsertTempo) {
			...
			for (i = 1; i < theTempoArray.GetSize (); i++) {
				...
				INT_PTR lTime = lBeginTime + lBeatInterval * (i - 1);
				lTime = CLIP (0, lTime, 0x7FFFFFFF);
				VERIFY (pMIDIEvent = MIDIEvent_CreateTempo ((long)lTime, lTempo));
				VERIFY (MIDITrack_InsertEvent (pMIDITrack, pMIDIEvent));
				VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pMIDIEvent));
...
void CSekaijuDoc::OnEditStroke () {
	...
	if (theDlg.DoModal () == IDOK) {
		...
		INT_PTR j = 0;
		INT_PTR jMax = theTempEventArray.GetSize ();
		INT_PTR jSortMin = 0;
		INT_PTR jSortMax = 0;
		INT_PTR lOldTrackIndex = 0;
		INT_PTR lOldGroupIndex = 0;
		for (j = 0; j < jMax; j++) {
...
void CSekaijuDoc::OnEditInsertMeasure () {
	...
	if (theDlg.DoModal () == IDOK) {
		...
		// 一時配列のイベント1個につき1回の移動をかける。
		INT_PTR lTempFollowingEventArrayCount = theTempFollowingEventArray.GetSize ();
		// SetTimeは同時刻イベントの最後に挿入するため、EOT含め昇順に(20170527修正)
		// EOTの旧状態保護のため、除去フェーズと挿入フェーズに分離(20170527修正)
		for (INT_PTR j = 0; j < lTempFollowingEventArrayCount; j++) {
...
void CSekaijuDoc::OnEditRemoveMeasure () {
	...
	if (theDlg.DoModal () == IDOK) {
		...
		INT_PTR lTempDuringEventArrayCount = theTempDuringEventArray.GetSize ();
		for (INT_PTR j = 0; j < lTempDuringEventArrayCount; j++) {
...
void CSekaijuDoc::OnPopupInsertTempo () {
	...
	if (theDlg.DoModal () == IDOK) {
		...
		long lMillisec = MIDIData_TimeToMillisec (m_pMIDIData, lCurTime);
		MIDIClock_SetMillisec (m_pMIDIClock, lMillisec);
		MIDIData_FindTempo (m_pMIDIData, lCurTime, &lValue); //20200822修正
...
void CSekaijuDoc::OnPopupModifyMarker () {
	...
	codestr2str (szText1, size_ttolong (_tcslen (szText1)), szText2, 1023);
	theDlg.m_strText = szText2;
	if (theDlg.DoModal () == IDOK) {
		// タイム文字列をlong型に変換
		long lNewTime = lTime;
...
void CSekaijuDoc::OnPopupLoadSysExData () {
	...
	if (pSekaijuApp->m_strSysxOpenPath == _T("")) {
		pSekaijuApp->m_strSysxOpenPath = strInitialDir;
	}
	...
	//#endif
	    ...
	    theFileDlg.m_ofn.lStructSize = sizeof (OPENFILENAME); //=88(With placebar)
	...
	theFileDlg.m_ofn.lpstrInitialDir = strInitialDir;
	...
	TCHAR szOldDirectory[_MAX_PATH];
	memset (szOldDirectory, 0, sizeof (szOldDirectory));
	::GetCurrentDirectory (_MAX_PATH, szOldDirectory);
	::SetCurrentDirectory (pSekaijuApp->m_strSysxOpenPath);*/
	BOOL bResult = (theFileDlg.DoModal() == IDOK ? TRUE : FALSE);
	/*TCHAR* pszNewDirectory = pSekaijuApp->m_strSysxOpenPath.GetBuffer (_MAX_PATH);
	::GetCurrentDirectory (_MAX_PATH, pszNewDirectory);
	pSekaijuApp->m_strSysxOpenPath.ReleaseBuffer ();
