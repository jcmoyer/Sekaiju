...
// ノート矩形を取得する
CRect CPianoRollKeyTimeView::GetNoteRect (MIDIEvent* pNoteOnEvent, long lLogPixelsX, long lLogPixelsY) {
	ASSERT (pNoteOnEvent);
	...
	long x = pPianoRollFrame->TimetoX (lTime, lLogPixelsX);
	long w = pPianoRollFrame->TimetoX (lDuration, lLogPixelsX);
	long y = pPianoRollFrame->KeytoY (lKey, lLogPixelsY);
	long h = pPianoRollFrame->GetKeyZoom () * lLogPixelsY / 96; // 20211015修正
...
// 旧位置の縦線消去
void CPianoRollKeyTimeView::EraseOldLine (CDC* pDC, long lLogPixelsX, long lLogPixelsY) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CRect rcClient;
...
// 現在位置の縦線描画
void CPianoRollKeyTimeView::DrawCurLine (CDC* pDC, long lLogPixelsX, long lLogPixelsY) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CRect rcClient;
	GetClientRect (&rcClient);
	rcClient += CSize (pPianoRollFrame->GetTimeScrollPos (), pPianoRollFrame->GetKeyScrollPos ());
	long x = pPianoRollFrame->TimetoX (m_lCurTime, lLogPixelsX);
...
void CPianoRollKeyTimeView::OnDraw (CDC* pDC) {
	...
	MIDIData* pMIDIData = pSekaijuDoc->m_pMIDIData;
	long x, y, lTime;
	long lVisibleLeftTime = pPianoRollFrame->GetVisibleLeftTime (lLogPixelsX);
	long lVisibleRightTime = pPianoRollFrame->GetVisibleRightTime (lLogPixelsX);
	long lVisibleTopKey = pPianoRollFrame->GetVisibleTopKey (lLogPixelsY);
	long lVisibleBottomKey = pPianoRollFrame->GetVisibleBottomKey (lLogPixelsY);
	...
	for (i = lVisibleBottomKey; i <= lVisibleTopKey + 1; i++) {
		y = pPianoRollFrame->KeytoY (i, lLogPixelsY);
		// 黒鍵部の塗り潰し
		temp = i % 12;
	...
	if (lTimeMode == MIDIDATA_TPQNBASE) {
		...
		for (i = lLeftMeasure; i <= lRightMeasure; i++) {
			...
			x = pPianoRollFrame->TimetoX (lTime, lLogPixelsX);
			...
			for (long j = 1; j < lnn; j++) {
				x = pPianoRollFrame->TimetoX (lTime + j * lUnitTick, lLogPixelsX);
				pDC->MoveTo (x, rcClient.top);
				pDC->LineTo (x, rcClient.bottom);
			}
		}
	...
	for (j = lVisibleBottomKey; j <= lVisibleTopKey; j++) {
		if (j % 12 == 0) {
			y = pPianoRollFrame->KeytoY (j, lLogPixelsX);
			pDC->MoveTo (rcClient.left, y);
			pDC->LineTo (rcClient.right, y);
		}
	}
			...
			forEachEvent (pMIDITrack, pMIDIEvent) {
				if (MIDIEvent_IsNoteOn (pMIDIEvent)) {
					if (pMIDIEvent->m_lTime < lVisibleRightTime) {
						x = pPianoRollFrame->TimetoX (MIDIEvent_GetTime (pMIDIEvent), lLogPixelsX);
						w = pPianoRollFrame->TimetoX (MIDIEvent_GetDuration (pMIDIEvent), lLogPixelsX);
						y = pPianoRollFrame->KeytoY (MIDIEvent_GetKey (pMIDIEvent), lLogPixelsY);
	...
	// 現在位置の描画
	DrawCurLine (pDC, lLogPixelsX, lLogPixelsY);
...
void CPianoRollKeyTimeView::OnTimer (UINT_PTR nIDEvent) {
	...
	if (nIDEvent == 0x11) {
		if (pSekaijuDoc->m_pMIDIData && pSekaijuDoc->m_pMIDIClock) {
			...
			// リアルタイム入力中の画面更新
			if (m_lOldTime != m_lCurTime && pSekaijuApp->m_bRecording) {
				long lOldX = pPianoRollFrame->TimetoX (m_lOldTime, lLogPixelsX);
				long lCurX = pPianoRollFrame->TimetoX (m_lCurTime, lLogPixelsY);
				long lTop = rcClient.top;
				long lBottom = rcClient.bottom;
				InvalidateRect (CRect (lOldX, lTop, lCurX, lBottom), FALSE);
			}
			// 前回の時刻と現在の時刻が違っている場合のみ
			if (m_lOldTime != m_lCurTime) {
				EraseOldLine (pDC, lLogPixelsX, lLogPixelsY);
				DrawCurLine (pDC, lLogPixelsX, lLogPixelsY);
			}
			...
			if (pPianoRollFrame->m_bAutoPageUpdate == TRUE) {
				...
				if (lTimeMode == MIDIDATA_TPQNBASE) {
					...
					MIDIData_BreakTime (pMIDIData, pPianoRollFrame->GetVisibleRightTime (lLogPixelsX),
					...
					if (m_lCurTime >= lTempRightTime) {
						...
						int x = pPianoRollFrame->TimetoX (lTempRightTime, lLogPixelsX);
						pPianoRollFrame->SetTimeScrollPos (x);
					}
					// 現在位置が左側へはみ出した場合
					long lLeftMeasure; long lLeftBeat; long lLeftTick; long lTempLeftTime;
					MIDIData_BreakTime (pMIDIData, pPianoRollFrame->GetVisibleLeftTime (lLogPixelsX),
					...
					if (m_lCurTime < lTempLeftTime) {
						...
						int x = pPianoRollFrame->TimetoX (lTempLeftTime, lLogPixelsX);
						pPianoRollFrame->SetTimeScrollPos (x);
					}
				}
				else {
					// 現在位置が右側へはみ出した場合
					long lRightFrameNumber = pPianoRollFrame->GetVisibleRightTime (lLogPixelsX) / lTimeResolution;
					long lTempRightTime = lRightFrameNumber * lTimeResolution;
					if (m_lCurTime >= lTempRightTime) {
						lTempRightTime = (m_lCurTime / lTimeResolution) * lTimeResolution;
						long x = pPianoRollFrame->TimetoX (lTempRightTime, lLogPixelsX);
						pPianoRollFrame->SetTimeScrollPos (x);
					}
					// 現在位置が左側へはみ出した場合
					long lLeftFrameNumber = pPianoRollFrame->GetVisibleLeftTime (lLogPixelsX) / lTimeResolution;
					long lTempLeftTime = lLeftFrameNumber * lTimeResolution;
					if (m_lCurTime < lTempLeftTime) {
						lTempLeftTime = (m_lCurTime / lTimeResolution) * lTimeResolution;
						long x = pPianoRollFrame->TimetoX (lTempLeftTime, lLogPixelsX);
						pPianoRollFrame->SetTimeScrollPos (x);
					}
				}
			}
	...
	// リアルタイム入力中の自動画面更新処理
		...
		if (pSekaijuDoc->m_pMIDIData && pSekaijuDoc->m_pMIDIClock && pSekaijuApp->m_bRecording) {
			...
			if (m_lOldTimeRealTimeInput != lCurTime) {
				long lOldX = pPianoRollFrame->TimetoX (m_lOldTimeRealTimeInput, lLogPixelsX);
				long lCurX = pPianoRollFrame->TimetoX (m_lCurTime, lLogPixelsY);
				long lTimeScrollPos = pPianoRollFrame->GetTimeScrollPos ();
				long lTop = rcClient.top;
				long lBottom = rcClient.bottom;
...
void CPianoRollKeyTimeView::OnKeyDown (UINT nChar, UINT nRepCnt, UINT nFlags) {
	...
	case VK_DELETE:
		...
		if (GetCapture () == this) {
			...
			long lOldKey = pPianoRollFrame->YtoKey (m_ptMouseMove.y, lLogPixelsY);
			switch (m_lTempTool) {
			// ペン
...
void CPianoRollKeyTimeView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	m_lTempKey = pPianoRollFrame->YtoKey (point.y, lLogPixelsY);
	m_lTempTime = pPianoRollFrame->XtoTime (point.x, lLogPixelsX);
			...
			if (pPianoRollFrame->IsTrackVisible (i)) { // 20090629修正
				forEachEventInverse (pTempTrack, pTempEvent) {
					if (MIDIEvent_IsNoteOn (pTempEvent) && MIDIEvent_IsNote (pTempEvent)) {
						CRect rcNote = GetNoteRect (pTempEvent, lLogPixelsX, lLogPixelsY);
						if (rcNote.top <= point.y && point.y <= rcNote.bottom) {
							...
							if (rcNote.left <= point. x && point.x < rcNote.left + rcNote.Width () / 4) {
								...
								rcRegion = GetNoteRect (m_pTempEvent, lLogPixelsX, lLogPixelsY);
			...
			if (pPianoRollFrame->IsTrackVisible (i)) {
				forEachEvent (pTempTrack, pTempEvent) {
					if (MIDIEvent_IsNoteOn (pTempEvent) && MIDIEvent_IsNote (pTempEvent) &&
						...
						rcNote = GetNoteRect (pTempEvent, lLogPixelsX, lLogPixelsY);
						if (rcNote.top <= point.y && point.y <= rcNote.bottom &&
							rcNote.left <= point.x && point. x<= rcNote.right) {
							bPtInNote = TRUE;
							break;
...
void CPianoRollKeyTimeView::OnLButtonUp (UINT nFlags, CPoint point) {
	...
	long lDownKey = pPianoRollFrame->YtoKey (m_ptMouseDown.y, lLogPixelsY);
	long lOldKey = pPianoRollFrame->YtoKey (m_ptMouseMove.y, lLogPixelsY);
	long lNewKey = pPianoRollFrame->YtoKey (point.y, lLogPixelsY);
	long lDownTime = pPianoRollFrame->XtoTime (m_ptMouseDown.x, lLogPixelsX);
	long lOldTime = pPianoRollFrame->XtoTime (m_ptMouseMove.x, lLogPixelsX);
	long lNewTime = pPianoRollFrame->XtoTime (point.x, lLogPixelsX);
				...
				if (lOldKey != lNewKey) {
					if (m_lTempMode == 0x0402 || m_lTempMode == 0x0403) {
						...
						for (i = 0; i < lTempSelectedEventArrayCount; i++) {
							...
							rcRegion = GetNoteRect (pTempEvent, lLogPixelsX, lLogPixelsY);
							rcRegion.InflateRect (1, 1);
							InvalidateRect (rcRegion - sizWndOrg);
...
// マウスホイールが回された時
BOOL CPianoRollKeyTimeView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	// ユニバーサルスクロール停止
	...
	if (lFlags & MK_CONTROL) {
		...
		long lKeyZoom = pPianoRollFrame->GetKeyZoom ();
		lKeyScrollPos -= lKeyZoom * 4 * lLogPixelsY / 96 * lDelta / 120;
