...
void CMusicalScoreTrackScaleView::DrawFClef 
	...
	pt[2].x = ptCenter.x + rx * 3 * lLogPixelsY / 96 - rx / 2 * lLogPixelsX / 96;
	pt[2].y = ptCenter.y - ry * 1 * lLogPixelsY / 96;
...
void CMusicalScoreTrackScaleView::DrawSharp
	...
	POINT pt[9];
	pt[0].x = ptCenter.x - rx * lLogPixelsX / 96;
	pt[0].y = ptCenter.y + ry * lLogPixelsY / 96 + ry / 2 * lLogPixelsY / 96;
	pt[1].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[1].y = ptCenter.y + ry * lLogPixelsY / 96 - ry / 2 * lLogPixelsY / 96;
	pDC->Polyline (pt, 2);
	pt[0].x = ptCenter.x - rx * lLogPixelsX / 96;
	pt[0].y = ptCenter.y - ry * lLogPixelsY / 96 + ry / 2 * lLogPixelsY / 96;
	pt[1].x = ptCenter.x + rx * lLogPixelsX / 96;
...
BOOL CMusicalScoreTrackScaleView::MoveTextBox (long lTrackIndex, long lLogPixelsX, long lLogPixelsY) {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CMusicalScoreFrame* pMusicalScoreFrame = (CMusicalScoreFrame*) GetParent ();
	...
	CRect rcNewCell (40 * lLogPixelsX / 96, pTrackInfo->m_lTop * lTrackZoom * lLogPixelsY / 96, 
		rcClient.right, pTrackInfo->m_lTop * lTrackZoom * lLogPixelsY);
	rcNewCell -= CSize (0, pMusicalScoreFrame->GetTrackScrollPos ());
	long lHeight = (pSekaijuApp->m_theFontOption.m_lFontSize[0] + 4) * lLogPixelsY / 96;
...
void CMusicalScoreTrackScaleView::OnDraw (CDC* pDC) {
	...
	forEachTrack (pMIDIData, pTempTrack) {
		if (pMusicalScoreFrame->IsTrackVisible (i)) {
			if (lVisibleTopTrack <= i && i <= lVisibleBottomTrack) { 
				...
				_sntprintf_s (szBuf, 256, 255, _T("%d"), i + (bTrackZeroOrigin ? 0 : 1));
				...
				CRect theRect (0, pTrackInfo->m_lTop * lTrackZoom * lLogPixelsY / 96, 
					32 * lLogPixelsX / 96, 
					(pTrackInfo->m_lTop + pTrackInfo->m_lHeight) * lTrackZoom * lLogPixelsY / 96);
	...
	forEachTrack (pMIDIData, pTempTrack) {
		if (pMusicalScoreFrame->IsTrackVisible (i)) {
			if (lVisibleTopTrack <= i && i <= lVisibleBottomTrack) { 
				MusicalScoreTrackInfo* pTrackInfo = pMusicalScoreFrame->GetTrackInfo (i);
				long lTrackTop = pTrackInfo->m_lTop * lLogPixelsX / 96;
				long lTrackHeight = pTrackInfo->m_lHeight * lLogPixelsX / 96;
				long lColor = MIDITrack_GetForeColor (pTempTrack);
				pDC->SetTextColor (lColor);
				CString strTrackName;
				strTrackName = GetCellString (i);
				CRect rcTrackName (40 * lLogPixelsX / 96, lTrackTop * lTrackZoom, rcClient.right, 
		...
		if (pMusicalScoreFrame->IsTrackVisible (i)) {
			if (lVisibleTopTrack <= i && i <= lVisibleBottomTrack) { 
				...
				case 2: // ヘ音記号
					for (ii = -2; ii <= 2; ii++) {
						y = yc + ii * 2 * ry;
						pDC->MoveTo (40 * lLogPixelsX / 96, y);
				...
				case 3:
					...
					y2 = yc + 10 * ry;
					pDC->FillSolidRect
						(40 * lLogPixelsX / 96 - 1, y1, 1, y2 - y1 + 1, pSekaijuApp->m_theColorOption.m_lVertColor[0]);
					pDC->FillSolidRect
						(40 * lLogPixelsX / 96 - 5, y1, 2, y2 - y1 + 1, pSekaijuApp->m_theColorOption.m_lVertColor[0]);
				}
			}
		}
		i++;
	...
	long lTime = pMusicalScoreFrame->XtoTime (lTimeScrollPos, lLogPixelsX);
	forEachTrack (pMIDIData, pTempTrack) {
		if (pMusicalScoreFrame->IsTrackVisible (i)) {
			if (lVisibleTopTrack <= i && i <= lVisibleBottomTrack) { 
				long lColor = MIDITrack_GetForeColor (pTempTrack);
...
void CMusicalScoreTrackScaleView::OnKillFocus (CWnd* pNewWnd) {
	_RPTF1 (_CRT_WARN, "CMusicalScoreTrackScaleView::OnKillFocus (pNewWnd=0x%p)\n", pNewWnd);
	CMusicalScoreFrame* pMusicalScoreFrame = (CMusicalScoreFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
...
void CMusicalScoreTrackScaleView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	if (0 <= point.x && point.x < 32) { 
		...
		m_lOldTrack = pMusicalScoreFrame->YtoTrackIndex (m_ptMouseDown.y, lLogPixelsY);
		m_lDownTrack = pMusicalScoreFrame->YtoTrackIndex (point.y, lLogPixelsY);
		m_lCurTrack = pMusicalScoreFrame->YtoTrackIndex (point.y, lLogPixelsY);
...
void CMusicalScoreTrackScaleView::OnMouseMove (UINT nFlags, CPoint point) {
	...
	if (GetCapture () == this) {
		...
		m_lOldTrack = pMusicalScoreFrame->YtoTrackIndex (m_ptMouseMove.y, lLogPixelsY);
		m_lCurTrack = pMusicalScoreFrame->YtoTrackIndex (point.y, lLogPixelsY);
...
void CMusicalScoreTrackScaleView::OnLButtonDblClk (UINT nFlags, CPoint point) {
	...
	if (40 * lLogPixelsX / 96 <= point.x && point.x < rcClient.right) {
		long lCurTrack = pMusicalScoreFrame->YtoTrackIndex (point.y, lLogPixelsY);
		long lNumTrack = MIDIData_CountTrack (pMIDIData);
		if (0 <= lCurTrack && lCurTrack < lNumTrack) {
			MusicalScoreTrackInfo* pTrackInfo = pMusicalScoreFrame->GetTrackInfo (lCurTrack);
			MoveTextBox (lCurTrack, lLogPixelsX, lLogPixelsY);
			// テキスト編集モードに突入
...
// マウスホイールが回された時
BOOL CMusicalScoreTrackScaleView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CMusicalScoreFrame* pMusicalScoreFrame = (CMusicalScoreFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	long lDelta = zDelta;
	...
	if (lFlags & MK_CONTROL) {
		...
		long ry = 4;
		lTrackScrollPos -= ry * 2 * 8 * lDelta / 120;
