...
CEventListIndexPropertyView::CEventListIndexPropertyView () {
	...
	m_lOldRowRealTimeInput = 0;
	m_lOldVisibleEventCountRealTimeInput = 0;
...
(CPoint pt, long* pRow, long* pColumn, long lLogPixelsX, long lLogPixelsY) {
	_ASSERT (pRow);
	_ASSERT (pColumn);
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	*pRow = pEventListFrame->YtoRow (pt.y, lLogPixelsY);
	*pColumn = pEventListFrame->XtoColumn (pt.x, lLogPixelsY);
	return TRUE;
}

// 指定セルがビューからはみ出した場合のオートスクロール処理
BOOL CEventListIndexPropertyView::AutoScrolltoShowCell (long lRow, long lColumn, long lLogPixelsX, long lLogPixelsY) {
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	CRect rcClient;
	...
	if (pEventListFrame->GetColumnLeft (lColumn, lLogPixelsX) < rcClient.left) {
		lNewPos = pEventListFrame->GetColumnLeft (lColumn, lLogPixelsX);
		pEventListFrame->SetColumnScrollPos (lNewPos);
	}
	// 指定セルがビューの右にはみ出した場合の処理
	else if (pEventListFrame->GetColumnLeft (lColumn, lLogPixelsX) + 
		pEventListFrame->GetColumnWidth (lColumn, lLogPixelsX) > rcClient.right) {
		lNewPos = pEventListFrame->GetColumnLeft (lColumn, lLogPixelsX) +
			pEventListFrame->GetColumnWidth (lColumn, lLogPixelsX) - rcClient.Width ();
		pEventListFrame->SetColumnScrollPos (lNewPos);
	}
	// 指定セルがビューの上にはみ出した場合の処理
	if (lRow * pEventListFrame->GetRowZoom () * lLogPixelsY / 96 < rcClient.top) {
		lNewPos = lRow * pEventListFrame->GetRowZoom () * lLogPixelsY / 96;
		pEventListFrame->SetRowScrollPos (lNewPos);		
	}
	// 指定セルがビューの下にはみ出した場合の処理
	else if ((lRow + 1) * pEventListFrame->GetRowZoom () * lLogPixelsY / 96  > rcClient.bottom) {
		lNewPos = (lRow + 1) * pEventListFrame->GetRowZoom () * lLogPixelsY / 96 - rcClient.Height ();
		pEventListFrame->SetRowScrollPos (lNewPos);
	}
	return TRUE;
}
...
BOOL CEventListIndexPropertyView::BeginTextEditing () {
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	long lVisibleEventCount = INT_PTRtolong (pEventListFrame->GetVisibleEventCount ());
	...
	if ((MIDIEvent_IsSequencerSpecific (pMIDIEvent) || MIDIEvent_IsSysExEvent (pMIDIEvent)) &&
		m_lCurColumn == 5) { // 20230103:修正追加(HexEditはカラム5のみとする。M:B:Tカラムは除外)
		m_theTextBox.m_bIsHexOnly = TRUE;
	}
...
BOOL CEventListIndexPropertyView::EndTextEditingOK () {
	...
	// 編集終了
	m_theTextBox.m_bIsHexOnly = FALSE;
	...
	case 5:
		if (MIDIEvent_IsNoteOn (pMIDIEvent) ||
			...
			if (pSekaijuApp->m_theGeneralOption.m_bChangeUpdate) {
				...
				pSekaijuApp->SendMinimumEachNoteOffEx (lPort, lChannel);
			...
			if (pSekaijuApp->m_theGeneralOption.m_bPatchSearch) {
				...
				if (pMIDIInstDef) {
					...
					while (0 <= lNewValue && lNewValue <= 127) {
						lBank = ((lBankMSB & 0x7F) << 7) | (lBankLSB & 0x7F);
						// 音色があるか調べる
						pMIDIPatchNameTable = 
		...
		if (MIDIEvent_IsNoteOn (pMIDIEvent)) {
			...
			if ((lNumber == 0 || lNumber == 32) && pSekaijuApp->m_theGeneralOption.m_bPatchSearch) {
				...
				if (pMIDIInstDef) {
					...
					while (0 <= lNewValue && lNewValue <= 127) {
						if (lNumber == 0) {
							lBank = ((lNewValue & 0x7F) << 7) | (lBankLSB & 0x7F);
						}
						else if (lNumber == 32) {
							lBank = ((lBankMSB & 0x7F) << 7) | (lNewValue & 0x7F);
						}
						// 音色があるか調べる
						pMIDIPatchNameTable = 
						MIDIInstrumentDefinition_GetPatchNameTable (pMIDIInstDef, lBank);				
...
CString CEventListIndexPropertyView::GetCellString (long lRow, long lColumn) {
	...
	case 0:
		...
		codestr2str (szBuf1, size_ttolong (_tcslen (szBuf1)), szBuf2, TSIZEOF (szBuf2) - 1);
	...
	case 5:
		...
		// プログラムチェンジ
			...
			if (pMIDIInstDef) {
				...
				long lBank = ((lBankMSB & 0x7F) << 7) | (lBankLSB & 0x7F);
		...
		// その他のイベント(16進ダンプ)
			...
			BYTE byBin[4096];
			TCHAR szText[4096 * 3 + 1]; // 20210102修正
...
BOOL CEventListIndexPropertyView::SetCellString (long lRow, long lColumn, CString strText) {
	...
	case 2:
		...
		// 20201102:テンポイベントの時刻が変更された場合は、時:分:秒:ミリ秒と演奏速度を更新する。
	...
	case 3:
		...
		if (0 <= i && i < 255) {
			...
			if (MIDIData_GetFormat (pMIDIData) == 1 && (pMIDITrack != pFirstTrack) &&
				...
				pSekaijuDoc->m_theCriticalSection.Unlock (); // 20220105 バグ修正
				pSekaijuApp->m_theCriticalSection.Unlock (); // 20220105 バグ修正
				CString strMsg;
	...
	case 5:
		...
		// システムエクスクルーシブ(通常)
			...
			long lRet = txt2bin ((TCHAR*)(LPCTSTR)strText, strText.GetLength (), byData, sizeof (byData));
			if (byData[0] != 0xF0) {
				byData[0] = 0xF0; // 20210106：可能な限り自動修正して正常終了する。
				pSekaijuDoc->m_theCriticalSection.Unlock ();
				pSekaijuApp->m_theCriticalSection.Unlock ();
				CString strMsg;
				...
				pSekaijuApp->m_theCriticalSection.Lock ();
				pSekaijuDoc->m_theCriticalSection.Lock ();
				//m_bSettingCellString = FALSE;
				//return FALSE;
			}
			BOOL bChanged = FALSE; 
			for (long j = 1; j < lRet - 1; j++) { // 20080809訂正
				if (byData[j] >= 0x80) {
					byData[j] = 0x7F; // 20210106：可能な限り自動修正して正常終了する。
					bChanged = TRUE;
					//m_bSettingCellString = 0;
					//return FALSE;
				}
			...
			if (byData[lRet - 1] != 0xF7) {
				...
				pSekaijuDoc->m_theCriticalSection.Unlock ();
				pSekaijuApp->m_theCriticalSection.Unlock ();
				VERIFY (strMsg.LoadString (IDS_SYSEX_DATA_MUST_END_WITH_F7_ARE_YOU_SURE_TO_FIX)); // 20220722：F7を修正するかY/Nに変更。
				int nRet = AfxMessageBox (strMsg, MB_ICONEXCLAMATION | MB_YESNO);
				pSekaijuApp->m_theCriticalSection.Lock ();
				pSekaijuDoc->m_theCriticalSection.Lock ();
				if (nRet == IDYES) {
					byData[lRet - 1] = 0xF7; // 20210106：可能な限り自動修正して正常終了する。
				}
				//m_bSettingCellString = 0;
				//return FALSE;
			}
...
int CEventListIndexPropertyView::DrawCurFrame (CDC* pDC, long lLogPixelsX, long lLogPixelsY) {
	CRect rcCurFrame = GetRectFromCell (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
	rcCurFrame.left -= 1;
	rcCurFrame.right -= 1;
	rcCurFrame.top -= 1;
	rcCurFrame.bottom -= 1;
...
// セルの長方形を取得
CRect CEventListIndexPropertyView::GetRectFromCell (long lRow, long lColumn, long lLogPixelsX, long lLogPixelsY) {
	...
	if (bCombined && lColumn >= 5) {
		rcCell.left = pEventListFrame->GetColumnLeft (5, lLogPixelsX);
		rcCell.right = rcCell.left + 
			pEventListFrame->GetColumnWidth (5, lLogPixelsX) + 
			pEventListFrame->GetColumnWidth (6, lLogPixelsX) + 
			pEventListFrame->GetColumnWidth (7, lLogPixelsX);
	}
	else {
		rcCell.left = pEventListFrame->GetColumnLeft (lColumn, lLogPixelsX);
		rcCell.right = rcCell.left + pEventListFrame->GetColumnWidth (lColumn, lLogPixelsX);
	}
	long lRowZoom = pEventListFrame->GetRowZoom ();
	rcCell.top = lRow * lRowZoom * lLogPixelsY / 96;
	rcCell.bottom = (lRow + 1) * lRowZoom * lLogPixelsY / 96;
...
BOOL CEventListIndexPropertyView::MoveTextBox (long lRow, long lColumn, long lLogPixelsX, long lLogPixelsY) {
	ASSERT (0 <= lRow && lRow <= 0x7FFFFFFF);
	ASSERT (0 <= lColumn && lColumn < 8);
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	long lButtonWidth = 12 * lLogPixelsX / 96; //::GetSystemMetrics (SM_CXVSCROLL);
	CRect rcNewCell = GetRectFromCell (lRow, lColumn, lLogPixelsX, lLogPixelsY);
	...
	m_theTextBox.MoveWindow 
		(rcNewCell.left, rcNewCell.top + rcNewCell.Height () / 2 - lHeight / 2, 
...
BOOL CEventListIndexPropertyView::MoveListBox (long lRow, long lColumn, long lLogPixelsX, long lLogPixelsY) {
	ASSERT (0 <= lRow && lRow <= 0x7FFFFFFF);
	ASSERT (0 <= lColumn && lColumn < 8);
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	CRect rcNewCell = GetRectFromCell (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
	rcNewCell -= CSize (pEventListFrame->GetColumnScrollPos (), pEventListFrame->GetRowScrollPos ());
	m_theListBox.MoveWindow 
		(rcNewCell.left, rcNewCell.bottom, 
		rcNewCell.Width (), rcNewCell.Height () + 64 * lLogPixelsY / 96);
...
BOOL CEventListIndexPropertyView::UpdateParentToolbarControl () {
	...
	if (MIDIEvent_IsControlChange (pMIDIEvent)) {
		long lNumber = MIDIEvent_GetNumber (pMIDIEvent);
		if (0 <= lNumber && lNumber <= 127) {
			pEventListFrame->m_pControlChangeListBox->SetCurSel (lNumber);
		}
	}
	return TRUE;
}
...
BOOL CEventListIndexPropertyView::ShowPopupMenu (CPoint ptMenu) {
	...
	else if (pSekaijuDoc->m_pTempEvent && MIDIEvent_IsControlChange (pSekaijuDoc->m_pTempEvent)) {
		VERIFY (theMenu.LoadMenu (IDR_POPUPMENU28));
	}
	else {
...
void CEventListIndexPropertyView::OnDraw (CDC* pDC) {
	...
	// 背景の塗りつぶし
	long lVisibleTopRow = pEventListFrame->GetVisibleTopRow (lLogPixelsY);
	long lVisibleBottomRow = pEventListFrame->GetVisibleBottomRow (lLogPixelsY);
	long lRowZoom = pEventListFrame->GetRowZoom ();
	for (i = lVisibleTopRow; i <= lVisibleBottomRow; i++) {
		...
		long y = i * lRowZoom * lLogPixelsY / 96;
		pDC->FillSolidRect (rcClient.left, y, rcClient.right, y + lRowZoom * lLogPixelsY / 96, lBackColor);
	}	
	...
	for (i = lVisibleTopRow; i <= lVisibleBottomRow; i++) {
		long y = (i + 1) * lRowZoom * lLogPixelsY / 96 - 1;
	...
	for (j = 0; j < 8; j++) {
		x += pEventListFrame->GetColumnWidth (j, lLogPixelsX);
		if (j == 5 || j == 6) {
			continue;
		}
	...
	for (i = lVisibleTopRow; i <= lVisibleBottomRow; i++) {
		...
		// ﾄﾗｯｸ名
		rcCell = GetRectFromCell (i, 0, lLogPixelsX, lLogPixelsY);
		rcText = rcCell;
		rcText.right -= lButtonWidth;
		_ASSERT (1 <= rcText.Height () && rcText.Height () <= 64);
		if (pMIDIEvent) {
			strText = GetCellString (i, 0);
			pDC->DrawText (strText, &rcText, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
		}
		// 時:分:秒:ﾐﾘ秒
		rcCell = GetRectFromCell (i, 1, lLogPixelsX, lLogPixelsY);
		rcText = rcCell;
		rcText.right -= lButtonWidth;
		if (pMIDIEvent) {
		...
		// 小節:拍:ﾃｨｯｸ
		rcCell = GetRectFromCell (i, 2, lLogPixelsX, lLogPixelsY);
		rcText = rcCell;
		rcText.right -= lButtonWidth;
		if (pMIDIEvent) {
		...
		// イベントの種類
		rcCell = GetRectFromCell (i, 3, lLogPixelsX, lLogPixelsY);
		rcText = rcCell;
		rcText.right -= lButtonWidth;
		if (pMIDIEvent) {
		...
		// チャンネル
		rcCell = GetRectFromCell (i, 4, lLogPixelsX, lLogPixelsY);
		rcText = rcCell;
		rcText.right -= lButtonWidth;
		if (pMIDIEvent) {
		...
		// 値1
		rcCell = GetRectFromCell (i, 5, lLogPixelsX, lLogPixelsY);
		rcText = rcCell;
		rcText.right -= lButtonWidth;
		if (pMIDIEvent) {
		...
		if (pMIDIEvent) {
			...
			if (0x80 <= lEventKind && lEventKind <= 0xEF) {
				rcCell = GetRectFromCell (i, 6, lLogPixelsX, lLogPixelsY);
				rcText = rcCell;
				rcText.right -= lButtonWidth;
				strText = GetCellString (i, 6);
				pDC->DrawText (strText, &rcText, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
			
				rcCell = GetRectFromCell (i, 7, lLogPixelsX, lLogPixelsY);
				rcText = rcCell;
				rcText.right -= lButtonWidth;
	...
	for (i = lVisibleTopRow; i <= lVisibleBottomRow; i++) {
		...
		for (j = 0; j < jMax; j++) {
			...
			rcCell = GetRectFromCell (i, j, lLogPixelsX, lLogPixelsY);
			rcButton  = rcCell;
			POINT pt[3];
			// ドロップダウンボタンの場合
			if (bDropDown) {
				...
				pt[0].y = rcCell.top + rcCell.Height () * 1 / 2 + 1 * lLogPixelsY / 96;
				pt[1].x = pt[0].x - 2 * lLogPixelsX / 96;
				pt[1].y = pt[0].y - 2 * lLogPixelsY / 96;
				pt[2].x = pt[0].x + 2 * lLogPixelsX / 96;
				pt[2].y = pt[0].y - 2 * lLogPixelsY / 96;
				pDC->Polygon (pt, 3);
			}
			// 上下ボタンの場合
			else {
				// 上ボタン
				rcButton.left = rcCell.right - lButtonWidth;
				rcButton.bottom = rcCell.top + rcCell.Height () / 2;
				pDC->FillSolidRect (&rcButton, ::GetSysColor (COLOR_3DFACE));
				...
				pt[0].x = rcCell.right - lButtonWidth / 2;
				pt[0].y = rcCell.top + rcCell.Height () * 1 / 4 - 1 * lLogPixelsY / 96;
				pt[1].x = pt[0].x - 2 * lLogPixelsX / 96;
				pt[1].y = pt[0].y + 2 * lLogPixelsY / 96;
				pt[2].x = pt[0].x + 2 * lLogPixelsX / 96;
				pt[2].y = pt[0].y + 2 * lLogPixelsY / 96;
				pDC->Polygon (pt, 3);
				// 下ボタン
				rcButton.top = rcCell.top + rcCell.Height () / 2;
				...
				pt[0].x = rcCell.right - lButtonWidth / 2;
				pt[0].y = rcCell.top + rcCell.Height () * 3 / 4 + 1 * lLogPixelsY / 96;
				pt[1].x = pt[0].x - 2 * lLogPixelsX / 96;
				pt[1].y = pt[0].y - 2 * lLogPixelsY / 96;
				pt[2].x = pt[0].x + 2 * lLogPixelsX / 96;
				pt[2].y = pt[0].y - 2 * lLogPixelsY / 96;
	...
	// カレントセルの枠描画
	DrawCurFrame (pDC, lLogPixelsX, lLogPixelsY);
...
BOOL CEventListIndexPropertyView::OnCreate (LPCREATESTRUCT lpcs) {
	...
	m_theTextBox.Create (WS_CHILD | ES_AUTOHSCROLL, 
		CRect (0, pEventListFrame->GetRowZoom () / 2 - 6, pEventListFrame->GetColumnWidth (0, lLogPixelsX),
		13), 
		this, IDC_TEXTBOX);
	m_theTextBox.SetFont (&(pSekaijuApp->m_theFont[0]));
...
void CEventListIndexPropertyView::OnKillFocus (CWnd* pNewWnd) {
	_RPTF1 (_CRT_WARN, "CEventListIndexPropertyView::OnKillFocus (pNewWnd=0x%p)\n", pNewWnd);
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	pSekaijuDoc->m_theCriticalSection.Lock ();
...
void CEventListIndexPropertyView::OnKeyDown (UINT nChar, UINT nRepCnt, UINT nFlags) {
	...
	if (IsTextEditing () == FALSE && IsListSelecting () == FALSE
		...
		CRect rcOldCell = GetRectFromCell (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
	...
	case VK_UP:
		...
		AutoScrolltoShowCell (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
		MoveTextBox (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
		MoveListBox (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
	...
	case VK_PRIOR:
		...
		m_lCurButtonState = 0x0000;
		// カレントセルを1ページ上へ移動
		lVisibleRows = pEventListFrame->GetVisibleBottomRow (lLogPixelsY) - pEventListFrame->GetVisibleTopRow (lLogPixelsY);
	...
	case VK_RETURN:
		...
		if (IsTextEditing () == FALSE && IsListSelecting () == FALSE) {
			...
			MIDIEvent* pMIDIEvent = NULL;
			lVisibleEventCount = INT_PTRtolong (pEventListFrame->GetVisibleEventCount ());
...
void CEventListIndexPropertyView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	if (m_lCurButtonState == 0x0000) {
		long i = 0;
		long j = 0;
		GetCellFromPoint (point, &i, &j, lLogPixelsX, lLogPixelsY);
...
void CEventListIndexPropertyView::OnLButtonDblClk (UINT nFlags, CPoint point) {
	...
	CRect rcCell = GetRectFromCell (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
...
void CEventListIndexPropertyView::OnTimer (UINT_PTR nIDEvent) {	
	...
	// 再生時などのカレントセル自動移動用(ページ更新機能付き)(2016/01/10ちらつき修正)
		...
		if (pEventListFrame->m_bAutoPageUpdate == TRUE
			...
			if (i != m_lCurRow) {
				...
				// 旧フレームの消去
				DrawCurFrame (&dc, lLogPixelsX, lLogPixelsY);
				// カレントセルの更新								
				m_lCurRow = i;
				MoveTextBox (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
				MoveListBox (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
				...
				long lRowZoom = pEventListFrame->GetRowZoom ();
				if (m_lCurRow * lRowZoom * lLogPixelsY / 96 < rcClient.top) {
					n = m_lCurRow * lRowZoom * lLogPixelsY / 96;
					pEventListFrame->SetRowScrollPos (n);		
					MoveTextBox (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
					MoveListBox (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
				}
				// カレントセルがビューの下にはみ出した場合の処理
				else if ((m_lCurRow + 1) * lRowZoom * lLogPixelsY / 96 > rcClient.bottom) {
					n = m_lCurRow * lRowZoom * lLogPixelsY / 96;
					pEventListFrame->SetRowScrollPos (n);
					MoveTextBox (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
					MoveListBox (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
	...
	// リアルタイム入力中の自動画面更新処理
		...
		if (pSekaijuDoc->m_pMIDIData && pSekaijuDoc->m_pMIDIClock && pSekaijuApp->m_bRecording &&
			...
			CRect rcClient;
			GetClientRect (&rcClient);
			pEventListFrame->MakeVisibleEventArray (lLogPixelsX, lLogPixelsY); // 時間がかかるので注意。
			// 該当行のMIDIイベントを取得
			MIDIEvent* pMIDIEvent = NULL;
			long lCurTime = pSekaijuDoc->m_lNewTime;
			long lVisibleEventCount = INT_PTRtolong (pEventListFrame->GetVisibleEventCount ());
			...
			// リアルタイム入力中の画面更新
			if (m_lOldVisibleEventCountRealTimeInput != lVisibleEventCount &&
				m_lOldRowRealTimeInput != m_lCurRow) {
				InvalidateRect (rcClient, TRUE);
			}
			m_lOldRowRealTimeInput = m_lCurRow;
			m_lOldVisibleEventCountRealTimeInput = lVisibleEventCount;
			pSekaijuApp->m_theCriticalSection.Unlock ();
			pSekaijuDoc->m_theCriticalSection.Unlock ();
			::Sleep (0);
		}
...
// マウスホイールが回された時
BOOL CEventListIndexPropertyView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	long lDelta = zDelta;
	...
	// カレントセルの値の増減
		...
		// 新しいセルの描画
		rcNewCell = GetRectFromCell (m_lCurRow, m_lCurColumn, lLogPixelsX, lLogPixelsY);
		rcNewCell -= CSize (pEventListFrame->GetColumnScrollPos (), pEventListFrame->GetRowScrollPos ());
		rcNewCell.InflateRect (2, 2);
		InvalidateRect (&rcNewCell);
