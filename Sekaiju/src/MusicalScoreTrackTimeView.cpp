...
// ノート矩形を取得する(近々廃止予定)
CRect CMusicalScoreTrackTimeView::GetNoteRect (MIDIEvent* pNoteOnEvent, long lLogPixelsX, long lLogPixelsY) {
	ASSERT (pNoteOnEvent);
	ASSERT (FALSE);
	// この関数は使わないでください。
	...
	long x = pMusicalScoreFrame->TimetoX (lTime, lLogPixelsX);
	long w = pMusicalScoreFrame->TimetoX (lDuration, lLogPixelsX);
	long y = pMusicalScoreFrame->TrackIndexKeytoY (lTrackIndex, lKey, lKeySignature, lLogPixelsY);
...
CRect CMusicalScoreTrackTimeView::GetNoteRect (MusicalScoreNoteInfo* pNoteInfo, long lLogPixelsX, long lLogPixelsY) {
	CMusicalScoreFrame* pMusicalScoreFrame = (CMusicalScoreFrame*)GetParent ();
	...
	long lKey = MIDIEvent_GetKey (pNoteOnEvent);
	long lNoteX = pMusicalScoreFrame->TimetoX (lNoteOnTime, lLogPixelsX);
	long lNoteY = pMusicalScoreFrame->TrackIndexKeytoY (pTrack->m_lTempIndex, lKey, lKeySignature, lLogPixelsY);
	long rx = 4;
	long ry = 4;
...
// 旧位置の縦線消去
void CMusicalScoreTrackTimeView::EraseOldLine (CDC* pDC, long lLogPixelsX) {
	CMusicalScoreFrame* pMusicalScoreFrame = (CMusicalScoreFrame*)GetParent ();
	CRect rcClient;
...
// 現在位置の縦線描画
void CMusicalScoreTrackTimeView::DrawCurLine (CDC* pDC, long lLogPixelsX) {
	CMusicalScoreFrame* pMusicalScoreFrame = (CMusicalScoreFrame*)GetParent ();
	CRect rcClient;
	GetClientRect (&rcClient);
	rcClient += CSize (pMusicalScoreFrame->GetTimeScrollPos (), pMusicalScoreFrame->GetTrackScrollPos ());
	long x = pMusicalScoreFrame->TimetoX (m_lCurTime, lLogPixelsX);
...
void CMusicalScoreTrackTimeView::DrawTadpole 
	...
	if (lFlags & 0x00010000) {
		pt[0].x = ptCenter.x + rx * lLogPixelsX / 96;
		pt[0].y = ptCenter.y;
		pt[1].x = ptCenter.x + rx * lLogPixelsX / 96;
		pt[1].y = ptCenter.y - ry / 2* lLogPixelsY / 96;
		pt[2].x = ptCenter.x + rx / 2* lLogPixelsX / 96;
		pt[2].y = ptCenter.y - ry * lLogPixelsY / 96;
		pt[3].x = ptCenter.x;
		pt[3].y = ptCenter.y - ry * lLogPixelsY / 96;
		pt[4].x = ptCenter.x - rx * lLogPixelsX / 96;
		pt[4].y = ptCenter.y;
		pt[5].x = ptCenter.x - rx * lLogPixelsX / 96;
		pt[5].y = ptCenter.y + ry / 2 * lLogPixelsY / 96;
		pt[6].x = ptCenter.x - rx / 2 * lLogPixelsX / 96;
		pt[6].y = ptCenter.y + ry * lLogPixelsY / 96;
		pt[7].x = ptCenter.x;
		pt[7].y = ptCenter.y + ry * lLogPixelsY / 96;
		pt[8].x = ptCenter.x + rx * lLogPixelsX / 96;
		pt[8].y = ptCenter.y;
		pDC->Polyline (pt, 9); // 白玉
		pt[0].x = ptCenter.x;
		pt[0].y = ptCenter.y - ry * lLogPixelsY / 96;
		pt[1].x = ptCenter.x - rx / 2 * lLogPixelsX / 96;
		pt[1].y = ptCenter.y - ry * lLogPixelsY / 96;
		pt[2].x = ptCenter.x - rx * lLogPixelsX / 96;
		pt[2].y = ptCenter.y - ry / 2 * lLogPixelsX / 96;
		pt[3].x = ptCenter.x - rx * lLogPixelsX / 96;
		pt[3].y = ptCenter.y;
		pDC->Polygon (pt, 4); // 黒縁
		pt[0].x = ptCenter.x + rx * lLogPixelsX / 96;
		pt[0].y = ptCenter.y;
...
void CMusicalScoreTrackTimeView::DrawGClef 
	...
	pt[3].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[3].y = ptCenter.y - ry * lLogPixelsY / 96;
	pt[4].x = ptCenter.x + rx * 2 * lLogPixelsX / 96;
	pt[4].y = ptCenter.y;
	pt[5].x = ptCenter.x + rx * 2 * lLogPixelsX / 96;
	pt[5].y = ptCenter.y + ry * lLogPixelsY / 96;
	pt[6].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[6].y = ptCenter.y + ry * 2 * lLogPixelsY / 96;
	pt[7].x = ptCenter.x - rx * 1 * lLogPixelsX / 96;
	pt[7].y = ptCenter.y + ry * 2 * lLogPixelsY / 96;
	pt[8].x = ptCenter.x - rx * 2 * lLogPixelsX / 96;
	pt[8].y = ptCenter.y + ry * lLogPixelsY / 96;
	pt[9].x = ptCenter.x - rx * 2 * lLogPixelsX / 96;
	pt[9].y = ptCenter.y - ry * lLogPixelsY / 96;
	pt[10].x = ptCenter.x;
	pt[10].y = ptCenter.y - ry * 4 * lLogPixelsY / 96;
	pt[11].x = ptCenter.x + rx * 1 * lLogPixelsX / 96;
	pt[11].y = ptCenter.y - ry * 5 * lLogPixelsY / 96;
	pt[12].x = ptCenter.x + rx * 1 * lLogPixelsX / 96;
	pt[12].y = ptCenter.y - ry * 8 * lLogPixelsY / 96;
	pt[13].x = ptCenter.x;
	pt[13].y = ptCenter.y - ry * 6 * lLogPixelsY / 96;
	pt[14].x = ptCenter.x;
	pt[14].y = ptCenter.y - ry * 4 * lLogPixelsY / 96;
	pt[15].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[15].y = ptCenter.y + ry * 4 * lLogPixelsY / 96;
	pt[16].x = ptCenter.x;
	pt[16].y = ptCenter.y + ry * 5 * lLogPixelsY / 96;
	pt[17].x = ptCenter.x - rx * 1 / 2 * lLogPixelsX / 96;
	pt[17].y = ptCenter.y + ry * 5 * lLogPixelsY / 96;
	pDC->Polyline (pt, 18);
	pt[0].x = ptCenter.x - rx * lLogPixelsX / 96;
	pt[0].y = ptCenter.y; 
	pt[1].x = ptCenter.x;
	...
	pt[2].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[2].y = ptCenter.y - ry * lLogPixelsY / 96; 
	pt[3].x = ptCenter.x + rx * 2 * lLogPixelsX / 96;
	pt[3].y = ptCenter.y; 
	pt[4].x = ptCenter.x + rx * 2 * lLogPixelsX / 96;
	pt[4].y = ptCenter.y - ry * lLogPixelsY / 96; 
	pt[5].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[5].y = ptCenter.y - ry * 2 * lLogPixelsY / 96; 
	pt[6].x = ptCenter.x;
	pt[6].y = ptCenter.y - ry * 2 * lLogPixelsY / 96; 
	pt[7].x = ptCenter.x - rx * lLogPixelsX / 96;
	pt[7].y = ptCenter.y - ry * lLogPixelsY / 96;
	pDC->Polygon (pt, 8);
	pt[0].x = ptCenter.x - rx * 2 * lLogPixelsX / 96;
	pt[0].y = ptCenter.y - ry * 1 * lLogPixelsY / 96;
	pt[1].x = ptCenter.x - rx * 2 * lLogPixelsX / 96;
	pt[1].y = ptCenter.y - ry * 2 * lLogPixelsY / 96;
	pt[2].x = ptCenter.x + rx * 1 * lLogPixelsX / 96;
	pt[2].y = ptCenter.y - ry * 6 * lLogPixelsY / 96;
	pt[3].x = ptCenter.x + rx * 1 * lLogPixelsX / 96;
	pt[3].y = ptCenter.y - ry * 5 * lLogPixelsY / 96;
	pDC->Polygon (pt, 4);
	pt[0].x = ptCenter.x;
	pt[0].y = ptCenter.y - ry * 6 * lLogPixelsY / 96; 
	pt[1].x = ptCenter.x;
	pt[1].y = ptCenter.y - ry * 7 * lLogPixelsY / 96; 
	pt[2].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[2].y = ptCenter.y - ry * 8 * lLogPixelsY / 96;
	pt[3].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[3].y = ptCenter.y - ry * 7 * lLogPixelsY / 96;
	pDC->Polygon (pt, 4);
	pt[0].x = ptCenter.x - rx * 1 * lLogPixelsX / 96;
	pt[0].y = ptCenter.y + ry * 9 / 2 * lLogPixelsY / 96;
	pt[1].x = ptCenter.x - rx * 1 / 2 * lLogPixelsX / 96;
	pt[1].y = ptCenter.y + ry * 4 * lLogPixelsY / 96;
	pt[2].x = ptCenter.x;
	pt[2].y = ptCenter.y + ry * 9 / 2 * lLogPixelsY / 96;
	pt[3].x = ptCenter.x - rx * 1 / 2 * lLogPixelsX / 96;
	pt[3].y = ptCenter.y + ry * 5 * lLogPixelsY / 96;
	pDC->Polygon (pt, 4);
}
...
void CMusicalScoreTrackTimeView::DrawFClef 
	...
	pt[2].y = ptCenter.y - ry * lLogPixelsY / 96;
	pt[3].x = ptCenter.x - rx * lLogPixelsX / 96;
	pt[3].y = ptCenter.y - ry * 2 * lLogPixelsY / 96;
	pt[4].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[4].y = ptCenter.y - ry * 2 * lLogPixelsY / 96;
	pt[5].x = ptCenter.x + rx * 2 * lLogPixelsX / 96;
	pt[5].y = ptCenter.y - ry * lLogPixelsY / 96;
	pt[6].x = ptCenter.x + rx * 2 * lLogPixelsX / 96;
	pt[6].y = ptCenter.y + ry * lLogPixelsY / 96;
	pt[7].x = ptCenter.x;
	pt[7].y = ptCenter.y + ry * 4 * lLogPixelsY / 96;
	pt[8].x = ptCenter.x - rx * 2 * lLogPixelsX / 96;
	pt[8].y = ptCenter.y + ry * 6 * lLogPixelsY / 96;
	pDC->Polyline (pt, 9);
	pt[0].x = ptCenter.x - rx * 2 * lLogPixelsX / 96;
	pt[0].y = ptCenter.y;
	pt[1].x = ptCenter.x - rx * 3 / 2 * lLogPixelsX / 96;
	pt[1].y = ptCenter.y - ry * 1 / 2 * lLogPixelsY / 96;
	pt[2].x = ptCenter.x - rx * lLogPixelsX / 96;
	pt[2].y = ptCenter.y;
	pt[3].x = ptCenter.x - rx * 3 / 2 * lLogPixelsX / 96;
	pt[3].y = ptCenter.y + ry * 1 / 2 * lLogPixelsY / 96;
	pDC->Polygon (pt, 4);
	pt[0].x = ptCenter.x;
	pt[0].y = ptCenter.y - ry * 2 * lLogPixelsY / 96;
	pt[1].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[1].y = ptCenter.y - ry * 1 * lLogPixelsY / 96;
	pt[2].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[2].y = ptCenter.y + ry * 1 * lLogPixelsY / 96;
	pt[3].x = ptCenter.x;
	pt[3].y = ptCenter.y + ry * 4 * lLogPixelsY / 96;
	pt[4].x = ptCenter.x + rx * 2 * lLogPixelsX / 96;
	pt[4].y = ptCenter.y + ry * 1 * lLogPixelsY / 96;
	pt[5].x = ptCenter.x + rx * 2 * lLogPixelsX / 96;
	pt[5].y = ptCenter.y - ry * 1 * lLogPixelsY / 96;
	pt[6].x = ptCenter.x + rx * 1 * lLogPixelsX / 96;
	pt[6].y = ptCenter.y - ry * 2 * lLogPixelsY / 96;
	pDC->Polygon (pt, 7);
	pt[0].x = ptCenter.x + rx * 3 * lLogPixelsX / 96 + rx / 2 * lLogPixelsX / 96;
	pt[0].y = ptCenter.y - ry * 1 * lLogPixelsY / 96;
	pt[1].x = ptCenter.x + rx * 3 * lLogPixelsX / 96;
	pt[1].y = ptCenter.y - ry * 1 * lLogPixelsY / 96 - ry / 2 * lLogPixelsY / 96;
	pt[2].x = ptCenter.x + rx * 3 * lLogPixelsX / 96 - rx / 2 * lLogPixelsX / 96;
	pt[2].y = ptCenter.y - ry * 1 * lLogPixelsY / 96;
	pt[3].x = ptCenter.x + rx * 3 * lLogPixelsX / 96;
	pt[3].y = ptCenter.y - ry * 1 * lLogPixelsY / 96 + ry / 2 * lLogPixelsY / 96;
	pDC->Polygon (pt, 4);
	pt[0].x = ptCenter.x + rx * 3 * lLogPixelsX / 96 + rx / 2 * lLogPixelsX / 96;
	pt[0].y = ptCenter.y + ry * 1 * lLogPixelsY / 96;
	pt[1].x = ptCenter.x + rx * 3 * lLogPixelsX / 96;
	pt[1].y = ptCenter.y + ry * 1 * lLogPixelsX / 96 - ry / 2 * lLogPixelsY / 96;
	pt[2].x = ptCenter.x + rx * 3 * lLogPixelsX / 96 - rx / 2 * lLogPixelsX / 96;
	pt[2].y = ptCenter.y + ry * 1 * lLogPixelsY / 96;
	pt[3].x = ptCenter.x + rx * 3 * lLogPixelsX / 96;
	pt[3].y = ptCenter.y + ry * 1 * lLogPixelsY / 96 + ry / 2 * lLogPixelsY / 96;
...
void CMusicalScoreTrackTimeView::DrawFlat 
	...
	pt[4].y = ptCenter.y + ry * lLogPixelsY / 96;
	pt[5].x = ptCenter.x - rx / 2 * lLogPixelsX / 96;
	pt[5].y = ptCenter.y - ry * 4 * lLogPixelsY / 96;
	pDC->Polyline (pt, 6);
}
...
void CMusicalScoreTrackTimeView::DrawNatural
	...
	pt[1].y = ptCenter.y + ry * lLogPixelsY / 96;
	pt[2].x = ptCenter.x + rx / 2 * lLogPixelsX / 96;
	pt[2].y = ptCenter.y + ry * 3 / 4 * lLogPixelsY / 96;
	pDC->Polyline (pt, 3);
	pt[0].x = ptCenter.x - rx / 2 * lLogPixelsX / 96;
	pt[0].y = ptCenter.y - ry * 3 / 4 * lLogPixelsY / 96;
	pt[1].x = ptCenter.x + rx / 2 * lLogPixelsX / 96;
	pt[1].y = ptCenter.y - ry * lLogPixelsY / 96;
	pt[2].x = ptCenter.x + rx / 2 * lLogPixelsX / 96;
	pt[2].y = ptCenter.y + ry * 2 * lLogPixelsY / 96;
	pDC->Polyline (pt, 3);
};
...
void CMusicalScoreTrackTimeView::DrawSharp
	...
	POINT pt[9];
	pt[0].x = ptCenter.x - rx * lLogPixelsX / 96;
	pt[0].y = ptCenter.y + ry + ry / 2 * lLogPixelsY / 96;
	pt[1].x = ptCenter.x + rx * lLogPixelsX / 96;
	pt[1].y = ptCenter.y + ry - ry / 2 * lLogPixelsY / 96;
	pDC->Polyline (pt, 2);
	pt[0].x = ptCenter.x - rx * lLogPixelsX / 96;
	pt[0].y = ptCenter.y - ry + ry / 2 * lLogPixelsY / 96;
	pt[1].x = ptCenter.x + rx * lLogPixelsX / 96;
...
void CMusicalScoreTrackTimeView::DrawHorzAuxiliaryLine
	...
	ptCenter.y = y;
	POINT pt[2];
	pt[0].x = ptCenter.x - r * lLogPixelsX / 96;
	pt[0].y = ptCenter.y;
	pt[1].x = ptCenter.x + r * lLogPixelsX / 96;
	pt[1].y = ptCenter.y;
	pDC->Polyline (pt, 2);
}
...
void CMusicalScoreTrackTimeView::DrawDot
	(CDC* pDC, long x, long y, long rx, long ry, long lFlags, long lLogPixelsX, long lLogPixelsY) {
	POINT ptCenter;
	ptCenter.x = x + rx * 2 * lLogPixelsX / 96;
	ptCenter.y = y - ry / 2 * lLogPixelsY / 96;
	POINT pt[4];
	pt[0].x = ptCenter.x - 1 * lLogPixelsX / 96;
	pt[0].y = ptCenter.y;
	pt[1].x = ptCenter.x;
	pt[1].y = ptCenter.y - 1 * lLogPixelsY / 96;
	pt[2].x = ptCenter.x + 1 * lLogPixelsX / 96;
	pt[2].y = ptCenter.y;
	pt[3].x = ptCenter.x;
	pt[3].y = ptCenter.y + 1 * lLogPixelsY / 96;
	pDC->Polygon (pt, 4);
...
void CMusicalScoreTrackTimeView::DrawTieHalf
	...
	ptCenter.y;
	POINT pt[3];
	pt[0].x = (x2 > x1 ? x1 + rx * 1 * lLogPixelsX / 96 : x1 - rx * 1* lLogPixelsX / 96);
	pt[0].y = y + ry * 1 * lLogPixelsY / 96;
	pt[1].x = (x2 > x1 ? x1 + rx * 2 * lLogPixelsX / 96 : x1 - rx * 2* lLogPixelsX / 96);
	pt[1].y = y + ry * 2 * lLogPixelsY / 96;
	pt[2].x = x2;
	pt[2].y = y + ry * 2 * lLogPixelsY / 96;
	pDC->Polyline (pt, 3);

}

// 縦棒を描画する
void CMusicalScoreTrackTimeView::DrawPole (CDC* pDC, long x1, long x2, long y1, long y2, long rx, long ry, long lFlags, long lLogPixelsX, long lLogPixelsY) {
	POINT pt[2];
	pt[0].x = x1 + rx * lLogPixelsX / 96;
	pt[0].y = y1;
	pt[1].x = x1 + rx * lLogPixelsX / 96;
	pt[1].y = y2;
	pDC->Polyline (pt, 2);
}

// 単一旗を描画する
void CMusicalScoreTrackTimeView::DrawSingleFlag (CDC* pDC, long x1, long x2, long y1, long y2, long rx, long ry, long lFlags, long lLogPixelsX, long lLogPixelsY) {
	POINT pt[4];
	pt[0].x = x1 + rx * lLogPixelsX / 96;
	pt[0].y = y2;
	pt[1].x = x1 + rx * 3 * lLogPixelsX / 96;
	pt[1].y = y2 + ry * 2 * lLogPixelsY / 96;
	pt[2].x = x1 + rx * 3 * lLogPixelsX / 96;
	pt[2].y = y2 + ry * 3 * lLogPixelsY / 96;
	pt[3].x = x1 + rx * lLogPixelsX / 96;
	pt[3].y = y2 + ry * lLogPixelsY / 96;
	pDC->Polygon (pt, 4);
	pt[0].x = x1 + rx * 3 * lLogPixelsX / 96;
	pt[0].y = y2 + ry * 3 * lLogPixelsY / 96;
	pt[1].x = x1 + rx * lLogPixelsX / 96;
	pt[1].y = y2 + ry * 5 * lLogPixelsY / 96;
	pDC->Polyline (pt, 2);
}

// 連続旗を描画する
void CMusicalScoreTrackTimeView::DrawChainedFlag (CDC* pDC, long x1, long x2, long y1, long y2, long rx, long ry, long lFlags, long lLogPixelsX, long lLogPixelsY) {
	POINT pt[4];
	pt[0].x = x1 + rx * lLogPixelsX / 96;
	pt[0].y = y2;
	pt[1].x = x2 + rx * lLogPixelsX / 96;
	pt[1].y = y2;
	pt[2].x = x2 + rx * lLogPixelsX / 96;
	pt[2].y = y2 + ry * lLogPixelsY / 96;
	pt[3].x = x1 + rx * lLogPixelsX / 96;
	pt[3].y = y2 + ry * lLogPixelsY / 96;
	pDC->Polygon (pt, 4);
}
...
void CMusicalScoreTrackTimeView::DrawTripletSign (CDC* pDC, long x1, long x2, long y1, long y2, long rx, long ry, long lFlags, long lLogPixelsX, long lLogPixelsY) {
	POINT pt[3];
	pt[0].x = x1;
	pt[0].y = y2;
	pt[1].x = x1;
	pt[1].y = y1;
	pt[2].x = (x1 + x2) / 2 - 6 * lLogPixelsX / 96;
	pt[2].y = y1;
	pDC->Polyline (pt, 3);
	pt[0].x = (x1 + x2) / 2 + 6 * lLogPixelsX / 96;
	pt[0].y = y1;
	pt[1].x = x2;
	pt[1].y = y1;
	pt[2].x = x2;
	pt[2].y = y2;
	pDC->Polyline (pt, 3);
	CRect rcText ((x1 + x2) / 2 - 6 * lLogPixelsX / 96, y1 - 6, (x1 + x2) / 2 + 6, y1 + 6 * lLogPixelsX / 96);
	CString strText;
...
void CMusicalScoreTrackTimeView::DrawNote
	...
	long x = pMusicalScoreFrame->TimetoX (pNoteInfo->m_lNoteOnTime, lLogPixelsX);
	long y = 0;
	long rx = 4;
	long ry = 4;
	long i = 0;
	...
	case 1: // ト音記号
		if (lLineNo >= 47) {
			for (i = 47; i <= lLineNo; i += 2) {
				y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, i, lLogPixelsY);
				pDC->MoveTo (x - rx * 2 * lLogPixelsX / 96, y);
				pDC->LineTo (x + rx * 2 * lLogPixelsX / 96, y);
			}
		}
		else if (lLineNo <= 35) {
	...
	case 3: // 大譜表
		if (lLineNo >= 47) {
			...
			y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, 35, lLogPixelsY);
			pDC->MoveTo (x - rx * 2 * lLogPixelsX / 96, y);
			pDC->LineTo (x + rx * 2 * lLogPixelsX / 96, y);
		}
		else if (lLineNo <= 23) {
	...
	// 臨時記号の描画
	y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, lLineNo, lLogPixelsY);
	switch (lSF) {
	case 0:
		break;
	case 2:
		DrawFlat (pDC, x - (4 * rx) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
		DrawFlat (pDC, x - (2 * rx) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
		break;
	case 3:
		DrawFlat (pDC, x - (2 * rx) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
		break;
	case 4:
		DrawNatural (pDC, x - (2 * rx) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
		break;
	case 5:
		DrawSharp (pDC, x - (2 * rx) * lLogPixelsX / 96 , y, rx, ry, lLogPixelsX, lLogPixelsY);
		break;
	case 6:
		DrawSharp (pDC, x - (2 * rx) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
		DrawSharp (pDC, x - (4 * rx) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
		break;
	}
	...
	DrawTadpole (pDC, x, y, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
	...
	if (lDuration == lTimeResolution * 3 ||
		...
		DrawDot (pDC, x, y, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
	}
	...
	case 1: // ♪_
		x2 = pMusicalScoreFrame->TimetoX (pNoteInfo->m_lNoteOffTime - 15, lLogPixelsX);
		DrawTieHalf (pDC, x, x2, y, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
		break;
	case 2: // _♪
		x2 = pMusicalScoreFrame->TimetoX (pNoteInfo->m_lNoteOnTime - 15, lLogPixelsX);
		DrawTieHalf (pDC, x, x2, y, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
		break;
	case 3: // _♪_
		x2 = pMusicalScoreFrame->TimetoX (pNoteInfo->m_lNoteOffTime - 15, lLogPixelsX);
		...
		break;
	case 4: // ♪_|
		x2 = pMusicalScoreFrame->MeasuretoX (pNoteInfo->m_lNoteOffMeasure, lLogPixelsX);
		DrawTieHalf (pDC, x, x2, y, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
		break;
	case 5: // |_♪
		x2 = pMusicalScoreFrame->MeasuretoX (pNoteInfo->m_lNoteOnMeasure, lLogPixelsX);
		DrawTieHalf (pDC, x, x2, y, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
		break;
	case 6: // _♪_|
		x2 = pMusicalScoreFrame->TimetoX (pNoteInfo->m_lNoteOnTime - 15, lLogPixelsX);
	...
	long y2 = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, lLineNo2, lLogPixelsY);
	if (lDuration < lTimeResolution * 4) {
		DrawPole (pDC, x, x2, y, y2 - ry * 7 * lLogPixelsY / 96, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
	}
	...
	if (lDuration < lTimeResolution && lDuration != lTimeResolution * 2 / 3) {
		...
		if (pNoteGroupInfo->m_pFirstNoteInfo->m_lNoteOnTime == pNoteGroupInfo->m_pLastNoteInfo->m_lNoteOnTime) {
			...
			if (lDuration == lTimeResolution / 2 ||
				...
				DrawSingleFlag (pDC, x, x2, y, y2 - ry * 7 * lLogPixelsY / 96, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
			}
			...
			if (lDuration == lTimeResolution / 4 ||
				...
				lDuration == lTimeResolution / 8 ||
				lDuration == lTimeResolution / 12) {
				DrawSingleFlag (pDC, x, x2, y, y2 - ry * 5 * lLogPixelsY / 96, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
			}
			// 32分音符、3連32分音符
			if (lDuration == lTimeResolution / 8 ||
				lDuration == lTimeResolution / 12) {
				DrawSingleFlag (pDC, x, x2, y, y2 - ry * 3 * lLogPixelsY / 96, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
			}
		}
		// グループ内の音符が複数の時刻にまたがる場合
			...
			if (pNoteGroupInfo->m_pFirstNoteInfo->m_lNoteOnTime == pNoteInfo->m_lNoteOnTime) {
				x1[0] = x1[1] = x1[2] = x;
				x2[0] = x2[1] = x2[2] = pMusicalScoreFrame->TimetoX (pNoteInfo->m_lNoteOffTime - lTimeResolution / 8, lLogPixelsX);
			}
			// グループ内の最後の時刻の音符である場合
			else if (pNoteGroupInfo->m_pLastNoteInfo->m_lNoteOnTime == pNoteInfo->m_lNoteOnTime) {
				x1[0] = x1[1] = x1[2] = pMusicalScoreFrame->TimetoX (pNoteInfo->m_lNoteOnTime - lTimeResolution / 8, lLogPixelsX);
				x2[0] = x2[1] = x2[2] = x;
			}
			// グループ内の途中の時刻の音符である場合
			else {
			...
			if (lDuration == lTimeResolution / 2 ||
				...
				lDuration == lTimeResolution / 8 ||
				lDuration == lTimeResolution / 12) {
				DrawChainedFlag (pDC, x1[0], x2[0], y, y2 - ry * 7 * lLogPixelsY / 96, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
			}
			// 連旗2段目描画
			// 16分音符、付点16分音符、3連16分音符、32分音符、3連32分音符
			if (lDuration == lTimeResolution / 4 ||
				...
				lDuration == lTimeResolution / 8 ||
				lDuration == lTimeResolution / 12) {
				DrawChainedFlag (pDC, x1[1], x2[1], y, y2 - ry * 5 * lLogPixelsY / 96, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
			}
			// 連旗3段目描画
			// 32分音符、3連32分音符
			if (lDuration == lTimeResolution / 8 ||
				lDuration == lTimeResolution / 12) {
				DrawChainedFlag (pDC, x1[2], x2[2], y, y2 - ry * 3 * lLogPixelsY / 96, rx, ry, lFlags2, lLogPixelsX, lLogPixelsY);
			}
		}
	}
	...
	if (pNoteInfo->m_pTripletGroupInfo) {
		...
		if (pTripletGroupInfo->m_pFirstNoteInfo == pNoteInfo) {
			long x1 = pMusicalScoreFrame->TimetoX (pTripletGroupInfo->m_pFirstNoteInfo->m_lNoteOnTime, lLogPixelsX);
			long x2 = pMusicalScoreFrame->TimetoX (pTripletGroupInfo->m_pLastNoteInfo->m_lNoteOnTime, lLogPixelsX);
			long lMinLineNo = pMusicalScoreFrame->KeytoLineNo (pTripletGroupInfo->m_lMinKey, lKeySignature);
			long y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, lMinLineNo, lLogPixelsY);
			...
			DrawTripletSign (pDC, x1, x2, y + 4 * ry * lLogPixelsY / 96, y + 3 * ry * lLogPixelsY / 96, rx, ry, n12, lLogPixelsX, lLogPixelsY);
		}
	}
...
void CMusicalScoreTrackTimeView::DrawTimeAndKeySignature 
	...
	long x = pMusicalScoreFrame->MeasuretoX (lMeasure, lLogPixelsX);
	...
	CRect rcText1 (x + (rx * 10 + rx * 2 * abs(lsf)) * lLogPixelsX / 96, 0,
		x + (rx * 10 + rx * 2 * abs(lsf) + 24) * lLogPixelsX / 96, 0);
	CRect rcText2 (x + (rx * 10 + rx * 2 * abs(lsf)) * lLogPixelsX / 96, 0, 
		x + (rx * 10 + rx * 2 * abs(lsf) + 24) * lLogPixelsX / 96, 0);
	long j;
	switch (lTrackFlags & 0x0000000F) {
	case 1: // ト音記号
		y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, 39, lLogPixelsY);
		DrawGClef (pDC, x + (rx * 4) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
		if (lsf > 0) {
			for (j = 1; j <= lsf; j++) {
				y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, lGCrefSharpLineNo[j - 1], lLogPixelsY);
				DrawSharp (pDC, x + (rx * 8 + rx * 2 * j) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
			}
		}
		else if (lsf < 0) {
			for (j = 1; j <= -lsf; j++) {
				y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, lGCrefFlatLineNo[j - 1], lLogPixelsY);
				DrawFlat (pDC, x + (rx * 8 + rx * 2 * j) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
			}
		}
		y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, 41, lLogPixelsY);
		rcText1.top = y - ry * 4 * lLogPixelsY / 96;
		rcText1.bottom = y;
		pDC->DrawText (strText1, &rcText1, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
		rcText2.top = y;
		rcText2.bottom = y + ry * 4 * lLogPixelsY / 96;
		pDC->DrawText (strText2, &rcText2, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
		break;
	case 2: // ヘ音記号
		y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, 31, lLogPixelsY);
		DrawFClef (pDC, x + (rx * 4) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
		if (lsf > 0) {
			for (j = 1; j <= lsf; j++) {
				y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, lFCrefSharpLineNo[j - 1], lLogPixelsY);
				DrawSharp (pDC, x + (rx * 8 + rx * 2 * j) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
			}
		}
		else if (lsf < 0) {
			for (j = 1; j <= -lsf; j++) {
				y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, lFCrefFlatLineNo[j - 1], lLogPixelsY);
				DrawFlat (pDC, x + (rx * 8 + rx * 2 * j) * lLogPixelsX / 96, y, rx, ry, lLogPixelsX, lLogPixelsY);
			}
		}
		y = pMusicalScoreFrame->TrackIndexLineNotoY (lTrackIndex, 29, lLogPixelsY);
		rcText1.top = y - ry * 4 * lLogPixelsY / 96;
		rcText1.bottom = y;
		pDC->DrawText (strText1, &rcText1, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
		rcText2.top = y;
...
void CMusicalScoreTrackTimeView::OnDraw (CDC* pDC) {
	...
	long x, y, lTime;
	long lVisibleLeftTime = pMusicalScoreFrame->GetVisibleLeftTime (lLogPixelsX);
	long lVisibleRightTime = pMusicalScoreFrame->GetVisibleRightTime (lLogPixelsX);
	long lVisibleTopTrack = pMusicalScoreFrame->GetVisibleTopTrack (lLogPixelsY);
	long lVisibleBottomTrack = pMusicalScoreFrame->GetVisibleBottomTrack (lLogPixelsY);
	...
	long rx = 4 * lLogPixelsX / 96;
	long ry = 4 * lLogPixelsY / 96;
	long i, j;
	i = 0;
	forEachTrack (pMIDIData, pMIDITrack) {
		if (pMusicalScoreFrame->IsTrackVisible (i)) {
			if (lVisibleTopTrack <= i && i <= lVisibleBottomTrack) {
				...
				long yc = (lTrackTop + lTrackHeight / 2) * lTrackZoom * lLogPixelsY / 96;
				long ii;
				switch (lTrackFlags & 0x0000000F) {
				case 0: // 表示しない
					break;
	...
	if (lTimeMode == MIDIDATA_TPQNBASE) {
		...
		for (i = lLeftMeasure; i <= lRightMeasure; i++) {
			...
			x = pMusicalScoreFrame->MeasuretoX (i, lLogPixelsX);
			...
			for (long j = 0; j < lnn; j++) {
				x = pMusicalScoreFrame->TimetoX (lTime + j * lUnitTick, lLogPixelsX);
				pDC->MoveTo (x, rcClient.top);
				pDC->LineTo (x, rcClient.bottom);
			}
		}
		...
		for (i = lLeftFrameNumber; i <= lRightFrameNumber; i++) {
			...
			x = pMusicalScoreFrame->TimetoX (lTime, lLogPixelsX);
			pDC->SelectObject (&penMeasure);
			pDC->MoveTo (x, rcClient.top);
			pDC->LineTo (x, rcClient.bottom);
		}
				...
				if (lVisibleLeftTime <= lTime && lTime <= lVisibleRightTime) {
					...
					forEachTrack (pMIDIData, pTempTrack) {
						if (pMusicalScoreFrame->IsTrackVisible (i)) {
							...
							DrawTimeAndKeySignature (pDC, i, lTime, lLogPixelsX, lLogPixelsY);
							pDC->SelectObject (pOldPen);
	...
	forEachTrack (pMIDIData, pMIDITrack) {
		if (pMusicalScoreFrame->IsTrackVisible (i)) {
			if (lVisibleTopTrack <= i && i <= lVisibleBottomTrack) {
				...
				long jMax = INT_PTRtolong (pTrackInfo->m_theNoteInfoArray.GetSize ());
				for (j = 0; j < jMax; j++) {
					...
					if (lVisibleLeftTime <= pNoteInfo->m_lNoteOnTime && pNoteInfo->m_lNoteOnTime <= lVisibleRightTime || 
						...
						DrawNote (pDC, i, pNoteInfo, 0x00000000, lLogPixelsX, lLogPixelsY);
						pDC->SelectObject (pOldPen);
						pDC->SelectObject (pOldBrush);
					}
				}
...
void CMusicalScoreTrackTimeView::OnTimer (UINT_PTR nIDEvent) {
		...
		if (pSekaijuDoc->m_pMIDIData && pSekaijuDoc->m_pMIDIClock) {
			...
			if (pMusicalScoreFrame->m_bAutoPageUpdate == TRUE) {
				...
				if (lTimeMode == MIDIDATA_TPQNBASE) {
					...
					MIDIData_BreakTime (GetDocument()->m_pMIDIData, pMusicalScoreFrame->GetVisibleRightTime (lLogPixelsX),
					...
					if (m_lCurTime >= lTempRightTime) {
						...
						int x = pMusicalScoreFrame->MeasuretoX2 (lNewMeasure, lLogPixelsX) + 1;
						pMusicalScoreFrame->SetTimeScrollPos (x);
					}
					// 現在位置が左側へはみ出した場合
					long lLeftMeasure; long lLeftBeat; long lLeftTick; long lTempLeftTime;
					MIDIData_BreakTime (GetDocument()->m_pMIDIData, pMusicalScoreFrame->GetVisibleLeftTime (lLogPixelsX),
					...
					// 現在位置が右側へはみ出した場合
					long lRightFrameNumber = pMusicalScoreFrame->GetVisibleRightTime (lLogPixelsX) / lTimeResolution;
					...
					if (m_lCurTime >= lTempRightTime) {
						...
						int x = pMusicalScoreFrame->MeasuretoX2 (lNewFrameNumber, lLogPixelsX) + 1;
						pMusicalScoreFrame->SetTimeScrollPos (x);
					}
					// 現在位置が左側へはみ出した場合
					long lLeftFrameNumber = pMusicalScoreFrame->GetVisibleLeftTime (lLogPixelsX) / lTimeResolution;
	...
	// リアルタイム入力中の自動画面更新処理
	else if (nIDEvent == 0x12) {
		if (pSekaijuDoc->m_pMIDIData && pSekaijuDoc->m_pMIDIClock && pSekaijuApp->m_bRecording) {
			pSekaijuDoc->m_theCriticalSection.Lock ();
			CRect rcClient;
			GetClientRect (&rcClient);
			long lCurTime = MIDIClock_GetTickCount (GetDocument ()->m_pMIDIClock);
			// リアルタイム入力中の画面更新
			if (m_lOldTimeRealTimeInput != lCurTime) {
				long lOldX = pMusicalScoreFrame->TimetoX (m_lOldTimeRealTimeInput, lLogPixelsX);
				long lCurX = pMusicalScoreFrame->TimetoX (m_lCurTime, lLogPixelsY);
				long lTimeScrollPos = pMusicalScoreFrame->GetTimeScrollPos ();
				long lTop = rcClient.top;
				long lBottom = rcClient.bottom;
				CRect rcUpdate = CRect (lOldX - lTimeScrollPos - 200 * lLogPixelsX / 96, lTop, lCurX - lTimeScrollPos, lBottom);
				pMusicalScoreFrame->UpdateTrackInfoArray (); // 時間がかかるので注意。
				InvalidateRect (rcUpdate, TRUE);
			}
			m_lOldTimeRealTimeInput = lCurTime;
			pSekaijuDoc->m_theCriticalSection.Unlock ();
			::Sleep (0);
		}
	}
...
void CMusicalScoreTrackTimeView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	long lsf, lmi;
	m_lTempTime = pMusicalScoreFrame->XtoTime (point.x, lLogPixelsX);
	...
	case ID_MUSICALSCORE_PEN:
		...
		forEachTrackInverse (pMIDIData, pTempTrack) {
			if (pMusicalScoreFrame->IsTrackVisible (i)) {
				...
				long lNumNoteInfo = INT_PTRtolong (pTrackInfo->m_theNoteInfoArray.GetSize ());
				long j;
				for (j = 0; j < lNumNoteInfo; j++) {
					MusicalScoreNoteInfo* pNoteInfo = (MusicalScoreNoteInfo*)(pTrackInfo->m_theNoteInfoArray.GetAt (j));
					CRect rcNote = GetNoteRect (pNoteInfo, lLogPixelsX, lLogPixelsY);
					// 既存の音符上をクリックした場合
		...
		if (m_pTempEvent == NULL) {
			...
			m_lTempTrackIndex = pMusicalScoreFrame->YtoTrackIndex (point.y, lLogPixelsY);
			...
			m_lTempKey = pMusicalScoreFrame->TrackIndexYtoKey (m_lTempTrackIndex, point.y, lKeySignature, lLogPixelsY);
			m_lMouseDownKey = m_lTempKey;
			m_lMouseMoveKey = m_lTempKey;
...
void CMusicalScoreTrackTimeView::OnRButtonDown (UINT nFlags, CPoint point) {
		...
		case ID_MUSICALSCORE_SELECT:
			...
			if (m_lTempMode == 0x0402 || m_lTempMode == 0x0403) {
				...
				if (lOldKey != lNewKey) {
					...
					long lTempSelectedEventArrayCount = INT_PTRtolong (m_theTempSelectedEventArray.GetSize ());
					long j;
					// 古いキー消音
					byMIDIMessage[0] = 0x90 | (m_lTempChannel & 0x0F);
	...
	// マウスキャプター中でない場合
		...
		if (m_pTempTrack == NULL && m_pTempEvent == NULL) {
			i = pMusicalScoreFrame->YtoTrackIndex (point.y, lLogPixelsY);
	...
	if (GetCapture () == this) {
		...
		case ID_MUSICALSCORE_SELECT:
			...
			if (m_lTempMode == 0x0402 || m_lTempMode == 0x0403) {
				...
				long j;
				long lTempSelectedEventCount = INT_PTRtolong (m_theTempSelectedEventArray.GetSize ());
			...
			// 選択範囲変更中の場合
				...
				forEachTrackInverse (pMIDIData, pTempTrack) {
					if (pMusicalScoreFrame->IsTrackVisible (i) && !(lFormat == 0 && i == 0)) {
						MusicalScoreTrackInfo* pTrackInfo = pMusicalScoreFrame->GetTrackInfo (i);
						long lNumNoteInfo = INT_PTRtolong (pTrackInfo->m_theNoteInfoArray.GetSize ());
						long lDownKey = pMusicalScoreFrame->TrackIndexYtoKey (i, m_ptMouseDown.y, lKeySignature, lLogPixelsY);
						long lNewKey = pMusicalScoreFrame->TrackIndexYtoKey (i, point.y, lKeySignature, lLogPixelsY);
...
void CMusicalScoreTrackTimeView::OnMouseMove (UINT nFlags, CPoint point) {
	...
	long lDownTime = m_lMouseDownTime;
	long lOldTime = m_lMouseMoveTime;
	long lNewTime = pMusicalScoreFrame->XtoTime (point.x, lLogPixelsX);
	long lsf, lmi;
	MIDIData_FindKeySignature (pMIDIData, lNewTime, &lsf, &lmi);
	long lKeySignature = lsf | (lmi << 8);
	long lDownKey = m_lMouseDownKey;
	long lOldKey = m_lMouseMoveKey;
	long lNewKey = pMusicalScoreFrame->TrackIndexYtoKey (m_lTempTrackIndex, point.y, lKeySignature, lLogPixelsY);
	BYTE byMIDIMessage[3];
	// 左ボタンキャプター中ならば
	if (GetCapture () == this) {
		MIDIOut* pMIDIOut = NULL;
...
// マウスホイールが回された時
BOOL CMusicalScoreTrackTimeView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CMusicalScoreFrame* pMusicalScoreFrame = (CMusicalScoreFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	// ユニバーサルスクロール停止
