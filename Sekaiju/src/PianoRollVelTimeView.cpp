...
// 旧位置の縦線消去
void CPianoRollVelTimeView::EraseOldLine (CDC* pDC, long lLogPixelsX, long lLogPixelsY) {
...
// 現在位置の縦線描画
void CPianoRollVelTimeView::DrawCurLine (CDC* pDC, long lLogPixelsX, long lLogPixelsY) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CRect rcClient;
	GetClientRect (&rcClient);
	rcClient += CSize (pPianoRollFrame->GetTimeScrollPos (), pPianoRollFrame->GetVelScrollPos ());
...
void CPianoRollVelTimeView::OnDraw (CDC* pDC) {
	...
	for (i = 0; i <= 8; i++) {
		...
		y = pPianoRollFrame->VeltoY (i * 16, lLogPixelsY);
	...
	forEachTrack (pMIDIData, pMIDITrack) {
		...
		if (pPianoRollFrame->IsTrackVisible (i)) {
				...
				if (lVisibleLeftTime <= lTime && lTime < lVisibleRightTime) {
					...
					if (MIDIEvent_IsNoteOn (pMIDIEvent)) {
						if (pPianoRollFrame->IsGraphVisible (1)) {
							...
							y = pPianoRollFrame->VeltoY (lValue, lLogPixelsY);
							long y0 = pPianoRollFrame->VeltoY (0, lLogPixelsY);
							//pDC->MoveTo (x, y);
							//pDC->LineTo (x, y0);
					...
					// ピッチベンドの描画
						...
						if (pPianoRollFrame->IsGraphVisible (3)) {
							...
							x = pPianoRollFrame->TimetoX (lTime, lLogPixelsX);
							lValue = MIDIEvent_GetValue (pMIDIEvent);
							y = pPianoRollFrame->PitchBendtoY (lValue, lLogPixelsY);
							long yc = pPianoRollFrame->PitchBendtoY (8192, lLogPixelsY);
							//pDC->MoveTo (x, y);
							//pDC->LineTo (x, yc);
				...
				if (MIDIEvent_IsTempo (pMIDIEvent) && i == 0) {
					if (pPianoRollFrame->IsGraphVisible (0)) {
						pOldPen = pDC->SelectObject 
							(pSekaijuDoc->IsEventSelected (pMIDIEvent) ? &penSelected : &penControl);
						x = pPianoRollFrame->TimetoX (lTime, lLogPixelsX);
						long xold = pPianoRollFrame->TimetoX (lOldTempoTime, lLogPixelsX);
						lValue = MIDIEvent_GetTempo (pMIDIEvent);
						lValue = CLIP (1, lValue, 60000000);
						y = pPianoRollFrame->TempoBPMtoY (60000000 / lValue, lLogPixelsY);
						long yold = pPianoRollFrame->TempoBPMtoY (60000000 / lOldTempo, lLogPixelsY);
			...
			if (pPianoRollFrame->IsGraphVisible (0) && i == 0) {
				...
				x = pPianoRollFrame->TimetoX (lTime, lLogPixelsX);
				long xold = pPianoRollFrame->TimetoX (lOldTempoTime, lLogPixelsY);
				long yold = pPianoRollFrame->TempoBPMtoY (60000000 / lOldTempo, lLogPixelsY);
				pDC->MoveTo (xold, yold);
				pDC->LineTo (x, yold);
				pDC->SelectObject (pOldPen);
...
void CPianoRollVelTimeView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	long lNewVel = pPianoRollFrame->YtoVel (point.y, lLogPixelsY);
	long lNewValue = pPianoRollFrame->YtoVel (point.y, lLogPixelsY);
	long lNewPitchBend = pPianoRollFrame->YtoPitchBend (point.y, lLogPixelsY);
	long lNewTempoBPM = pPianoRollFrame->YtoTempoBPM (point.y, lLogPixelsY);
...
void CPianoRollVelTimeView::OnLButtonUp (UINT nFlags, CPoint point) {
	...
	CSize sizWndOrg (pPianoRollFrame->GetTimeScrollPos (), pPianoRollFrame->GetVelScrollPos ());
	point += sizWndOrg;
	long lDownVel = pPianoRollFrame->YtoVel (m_ptMouseDown.y, lLogPixelsY);
	long lOldVel = pPianoRollFrame->YtoVel (m_ptMouseMove.y, lLogPixelsY);
	long lDownValue = pPianoRollFrame->YtoVel (m_ptMouseDown.y, lLogPixelsY);
	long lOldValue = pPianoRollFrame->YtoVel (m_ptMouseMove.y, lLogPixelsY);
	long lDownTempoBPM = pPianoRollFrame->YtoTempoBPM (m_ptMouseDown.y, lLogPixelsY);
	long lOldTempoBPM = pPianoRollFrame->YtoTempoBPM (m_ptMouseMove.y, lLogPixelsY);
	long lDownPitchBend = pPianoRollFrame->YtoPitchBend (m_ptMouseDown.y, lLogPixelsY);
	long lOldPitchBend = pPianoRollFrame->YtoPitchBend (m_ptMouseMove.y, lLogPixelsY);
	...
	if (GetCapture () == this) {
		...
		case ID_PIANOROLL_PEN:
			...
			// 20201102:テンポイベントが記入された場合は、時:分:秒:ミリ秒と演奏速度を更新する。
			if (m_lTempGraphKind == 0) {
				if (pSekaijuApp->m_bPlaying) {
			...
			if (m_lTempGraphKind == 0) {
				...
				if (m_lTempGraphSnap == 1) {
					for (lTempX = lMinX; lTempX <= lMaxX; lTempX++) {
						lTempTime = pPianoRollFrame->XtoTime (lTempX, lLogPixelsX);
				...
				// 20201102:テンポイベントが追加された場合は、時:分:秒:ミリ秒と演奏速度を更新する。
				if (pSekaijuApp->m_bPlaying) {
					MIDIClock_Stop (pSekaijuDoc->m_pMIDIClock);
				}
				long lValue = 60000000;
...
// マウスホイールが回された時
BOOL CPianoRollVelTimeView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	// ユニバーサルスクロール停止
	...
	if (lFlags & MK_CONTROL) {
		...
		lVelScrollPos -= lVelZoom * lDelta / 120;
