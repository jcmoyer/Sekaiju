...
BOOL CColorSelectComboBox::SetCurColor (COLORREF clr) {
	...
	if (GetSafeHwnd () != NULL) {
		...
		// 一致するカラーサンプルを選択状態にする
		for (i = 0; i <= nCount; i++) {
			COLORREF clrSampleColor = DWORD_PTRtoDWORD (GetItemData (i));
...
int CColorSelectComboBox::SetSampleColor (int nIndex, COLORREF clr) {
	...
	if (nCurSel == CB_ERR || nCurSel == nIndex) { 
		COLORREF clrSampleColor = DWORD_PTRtoDWORD (GetItemData (nIndex));
...
// 20201221 リフレクションをOnSelChangeからOnSelEndOK(より早期)に変更
// 20201221 OnSelChangeは親ウィンドウに通知する必要があるため。
void CColorSelectComboBox::OnSelEndOK () {
	int nCurSel = GetCurSel ();
