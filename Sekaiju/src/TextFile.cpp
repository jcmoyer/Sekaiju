...
void CTextFile::WriteTextA (LPCSTR lpszText, int nCodePage) {
	int nTextLen = size_ttolong (strlen (lpszText));
	char szLocale[16];
	memset (szLocale, 0, sizeof (szLocale));
	if (nCodePage == 0) {
		_snprintf_s (szLocale, 16, ".ACP");
	}
	else {
		_snprintf_s (szLocale, 16, ".%d", nCodePage);
	}
	// ANSIをUTF-16LEで書き込み
	if (nCodePage == 1200) {
		...
		INT_PTR nRet = mbstowcs (lpszWideText, lpszText, nTextLen);
		int  nWideTextLen = size_ttolong (wcslen (lpszWideText));
...
void CTextFile::WriteTextW (LPCWSTR lpszText, int nCodePage) {
	int nTextLen = size_ttolong (wcslen (lpszText));
	wchar_t szLocale[16];
	memset (szLocale, 0, sizeof (szLocale));
	if (nCodePage == 0) {
		_snwprintf_s (szLocale, 16, L".ACP");
	}
	else {
		_snwprintf_s (szLocale, 16, L".%d", nCodePage);
	}
	...
	// UTF16LEをANSIで書き込み
		...
		INT_PTR nRet = wcstombs (lpszAnsiText, lpszText, 4 * nTextLen);
		int nAnsiTextLen = size_ttolong (strlen (lpszAnsiText));
...
UINT CTextFile::ReadTextA (LPSTR lpszText, int nTextLen, int nCodePage) {
	...
	if (nCodePage == 1200) {
		...
		INT_PTR nRet = wcstombs (lpszText, lpszUnicodeText, nTextLen);
		free (lpszUnicodeText);
		return size_ttolong (strlen (lpszText));
	}
...
UINT CTextFile::ReadTextW (LPWSTR lpszText, int nTextLen, int nCodePage) {
	...
	// ANSIファイルをUTF16LEで読み込み
		...
		INT_PTR nRet = mbstowcs (lpszText, lpszAnsiText, (nTextLen - 1));
		INT_PTR nAnsiTextLen = strlen (lpszAnsiText);
		free (lpszAnsiText);
	}
	return size_ttolong (wcslen (lpszText));
}
...
CString CTextFile::ReadTextA (int nCodePage) {
	...
	if (nCodePage == 1200) {
		...
		setlocale (LC_CTYPE, szLocale);
		INT_PTR nRet = wcstombs (pszAnsiText, (const wchar_t*)pFileImage, nFileSize);
		strRet = pszAnsiText;
		free (pszAnsiText);
	}
	// UTF16BEファイルをANSIで読み込み
		...
		setlocale (LC_CTYPE, szLocale);
		INT_PTR nRet = wcstombs (lpszAnsiText, lpszUnicodeText, 4 * nFileSize);
		strRet = lpszAnsiText;
		free (lpszAnsiText);
		free (lpszUnicodeText);
	}
...
CString CTextFile::ReadTextW (int nCodePage) {
	...
	// ANSIファイルをUTF16LEで読み込み
		...
		_wsetlocale (LC_CTYPE, szLocale);
		INT_PTR nRet = mbstowcs ((LPWSTR)pszUnicodeText, (LPSTR)pFileImage, sizeof (wchar_t) * nFileSize);
