...
void CEventListIndexScaleView::OnDraw (CDC* pDC) {
	...
	long lTopIndex = pEventListFrame->GetVisibleTopRow (lLogPixelsY);
	long lBottomIndex = pEventListFrame->GetVisibleBottomRow (lLogPixelsY);
	long y = 0;
	long yold = 0;
	TCHAR szBuf[256];
	...
	for (long i = lTopIndex; i <= lBottomIndex; i++) {
		...
		y = i * lRowZoom * lLogPixelsY / 96;
		CRect theRect (0, y, rcClient.Width (), y + lRowZoom * lLogPixelsY / 96);
		memset (szBuf, 0, sizeof (szBuf));
		_sntprintf_s (szBuf, 256, 255, _T("%d"), i + (bEventZeroOrigin ? 0 : 1));
		if (bEventSelected) {
...
void CEventListIndexScaleView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	if ((nFlags & MK_SHIFT) == 0 && (nFlags & MK_CONTROL) == 0) {
		...
		pEventListFrame->MakeVisibleEventArray (lLogPixelsX, lLogPixelsY); // TODO 他のフレームウィンドウが不正になる
	}

	// イベントの選択
	long lOldRow = pEventListFrame->YtoRow (m_ptMouseDown.y, lLogPixelsY);
	long lCurRow = pEventListFrame->YtoRow (point.y, lLogPixelsY);
	// シフトが押されている場合
	if (nFlags & MK_SHIFT) {
...
void CEventListIndexScaleView::OnMouseMove (UINT nFlags, CPoint point) {
	...
	if (GetCapture () == this) {
		...
		// イベントの選択又は選択解除
		long lCurRow = pEventListFrame->YtoRow (point.y, lLogPixelsY);
		long lDownRow = pEventListFrame->YtoRow (m_ptMouseDown.y, lLogPixelsY);
		long lOldRow = pEventListFrame->YtoRow (m_ptMouseMove.y, lLogPixelsY);
		if (lOldRow != lCurRow) {
			long lVisibleEventCount = INT_PTRtolong (pEventListFrame->GetVisibleEventCount ());
			long lMinRow = MIN (lOldRow, lCurRow);
			long lMaxRow = MAX (lOldRow, lCurRow);
			long i;
			MIDITrack* pMIDITrack = NULL;
			MIDIEvent* pMIDIEvent = NULL;
			MIDIEvent* pCloneEvent = NULL;
...
// マウスホイールが回された時
BOOL CEventListIndexScaleView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
