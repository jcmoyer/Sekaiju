...
void CPianoRollScaleView::OnUpdate (CView* pSender, LPARAM lHint, CObject* pHint) {
	...
	if (lHint & SEKAIJUDOC_MIDITRACKCHANGED) {
		...
		pPianoRollFrame->UpdateGraphKindCombo ();
		pPianoRollFrame->UpdateGraphKindList ();
	}
	...
	if ((lHint & SEKAIJUDOC_MIDIDATACHANGED) ||
		...
		pPianoRollFrame->RecalcKeyScrollInfo (lLogPixelsY);
		pPianoRollFrame->RecalcVelScrollInfo (lLogPixelsY);
		pPianoRollFrame->RecalcTimeScrollInfo (lLogPixelsX);
	}
...
// マウスホイールが回された時
BOOL CPianoRollScaleView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	long lDelta = zDelta;
