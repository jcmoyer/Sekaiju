...
#define TRACKLISTFRAME_TIMEWIDTH 200
...
// ドキュメントへのポインタ取得
CDocument* CTrackListFrame::GetDocument () {
	ASSERT (m_pDummyView);
	return m_pDummyView->GetDocument ();
}
...
// 指定の列の左座標を取得
long CTrackListFrame::GetColumnLeft (long lColumn, long lLogPixelsX) {
	long lSum = 0;
...
// 指定の列の幅を取得
long CTrackListFrame::GetColumnWidth (long lColumn, long lLogPixelsX) {
	return m_lColumnBaseWidth[lColumn] * m_lColumnZoom * lLogPixelsX / 96;
}

// y座標をトラックに変換
long CTrackListFrame::YtoRow (long y, long lLogPixelsY) {
	return (y * 96 / lLogPixelsY) / m_lRowZoom;
}

// トラックをy座標に変換
long CTrackListFrame::RowtoY (long lTrack, long lLogPixelsY) {
	return (m_lRowZoom * lTrack) * lLogPixelsY / 96;
}

// x座標からモードを取得
long CTrackListFrame::XtoColumn (long x, long lLogPixelsX) {
...
// モードをx座標に変換
long CTrackListFrame::ColumntoX (long lMode, long lLogPixelsX) {
	return GetColumnLeft (lMode, lLogPixelsX);
}

// x座標からタイムを取得
long CTrackListFrame::XtoTime (long x, long lLogPixelsX) {
	long lTimeResolution = MIDIData_GetTimeResolution (((CSekaijuDoc*)GetDocument ())->m_pMIDIData);
	return (x * lTimeResolution / m_lTimeZoom) * 96 / lLogPixelsX;
}

// タイムをx座標に変換
long CTrackListFrame::TimetoX (long lTime, long lLogPixelsX) {
	long lTimeResolution = MIDIData_GetTimeResolution (((CSekaijuDoc*)GetDocument ())->m_pMIDIData);
	return (lTime * m_lTimeZoom / lTimeResolution) * lLogPixelsX / 96;
}
...
// 最も上に表示されている行を取得
long CTrackListFrame::GetVisibleTopRow (long lLogPixelsY) {
	return m_lRowScrollPos / m_lRowZoom * 96 / lLogPixelsY;
}

// 最も下に表示されている行を取得
long CTrackListFrame::GetVisibleBottomRow (long lLogPixelsY) {
	CRect rcClient;
	m_pTrackScaleView->GetClientRect (&rcClient);
	return (m_lRowScrollPos + rcClient.Height ()) / m_lRowZoom * 96 / lLogPixelsY + 1;
}

// 表示されているタイムの左端を計算
long CTrackListFrame::GetVisibleLeftTime (long lLogPixelsX) {
	long lTimeResolution = MIDIData_GetTimeResolution (((CSekaijuDoc*)GetDocument ())->m_pMIDIData);
	return m_lTimeScrollPos * lTimeResolution / m_lTimeZoom * 96 / lLogPixelsX;
}

// 表示されているタイムの右端を計算
long CTrackListFrame::GetVisibleRightTime (long lLogPixelsX) {
	CRect rcClient;
	m_pTimeScaleView->GetClientRect (&rcClient);
	long lTimeResolution = MIDIData_GetTimeResolution (((CSekaijuDoc*)GetDocument ())->m_pMIDIData);
	return (m_lTimeScrollPos + rcClient.Width ()) * lTimeResolution / m_lTimeZoom * 96 / lLogPixelsX;
}
...
// スプリッターキャプターの描画
void CTrackListFrame::DrawSplitterCaptor (CDC* pDC, CPoint pt, long lLogPixelsX, long lLogPixelsY) {
	CRect rcClient;
	GetClientRect (&rcClient);
	CPen pen;
	CPen* pOldPen;
	pen.CreatePen (PS_SOLID, TRACKLISTFRAME_SPLITTERWIDTH * lLogPixelsX / 96, ::GetSysColor(COLOR_BTNSHADOW));
...
// 行スクロールバー(縦)のデザイン設定
void CTrackListFrame::RecalcRowScrollInfo (long lLogPixelsY) {
	...
	si.nMin = 0;
	si.nMax = CLIP (17, lTrackCount + 16, MAXMIDITRACKNUM - 1) * m_lRowZoom * lLogPixelsY / 96;
...
// 列スクロールバー(縦)のデザイン設定
void CTrackListFrame::RecalcColumnScrollInfo (long lLogPixelsX) {
	SCROLLINFO si;
	si.fMask = SIF_RANGE | SIF_PAGE;
	si.nMin = 0;
	si.nMax = 0;
	for (int i = 0; i < TRACKLISTFRAME_NUMCOLUMN; i++) {
		si.nMax += GetColumnWidth (i, lLogPixelsX);
	}
...
void CTrackListFrame::RecalcTimeScrollInfo (long lLogPixelsX) {
	...
	si.fMask = SIF_RANGE | SIF_PAGE;
	si.nMin = 0;
	si.nMax = (lEndTime + lFeedTime) * m_lTimeZoom / lTimeResolution * lLogPixelsX / 96;
...
void CTrackListFrame::RecalcLayout (BOOL bNotify) {
	...
	if (rcClient.Height () < 
		m_lToolBar1Height +
		TRACKLISTFRAME_SCALEHEIGHT * lLogPixelsY / 96 + 
		m_lHScrollBarHeight +
		...
		m_lScaleHeight = TRACKLISTFRAME_SCALEHEIGHT * lLogPixelsY / 96;
		m_lTrackHeight = rcClient.Height () - 
			m_lToolBar1Height - 
			m_lScaleHeight - 
			m_lHScrollBarHeight - 
			TRACKLISTFRAME_BORDERHEIGHT * 2;
	...
	// 幅方向の位置計算
	if (rcClient.Width () <
		TRACKLISTFRAME_SCALEWIDTH * lLogPixelsX / 96 + 
		m_lVScrollBarWidth + 
		TRACKLISTFRAME_BORDERWIDTH * 4 + 
		TRACKLISTFRAME_SPLITTERWIDTH * lLogPixelsX / 96) {
		m_lScaleWidth = 
			rcClient.Width () - 
			TRACKLISTFRAME_SPLITTERWIDTH * lLogPixelsX / 96 -
			m_lVScrollBarWidth - 
		...
		TRACKLISTFRAME_SPLITTERWIDTH * lLogPixelsX / 96 +
		m_lVScrollBarWidth) {
		m_lScaleWidth = TRACKLISTFRAME_SCALEWIDTH * lLogPixelsX / 96;
		m_lModeWidth = rcClient.Width () - 
			TRACKLISTFRAME_SPLITTERWIDTH * lLogPixelsX / 96 - 
			TRACKLISTFRAME_SCALEWIDTH * lLogPixelsX / 96 - 
			m_lVScrollBarWidth - 
			TRACKLISTFRAME_BORDERWIDTH * 4;
	...
	if (m_pTimeScaleView) {
		m_pTimeScaleView->MoveWindow (TRACKLISTFRAME_BORDERWIDTH * 3 + m_lScaleWidth + 
			m_lModeWidth + TRACKLISTFRAME_SPLITTERWIDTH * lLogPixelsX / 96, 
			m_lToolBar1Height + TRACKLISTFRAME_BORDERHEIGHT,
	...
	m_wndRowScroll.MoveWindow (TRACKLISTFRAME_BORDERWIDTH * 3 + m_lScaleWidth +
		m_lModeWidth + m_lTimeWidth + TRACKLISTFRAME_SPLITTERWIDTH * lLogPixelsX / 96,
		m_lToolBar1Height + TRACKLISTFRAME_BORDERHEIGHT,
		m_lVScrollBarWidth,
	...
	m_wndTimeZoomDown.MoveWindow (TRACKLISTFRAME_BORDERWIDTH * 3 + m_lScaleWidth +
		m_lModeWidth + TRACKLISTFRAME_SPLITTERWIDTH * lLogPixelsX / 96 + m_lTimeWidth - 
	...
	m_wndRowZoomDown.MoveWindow
		...
		m_lTimeWidth + TRACKLISTFRAME_SPLITTERWIDTH * lLogPixelsX / 96, 
		m_lToolBar1Height + TRACKLISTFRAME_BORDERHEIGHT + m_lScaleHeight + m_lTrackHeight -
...
int CTrackListFrame::OnCreate (LPCREATESTRUCT lpCreateStruct) {
	...
	long lSumColumnWidth = 0;
	for (int j = 0; j < TRACKLISTFRAME_NUMCOLUMN; j++) {
		lSumColumnWidth += GetColumnWidth (j, lLogPixelsX);
	}
	m_lModeWidth = lSumColumnWidth;
	//m_lTimeWidth = TRACKLISTFRAME_TIMEWIDTH * lLogPixelsX/ 96;
...
void CTrackListFrame::OnPaint () {
	...
	rcClient2.left = TRACKLISTFRAME_BORDERWIDTH * 2 + m_lScaleWidth + m_lModeWidth + TRACKLISTFRAME_SPLITTERWIDTH * lLogPixelsX / 96;
...
void CTrackListFrame::OnRowZoomDown () {
	...
	RecalcRowScrollInfo (lLogPixelsX);
...
void CTrackListFrame::OnHScroll (UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) {
	...
	if (pScrollBar == &m_wndColumnScroll) {
		...
		switch (nSBCode) {
		case SB_LINELEFT:
			lNewPos = m_lColumnScrollPos - m_lColumnZoom * lLogPixelsX / 96;
			break;
		case SB_LINERIGHT:
			lNewPos = m_lColumnScrollPos + m_lColumnZoom * lLogPixelsX / 96;
			break;
		case SB_PAGELEFT:
			lNewPos = m_lColumnScrollPos - m_lColumnZoom * 4 * lLogPixelsX / 96;
			break;
		case SB_PAGERIGHT:
			lNewPos = m_lColumnScrollPos + m_lColumnZoom * 4 * lLogPixelsX / 96;
			break;
		...
		switch (nSBCode) {
		case SB_LINELEFT:
			lNewPos = m_lTimeScrollPos - m_lTimeZoom * 2 * lLogPixelsX / 96;
			break;
		case SB_LINERIGHT:
			lNewPos = m_lTimeScrollPos + m_lTimeZoom * 2 * lLogPixelsX / 96;
			break;
		case SB_PAGELEFT:
			lNewPos = m_lTimeScrollPos - m_lTimeZoom * 16 * lLogPixelsX / 96;
			break;
		case SB_PAGERIGHT:
			lNewPos = m_lTimeScrollPos + m_lTimeZoom * 16 * lLogPixelsX / 96;
			break;
...
void CTrackListFrame::OnVScroll (UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) {
	...
	if (pScrollBar == &m_wndRowScroll) {
		...
		case SB_LINEUP:
			...
			break;
		case SB_PAGEDOWN:
			lNewPos = m_lRowScrollPos + m_lRowZoom * 12 * lLogPixelsY / 96;
			break;
		case SB_PAGEUP:
			lNewPos = m_lRowScrollPos - m_lRowZoom * 12 * lLogPixelsY / 96;
			break;
...
void CTrackListFrame::OnTrackListMoveUpTrack () {
	...
	// 履歴記録
	CHistoryUnit* pCurHistoryUnit = NULL;
	MIDITrack* pPrevTrack = pMIDITrack->m_pPrevTrack;
	...
	VERIFY (pCurHistoryUnit = pSekaijuDoc->GetCurHistoryUnit ());
	VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVETRACK, pMIDITrack));
	VERIFY (pCloneTrack = pSekaijuDoc->ReplaceMIDITrack (pMIDITrack)); // 20230630 BugFix(ヒストリに同じトラックへのポインタを2回以上登場させない)
	// 該当トラックをいったんMIDIデータから除去する
	long lRet = MIDIData_RemoveTrack (pMIDIData, pCloneTrack); // 20230630 BugFix(ヒストリに同じトラックへのポインタを2回以上登場させない)
	if (lRet == 0) {
		pSekaijuDoc->m_theCriticalSection.Unlock ();
		CString strMsg;
	...
	lRet = MIDIData_InsertTrackBefore (pMIDIData, pCloneTrack, pPrevTrack); // 20230630 BugFix(ヒストリに同じトラックへのポインタを2回以上登場させない)
	if (lRet == 0) {
		pSekaijuDoc->m_theCriticalSection.Unlock ();
		CString strMsg;
		VERIFY (strMsg.LoadString (IDS_TRACK_MOVE_FAILED));
	...
	VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTTRACK, pCloneTrack)); // 20230630 BugFix(ヒストリに同じトラックへのポインタを2回以上登場させない)
	((CTrackListTrackModeView*)m_pTrackModeView)->m_lCurRow--;
	pSekaijuDoc->UpdateAllViews (NULL, SEKAIJUDOC_MIDITRACKCHANGED);
	pSekaijuDoc->m_theCriticalSection.Unlock ();
	::Sleep (0);
...
void CTrackListFrame::OnTrackListMoveDownTrack () {
	...
	// 履歴記録
	CHistoryUnit* pCurHistoryUnit = NULL;
	MIDITrack* pNextTrack = pMIDITrack->m_pNextTrack;
	MIDITrack* pCloneTrack = NULL;
	CString strHistoryName;
	VERIFY (strHistoryName.LoadString (IDS_MOVE_TRACK));
	VERIFY (pSekaijuDoc->AddHistoryUnit (strHistoryName));
	...
	// 除去したトラックをMIDIデータに挿入する
	lRet = MIDIData_InsertTrackAfter (pMIDIData, pCloneTrack, pNextTrack); // 20230630 BugFix(ヒストリに同じトラックへのポインタを2回以上登場させない)
	if (lRet == 0) {
		CString strMsg;
		VERIFY (strMsg.LoadString (IDS_TRACK_MOVE_FAILED));
		pSekaijuDoc->m_theCriticalSection.Unlock ();
