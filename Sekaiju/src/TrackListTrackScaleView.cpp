...
void CTrackListTrackScaleView::OnDraw (CDC* pDC) {
	...
	for (i = lVisibleTopRow; i <= lVisibleBottomRow; i++) {
		...
		_sntprintf_s (szBuf, 255, _T("%d"), i + (bTrackZeroOrigin ? 0 : 1));
...
void CTrackListTrackScaleView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	// 該当トラック内の全イベントの選択
	m_lOldRow = pTrackListFrame->YtoRow (m_ptMouseDown.y, lLogPixelsY);
	m_lDownRow = pTrackListFrame->YtoRow (point.y, lLogPixelsY);
	m_lCurRow = pTrackListFrame->YtoRow (point.y, lLogPixelsY);
	if (nFlags & MK_SHIFT) {
...
void CTrackListTrackScaleView::OnMouseMove (UINT nFlags, CPoint point) {
	...
	if (GetCapture () == this) {
		...
		m_lCurRow = pTrackListFrame->YtoRow (point.y, lLogPixelsY);
		m_lDownRow = pTrackListFrame->YtoRow (m_ptMouseDown.y, lLogPixelsY);
		m_lOldRow = pTrackListFrame->YtoRow (m_ptMouseMove.y, lLogPixelsY);
...
// マウスホイールが回された時
BOOL CTrackListTrackScaleView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CTrackListFrame* pTrackListFrame = (CTrackListFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
