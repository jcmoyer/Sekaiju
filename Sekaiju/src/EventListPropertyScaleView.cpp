...
void CEventListPropertyScaleView::OnDraw (CDC* pDC) {
	...
	for (long j = 0; j < 8; j++) {
		...
		theRect.right = theRect.right + pEventListFrame->GetColumnWidth (j, lLogPixelsX);
...
void CEventListPropertyScaleView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	long lBorderX = 0;
	for (j = 0; j < 8; j++) {
		lBorderX += pEventListFrame->GetColumnWidth (j, lLogPixelsX);
...
void CEventListPropertyScaleView::OnLButtonUp (UINT nFlags, CPoint point) {
	...
	if (GetCapture () == this) {
		...
		pEventListFrame->RecalcColumnScrollInfo (lLogPixelsX);
...
void CEventListPropertyScaleView::OnMouseMove (UINT nFlags, CPoint point) {
	...
	// 非キャプター中
		...
		for (j = 0; j < 8; j++) {
			lBorderX += pEventListFrame->GetColumnWidth (j, lLogPixelsX);
			if (lBorderX - 2 * lLogPixelsX / 96 <= point.x && 
				point.x <= lBorderX + 2 * lLogPixelsX / 96) {
				break;
			}
		}
		// カーソルが境界上にあった場合
...
// マウスホイールが回された時
BOOL CEventListPropertyScaleView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	...
	if (lFlags & MK_CONTROL) {
		...
		lRowScrollPos -= lRowZoom * lLogPixelsX / 96 * lDelta / 120;
