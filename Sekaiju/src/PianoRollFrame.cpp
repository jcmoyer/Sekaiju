...
#define PIANOROLLFRAME_VELHEIGHT (128 + 16)
...
#define PIANOROLLFRAME_TRACKLISTWIDTH 160
...
CPianoRollFrame::CPianoRollFrame () {
	...
	m_lVelHeight = PIANOROLLFRAME_VELHEIGHT;
	m_lScaleWidth = PIANOROLLFRAME_SCALEWIDTH * lLogPixelsX / 96;;
	m_lTimeWidth = PIANOROLLFRAME_TIMEWIDTH * lLogPixelsX / 96;
	...
	m_lTrackListWidth = PIANOROLLFRAME_TRACKLISTWIDTH * lLogPixelsX / 96;;
...
// y座標をノートキーに変換
long CPianoRollFrame::YtoKey (long y, long lLogPixelsY) {
	return (127 - ((y * 96 / lLogPixelsY) / GetKeyZoom ()));
}

// ノートキーをy座標に変換
long CPianoRollFrame::KeytoY (long lKey, long lLogPixelsY) {
	return ((128 - lKey) * GetKeyZoom ()) * lLogPixelsY / 96;
}

// y座標をベロシティに変換
long CPianoRollFrame::YtoVel (long y, long lLogPixelsY) {
	return (((128 + 8) * m_lVelZoom - (y * 96 / lLogPixelsY)) / m_lVelZoom);
}

// y座標をピッチベンドに変換
long CPianoRollFrame::YtoPitchBend (long y, long lLogPixelsY) {
	return (((128 + 8) * m_lVelZoom - (y * 96 / lLogPixelsY)) * 128 / m_lVelZoom);
}

// y座標をテンポ[BPM]に変換
long CPianoRollFrame::YtoTempoBPM (long y, long lLogPixelsY) {
	return (((128 + 8) * m_lVelZoom - (y * 96 / lLogPixelsY)) * 2 / m_lVelZoom); 
}

// ベロシティをy座標に変換
long CPianoRollFrame::VeltoY (long lVel, long lLogPixelsY) {
	return ((128 - lVel) * m_lVelZoom + 8 * m_lVelZoom) * lLogPixelsY / 96;
}

// ピッチベンドをy座標に変換
long CPianoRollFrame::PitchBendtoY (long lPitchBend, long lLogPixelsY) {
	return ((16384 - lPitchBend) * m_lVelZoom / 128 + 8) * lLogPixelsY / 96;
}

// テンポ[BPM]をy座標に変換
long CPianoRollFrame::TempoBPMtoY (long lTempoBPM, long lLogPixelsY) {
	return ((256 - lTempoBPM) * m_lVelZoom / 2 + 8) * lLogPixelsY / 96;
}

// x座標からタイムを取得
long CPianoRollFrame::XtoTime (long x, long lLogPixelsX) {
	long lTimeResolution = MIDIData_GetTimeResolution (GetDocument ()->m_pMIDIData);
	return (x * lTimeResolution / 4 / m_lTimeZoom) * 96 / lLogPixelsX;
	// 注：ズーム倍率1倍のとき4分音符の長さを4ピクセルと定義している。
}

// タイムをx座標に変換
long CPianoRollFrame::TimetoX (long lTime, long lLogPixelsX) {
	long lTimeResolution = MIDIData_GetTimeResolution (GetDocument ()->m_pMIDIData);
	return (lTime * 4 * m_lTimeZoom / lTimeResolution) * lLogPixelsX / 96;
	// 注：ズーム倍率1倍のとき4分音符の長さを4ピクセルと定義している。
}
...
// 表示されているタイムの左端を求める
long CPianoRollFrame::GetVisibleLeftTime (long lLogPixelsX) {
	long lTimeResolution = MIDIData_GetTimeResolution (GetDocument ()->m_pMIDIData);
	return (m_lTimeScrollPos * lTimeResolution / 4 /  m_lTimeZoom) * 96 / lLogPixelsX;
	// 注：ズーム倍率1倍のとき4分音符の長さを4ピクセルと定義している。
}

// 表示されているタイムの右端を求める
long CPianoRollFrame::GetVisibleRightTime (long lLogPixelsX) {
	CRect rcClient;
	m_pTimeScaleView->GetClientRect (&rcClient);
	long lTimeResolution = MIDIData_GetTimeResolution (GetDocument ()->m_pMIDIData);
	return ((m_lTimeScrollPos + rcClient.Width ()) * lTimeResolution / 4 / m_lTimeZoom) * 96 / lLogPixelsX;
	// 注：ズーム倍率1倍のとき4分音符の長さを4ピクセルと定義している。
}

// 表示されているキーの上限を計算
long CPianoRollFrame::GetVisibleTopKey (long lLogPixelsY) {
	return (127 - ((m_lKeyScrollPos * 96 / lLogPixelsY) / m_lKeyZoom));
}

// 表示されているキーの下限を計算
long CPianoRollFrame::GetVisibleBottomKey (long lLogPixelsY) {
	CRect rcClient;
	m_pKeyScaleView->GetClientRect (&rcClient);
	return (127 - ((m_lKeyScrollPos + rcClient.Height ()) * 96 / lLogPixelsY) / m_lKeyZoom);
}

// 表示されているベロシティの上限を計算
long CPianoRollFrame::GetVisibleTopVel (long lLogPixelsY) {
	return (127 - ((m_lVelScrollPos  * 96 / lLogPixelsY) / m_lVelZoom));
}

// 表示されているベロシティの下限を計算
long CPianoRollFrame::GetVisibleBottomVel (long lLogPixelsY) {
	CRect rcClient;
	m_pVelScaleView->GetClientRect (&rcClient);	
	return (127 - ((m_lVelScrollPos + rcClient.Height ())  * 96 / lLogPixelsY) / m_lVelZoom);
}
...
void CPianoRollFrame::DrawSplitterCaptor (CDC* pDC, CPoint pt) {
	...
	CPen pen;
	CPen* pOldPen;
	pen.CreatePen (PS_SOLID, PIANOROLLFRAME_SPLITTERWIDTH * lLogPixelsX / 96, ::GetSysColor(COLOR_BTNSHADOW));
...
BOOL CPianoRollFrame::UpdateGraphKindList () {
	...
	pSekaijuDoc->m_theCriticalSection.Lock ();
	CString strText;
	BOOL bOldGraphVisible[256];
	long i;
	long lOldCount = m_pGraphKindListBox->GetCount ();
	for (i = 0; i < 256; i++) {
		bOldGraphVisible[i] = m_bGraphVisible[i];
	}
	long lOldCurSel = m_pGraphKindListBox->GetCurSel ();
	...
	// 選択状態の更新
	long lNewCount = m_pGraphKindListBox->GetCount ();
	if (0 <= lOldCurSel && lOldCurSel <lNewCount) {
		m_pGraphKindListBox->SetCurSel (lOldCurSel);
	}
	// チェックボックスの完全更新
	for (i = 0; i < 256; i++) {
		m_bGraphVisible[i] = bOldGraphVisible[i];
	}
	if (m_bShowAllGraph) {
		for (i = 0; i < lNewCount; i++) {
	...
	else if (m_bOnlyCurGraph) {
		for (i = 0; i < lNewCount; i++) {
			m_pGraphKindListBox->SetCheck (i, 0);
		}
		if (0 <= lOldCurSel && lOldCurSel < lNewCount) {
			m_pGraphKindListBox->SetCheck (lOldCurSel, 1);
		}
	}
	else {
		for (i = 0; i < lNewCount; i++) {
			m_pGraphKindListBox->SetCheck (i, m_bGraphVisible[i]);
		}
	}
	pSekaijuDoc->m_theCriticalSection.Unlock ();
	return TRUE;
...
// キースクロールバー(縦)のデザイン設定
void CPianoRollFrame::RecalcKeyScrollInfo (long lLogPixelsX) {
	SCROLLINFO si;
	si.fMask = SIF_RANGE | SIF_PAGE;
	si.nMin = 0;
	si.nMax = 128 * m_lKeyZoom * lLogPixelsX / 96;
...
void CPianoRollFrame::RecalcVelScrollInfo (long lLogPixelsX) {
	SCROLLINFO si;
	si.fMask = SIF_RANGE | SIF_PAGE;
	si.nMin = 0;
	si.nMax = PIANOROLLFRAME_VELHEIGHT * m_lVelZoom * lLogPixelsX / 96;
...
void CPianoRollFrame::RecalcTimeScrollInfo (long lLogPixelsX) {
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	MIDIData* pMIDIData = pSekaijuDoc->m_pMIDIData;
	...
	si.fMask = SIF_RANGE | SIF_PAGE;
	si.nMin = 0;
	si.nMax = ((lEndTime + lFeedTime) * m_lTimeZoom * 4 / lTimeResolution) * lLogPixelsX / 96;
...
void CPianoRollFrame::RecalcLayout (BOOL bNotify) {
	...
	if (rcClient.Height () - m_lToolBar1Height < 
		PIANOROLLFRAME_SCALEHEIGHT * lLogPixelsY / 96 + 
		m_lHScrollBarHeight +
		PIANOROLLFRAME_BORDERHEIGHT * 4 + 
		PIANOROLLFRAME_SPLITTERHEIGHT * lLogPixelsY / 96) {
		m_lScaleHeight = rcClient.Height () - 
			m_lHScrollBarHeight -
			PIANOROLLFRAME_BORDERHEIGHT * 4 - 
			PIANOROLLFRAME_SPLITTERHEIGHT * lLogPixelsY / 96;
		m_lKeyHeight = 0;
		...
		PIANOROLLFRAME_BORDERHEIGHT * 4+ 
		PIANOROLLFRAME_SPLITTERHEIGHT * lLogPixelsY / 96) {
		m_lScaleHeight = PIANOROLLFRAME_SCALEHEIGHT * lLogPixelsX / 96;
		m_lKeyHeight = 0;
		...
		m_lScaleHeight = PIANOROLLFRAME_SCALEHEIGHT * lLogPixelsY / 96;
	...
	// 幅方向の位置計算
	if (rcClient.Width () <
		PIANOROLLFRAME_SCALEWIDTH * lLogPixelsX / 96
		+ m_lVScrollBarWidth + 
		...
		+ m_lVScrollBarWidth + 
		PIANOROLLFRAME_BORDERWIDTH * 4 + 
		PIANOROLLFRAME_SPLITTERWIDTH  * lLogPixelsX / 96 + 
		m_lTrackListWidth) {
		//m_lScaleWidth = rcClient.Width () - 
		//	m_lVScrollBarWidth - PIANOROLLFRAME_BORDERWIDTH * 2;
		m_lScaleWidth = PIANOROLLFRAME_SCALEWIDTH * lLogPixelsX / 96;
		m_lTimeWidth = 0;
		m_lTrackListWidth = rcClient.Width () - 
			PIANOROLLFRAME_SCALEWIDTH * lLogPixelsX / 96 -
			m_lVScrollBarWidth - 
			PIANOROLLFRAME_BORDERWIDTH * 4 -
			PIANOROLLFRAME_SPLITTERWIDTH * lLogPixelsX / 96;
	...
	if (m_pVelScaleView) {
		m_pVelScaleView->MoveWindow (PIANOROLLFRAME_BORDERWIDTH, 
			...
			PIANOROLLFRAME_SPLITTERHEIGHT * lLogPixelsY / 96, m_lScaleWidth, m_lVelHeight);
	}

	if (m_pVelTimeView) {
		m_pVelTimeView->MoveWindow (PIANOROLLFRAME_BORDERWIDTH + m_lScaleWidth,
			m_lToolBar1Height + PIANOROLLFRAME_BORDERHEIGHT * 3 + m_lScaleHeight + m_lKeyHeight + 
			PIANOROLLFRAME_SPLITTERHEIGHT * lLogPixelsY / 96, m_lTimeWidth, m_lVelHeight);
	}
	// スクロールバーの整列
	m_wndTimeScroll.MoveWindow (PIANOROLLFRAME_BORDERWIDTH,
		m_lToolBar1Height + PIANOROLLFRAME_BORDERHEIGHT * 3 + m_lScaleHeight + m_lKeyHeight + 
		PIANOROLLFRAME_SPLITTERHEIGHT * lLogPixelsY / 96 + m_lVelHeight,
	...
	m_wndVelScroll.MoveWindow (PIANOROLLFRAME_BORDERWIDTH + m_lScaleWidth + m_lTimeWidth, 
		m_lToolBar1Height + PIANOROLLFRAME_BORDERHEIGHT * 3 + m_lScaleHeight + m_lKeyHeight +
		PIANOROLLFRAME_SPLITTERHEIGHT * lLogPixelsY / 96, m_lVScrollBarWidth,
	...
	m_wndVelZoomDown.MoveWindow
		(PIANOROLLFRAME_BORDERWIDTH + m_lScaleWidth + m_lTimeWidth, 
		m_lToolBar1Height + PIANOROLLFRAME_BORDERHEIGHT * 3 + m_lScaleHeight + m_lKeyHeight + 
		PIANOROLLFRAME_SPLITTERHEIGHT * lLogPixelsY / 96 + m_lVelHeight - m_lVScrollBarWidth * 2, 
		m_lVScrollBarWidth, m_lHScrollBarHeight);

	m_wndVelZoomUp.MoveWindow 
		(PIANOROLLFRAME_BORDERWIDTH + m_lScaleWidth + m_lTimeWidth, 
		m_lToolBar1Height + PIANOROLLFRAME_BORDERHEIGHT * 3 + m_lScaleHeight + m_lKeyHeight + 
		PIANOROLLFRAME_SPLITTERHEIGHT * lLogPixelsY / 96 + m_lVelHeight - m_lVScrollBarWidth,
		m_lVScrollBarWidth, 
		m_lVScrollBarWidth, m_lHScrollBarHeight);
	...
	m_pTrackListBox->MoveWindow
		...
		m_lVScrollBarWidth + PIANOROLLFRAME_SPLITTERWIDTH * lLogPixelsX / 96, 
		m_lToolBar1Height + PIANOROLLFRAME_BORDERHEIGHT,
		m_lTrackListWidth,
	...
	m_pGraphKindListBox->MoveWindow
		...
		m_lScaleHeight + m_lKeyHeight + PIANOROLLFRAME_SPLITTERHEIGHT * lLogPixelsY / 96,
		m_lTrackListWidth,
		m_lVelHeight + m_lHScrollBarHeight);

	// スクロールバーのサイズが変化したので、バーのデザインを再調整する。
	RecalcKeyScrollInfo (lLogPixelsY);
	RecalcVelScrollInfo (lLogPixelsY);
...
int CPianoRollFrame::OnCreate (LPCREATESTRUCT lpCreateStruct) {
	...
	m_lKeyHeight = PIANOROLLFRAME_KEYHEIGHT * lLogPixelsY / 96;
	m_lVelHeight = PIANOROLLFRAME_VELHEIGHT * lLogPixelsY / 96;
	m_lTrackListWidth = PIANOROLLFRAME_TRACKLISTWIDTH * lLogPixelsX / 96;
...
//	DockControlBar (&m_wndToolBar1);
	...
	// トラックコンボの作成
	m_wndToolBar1.SetButtonInfo (6, IDC_TRACKCOMBO, TBBS_SEPARATOR, 160 * lLogPixelsX / 96);
	...
	// チャンネルコンボの作成
	m_wndToolBar1.SetButtonInfo (8, IDC_CHANNELCOMBO, TBBS_SEPARATOR, 40 * lLogPixelsX / 96);
	...
	// スナップコンボの作成
	m_wndToolBar1.SetButtonInfo (10, IDC_SNAPCOMBO, TBBS_SEPARATOR, 100 * lLogPixelsX / 96);
	...
	// ベロシティコンボの作成
	m_wndToolBar1.SetButtonInfo (12, IDC_VELOCITYCOMBO, TBBS_SEPARATOR, 50 * lLogPixelsX / 96);
	...
	// 長さコンボの作成
	m_wndToolBar1.SetButtonInfo (14, IDC_DURATIONCOMBO, TBBS_SEPARATOR, 50 * lLogPixelsX / 96);
	...
	// グラフの種類コンボの作成
	m_wndToolBar1.SetButtonInfo (16, IDC_GRAPHKINDCOMBO, TBBS_SEPARATOR, 120 * lLogPixelsX / 96);
	...
	m_wndGraphKindCombo.SetFont (&(pSekaijuApp->m_theDefaultFont));
	m_wndGraphKindCombo.SendMessage (CB_SETITEMHEIGHT, -1, 23 * lLogPixelsY / 96 - 8);
	
	// グラフのスナップコンボの作成
	m_wndToolBar1.SetButtonInfo (18, IDC_GRAPHSNAPCOMBO, TBBS_SEPARATOR, 100 * lLogPixelsX / 96);
	...
	m_wndGraphSnapCombo.SetFont (&(pSekaijuApp->m_theDefaultFont));
	m_wndGraphSnapCombo.SendMessage (CB_SETITEMHEIGHT, -1, 23 * lLogPixelsY / 96 - 8);
	...
	m_pGraphKindListBox->SetFont (&(pSekaijuApp->m_theDefaultFont));
	UpdateGraphKindList ();
	...
	if (GetCapture () == this) {
		...
		if (m_bSplitterMovingH) {
			m_lVelHeight = CLIP (0, (m_lVelHeight - ptDelta.y), 
				PIANOROLLFRAME_VELHEIGHT * m_lVelZoom * lLogPixelsY / 96);
		}
		if (m_bSplitterMovingV) {
...
void CPianoRollFrame::OnKeyZoomUp () {
	...
	pSekaijuDoc->m_theCriticalSection.Lock ();
	m_lKeyZoom = lNewKeyZoom;
	RecalcKeyScrollInfo (lLogPixelsX);
	m_wndKeyScroll.SetScrollPos (lNewKeyPos);
	m_lKeyScrollPos = m_wndKeyScroll.GetScrollPos ();
	m_pKeyScaleView->Invalidate ();
	m_pKeyTimeView->Invalidate ();
...
void CPianoRollFrame::OnVelZoomDown () {
	...
	//m_lVelHeight = PIANOROLLFRAME_VELHEIGHT * m_lVelZoom;
	RecalcLayout (FALSE);
...
void CPianoRollFrame::OnPianoRollChromaticTransposeUp() {
	CSekaijuDoc* pSekaijuDoc = GetDocument();
