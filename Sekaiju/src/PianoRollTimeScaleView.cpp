...
(CDC* pDC, long lTime, LPCTSTR lpszText, long lColor, long lLogPixelsX, long lLogPixelsY) {
	...
	CRect rcText (x + 5 * lLogPixelsX / 96, 0, x + 1024 * lLogPixelsX / 96, 16 * lLogPixelsY / 96);
	CPen pen;
	pen.CreatePen (PS_SOLID, 1, lColor);
	CPen* pOldPen = pDC->SelectObject (&pen);
	pDC->FillSolidRect (&rcBack, ::GetSysColor (COLOR_BTNFACE));
	pDC->FillSolidRect (&rcFlag, lColor);
	pDC->MoveTo (x, 4 * lLogPixelsY / 96);
	pDC->LineTo (x, 16 * lLogPixelsY / 96);
...
void CPianoRollTimeScaleView::OnDraw (CDC* pDC) {
	...
	if (lTimeMode == MIDIDATA_TPQNBASE) {
		...
		xold = pPianoRollFrame->TimetoX (lVisibleLeftTime, lLogPixelsX) - 1;
		CRect theRect;
		pDC->SetBkMode (TRANSPARENT);
		for (long i = lLeftMeasure; i <= lRightMeasure + 1; i++) {
			...
			pDC->Draw3dRect (xold, 16 * lLogPixelsY / 96, x - xold, 16 * lLogPixelsY / 96,
				lColorBtnHighlight, lColorBtnShadow);
			memset (szBuf, 0, sizeof (szBuf));
	...
	if (GetCapture () == this) {
		long lDownX = pPianoRollFrame->TimetoX (m_lDownTime, lLogPixelsX);
		long lCurX = pPianoRollFrame->TimetoX (m_lCurTime, lLogPixelsX);
...
void CPianoRollTimeScaleView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	if (point.y < rcClient.Height () / 2) {
		pSekaijuDoc->m_theCriticalSection.Lock ();
		long lTime = pPianoRollFrame->XtoTime (point.x, lLogPixelsX);
	...
	// 下半分(小節番号部)をクリックしたとき(小節選択)
		...
		m_lOldTime = pPianoRollFrame->XtoTime (m_ptMouseDown.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
		m_lDownTime = pPianoRollFrame->XtoTime (point.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
		m_lCurTime = pPianoRollFrame->XtoTime (point.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
...
void CPianoRollTimeScaleView::OnMouseMove (UINT nFlags, CPoint point) {
	...
	if (GetCapture () == this) {
		...
		m_lCurTime = pPianoRollFrame->XtoTime (point.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
		m_lDownTime = pPianoRollFrame->XtoTime (m_ptMouseDown.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
		m_lOldTime = pPianoRollFrame->XtoTime (m_ptMouseMove.x, lLogPixelsX) / m_lTempSnap * m_lTempSnap;
...
// マウスホイールが回された時
BOOL CPianoRollTimeScaleView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	long lDelta = zDelta;
	...
	if (lFlags & MK_CONTROL) {
		...
		lKeyScrollPos -= lKeyZoom * lDelta / 120;
