...
void CTrackListTrackTimeView::DrawCurLine (CDC* pDC) {
	...
	long x = pTrackListFrame->TimetoX (m_lCurTime, lLogPixelsX);
...
void CTrackListTrackTimeView::OnDraw (CDC* pDC) {
	...
	long lVisibleLeftTime = pTrackListFrame->GetVisibleLeftTime (lLogPixelsX);
	long lVisibleRightTime = pTrackListFrame->GetVisibleRightTime (lLogPixelsX);
	...
	for (i = lVisibleTopRow; i <= lVisibleBottomRow; i++) {
		...
		for (j = lLeftMeasure; j <= lRightMeasure; j++) {
			...
			long x1 = pTrackListFrame->TimetoX (lTime1, lLogPixelsX);
			long x2 = pTrackListFrame->TimetoX (lTime2, lLogPixelsX);
			long lOffset = 
			...
			if (pMIDITrack) {
				...
				case 1: // 小節内のイベントはひとつも選択されていない(20100517調整)
					pDC->FillSolidRect (x1, y, x2 - x1, lRowZoom * lLogPixelsY / 96, lNormalColor);
					break;
				case 2: // 小節内の一部のイベントが選択されている(20100517追加)
					...
					pDC->FillRect (CRect (x1, y, x2, y + lRowZoom * lLogPixelsY / 96), &brush); 
					break;
				case 3: // 小節内の全てのイベントが選択されている(20100517調整)
					pDC->FillSolidRect (x1, y, x2 - x1, lRowZoom * lLogPixelsY / 96, lSelectedColor); 
					break;
				}
			}
			// トラックは存在しない(20100517調整)
				...
				pDC->FillSolidRect 
					(x1, y, x2 - x1, lRowZoom * lLogPixelsY / 96, lNormalColor);
			}
		}
	...
	for (i = lLeftMeasure; i <= lRightMeasure; i++) {
		...
		long x = pTrackListFrame->TimetoX (lTime, lLogPixelsX);
		...
		if (lVisibleTopRow <= i && i <= lVisibleBottomRow) {
			...
			forEachEvent (pMIDITrack, pMIDIEvent) {
				...
				if (MIDIEvent_IsNote (pMIDIEvent)) {
					...
					long x1 = pTrackListFrame->TimetoX (lTime, lLogPixelsX);
					long x2 = pTrackListFrame->TimetoX (lTime + lDur, lLogPixelsX) + 1;
					long y1 = y + lRowZoom * (127 - lKey) / 128 * lLogPixelsY / 96;
					long y2 = y1 + 1;
				...
				// キーアフタータッチの描画
					...
					long x1 = pTrackListFrame->TimetoX (lTime, lLogPixelsX);
					long x2 = x1 + 1;
					long y1 = (i + 1) * lRowZoom * lLogPixelsY / 96;
					long y2 = y1 - lRowZoom  * (lValue) / 128 * lLogPixelsY / 96;
					CRect rcNote (x1, y1, x2, y2);
					pDC->FillSolidRect (&rcNote, lColor);
					if (lValue == 0) {
				...
				// ピッチベンドの描画
					...
					long x1 = pTrackListFrame->TimetoX (lTime, lLogPixelsX);
					long x2 = x1 + 1;
					long y1 = (i + 1) * lRowZoom  * lLogPixelsY / 96 - lRowZoom / 2 * lLogPixelsY / 96;
					long y2 = y1 - lRowZoom * (lValue - 8192) / 16384  * lLogPixelsY / 96; // 20110508修正
					CRect rcNote (x1, y1, x2, y2);
					pDC->FillSolidRect (&rcNote, lColor);
					if (lValue == 8192) {
						pDC->SelectObject (lIsEventSelected ? &theSelectedPen : &theTrackPen);
		...
		if (pSekaijuDoc->m_pMIDIData && pSekaijuDoc->m_pMIDIClock) {
			...
			if (pTrackListFrame->m_bAutoPageUpdate == TRUE) {
				...
				if (lTimeMode == MIDIDATA_TPQNBASE) {
					...
					MIDIData_BreakTime (pMIDIData, pTrackListFrame->GetVisibleRightTime (lLogPixelsX), 
					...
					if (m_lCurTime >= lTempRightTime) {
						...
						long x = pTrackListFrame->TimetoX (lTempRightTime, lLogPixelsX);
						pTrackListFrame->SetTimeScrollPos (x);
					}
					// 現在位置が左側へはみ出した場合
					long lLeftMeasure; long lLeftBeat; long lLeftTick; long lTempLeftTime;
					MIDIData_BreakTime (pMIDIData, pTrackListFrame->GetVisibleLeftTime (lLogPixelsX), 
					...
					if (m_lCurTime < lTempLeftTime) {
						...
						long x = pTrackListFrame->TimetoX (lTempLeftTime, lLogPixelsX);
						pTrackListFrame->SetTimeScrollPos (x);
					}
				}
				else {
					// 現在位置が右側へはみ出した場合
					long lRightFrameNumber = pTrackListFrame->GetVisibleRightTime (lLogPixelsX) / lTimeResolution;
					...
					// 現在位置が左側へはみ出した場合
					long lLeftFrameNumber = pTrackListFrame->GetVisibleLeftTime (lLogPixelsX) / lTimeResolution;
	...
	// リアルタイム入力中の自動画面更新処理
		...
		if (pSekaijuDoc->m_pMIDIData && pSekaijuDoc->m_pMIDIClock && pSekaijuApp->m_bRecording) {
			...
			if (m_lOldTimeRealTimeInput != lCurTime) {
				long lOldX = pTrackListFrame->TimetoX (m_lOldTimeRealTimeInput, lLogPixelsX);
				long lCurX = pTrackListFrame->TimetoX (m_lCurTime, lLogPixelsY);
				long lTimeScrollPos = pTrackListFrame->GetTimeScrollPos ();
...
void CTrackListTrackTimeView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	m_lTempTime = pTrackListFrame->XtoTime (point.x, lLogPixelsX);
	m_lTempTrackIndex = pTrackListFrame->YtoRow (point.y, lLogPixelsY);
	CRect rcRegion;
	long lNewTrackIndex = pTrackListFrame->YtoRow (point.y, lLogPixelsY);
	long lNewTime = pTrackListFrame->XtoTime (point.x, lLogPixelsX);
	...
	case ID_TRACKLIST_SELECT:
		...
		// 選択されていないセルの上にマウスが置かれた場合(選択モード)
			...
			// 新しい選択範囲矩形を描画する
			m_lMinY = pTrackListFrame->RowtoY (lNewTrackIndex, lLogPixelsY);
			m_lMaxY = pTrackListFrame->RowtoY (lNewTrackIndex + 1, lLogPixelsY);
			...
			m_lMinX = pTrackListFrame->TimetoX (lMinTime, lLogPixelsX);
			m_lMaxX = pTrackListFrame->TimetoX (lMaxTime, lLogPixelsX);
...
void CTrackListTrackTimeView::OnRButtonDown (UINT nFlags, CPoint point) {
	...
	long lTime = pTrackListFrame->XtoTime (point.x, lLogPixelsX);
...
void CTrackListTrackTimeView::OnLButtonUp (UINT nFlags, CPoint point) {
	...
	long lDownTrackIndex = pTrackListFrame->YtoRow (m_ptMouseDown.y, lLogPixelsY);
	long lOldTrackIndex = pTrackListFrame->YtoRow (m_ptMouseMove.y, lLogPixelsY);
	long lNewTrackIndex = pTrackListFrame->YtoRow (point.y, lLogPixelsY);
	long lDownTime = pTrackListFrame->XtoTime (m_ptMouseDown.x, lLogPixelsX);
	long lOldTime = pTrackListFrame->XtoTime (m_ptMouseMove.x, lLogPixelsX);
	long lNewTime = pTrackListFrame->XtoTime (point.x, lLogPixelsX);
	CRect rcRegion;
	// キャプター中ならば
	if (GetCapture () == this) {
		...
		case ID_TRACKLIST_SELECT:
			...
			if (m_lTempMode == 0x0402 || m_lTempMode == 0x0403) {
				...
				MIDIData_BreakTime (pMIDIData, lNewTime, &lNewMeasure, &lNewBeat, &lNewTick);
				long lTempSelectedEventCount = (long)CLIP(0, m_theTempSelectedEventArray.GetSize (), 0x7FFFFFFF);
				...
				long lTempEndofTrackEventCount = (long)CLIP (0, m_theTempEndofTrackEventArray.GetSize (), 0x7FFFFFFF);
		...
		case ID_TRACKLIST_SELECT:
			...
			if (m_lTempMode == 0x0402 || m_lTempMode == 0x0403) {
				...
				if (lOldTrackIndex != lNewTrackIndex) {
					...
					long lSelectedEventCount = (long)CLIP (0, m_theTempSelectedEventArray.GetSize (), 0x7FFFFFFF);
			...
			// 選択範囲変更中
				...
				m_lMinY = pTrackListFrame->RowtoY (lMinTrackIndex, lLogPixelsY);
				m_lMaxY = pTrackListFrame->RowtoY (lMaxTrackIndex + 1, lLogPixelsY);
...
// マウスホイールが回された時
BOOL CTrackListTrackTimeView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CTrackListFrame* pTrackListFrame = (CTrackListFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	// ユニバーサルスクロール停止
