...
void CPianoRollKeyScaleView::OnDraw (CDC* pDC) {
	...
	long lTime = pPianoRollFrame->GetVisibleLeftTime (lLogPixelsX);
	...
	// 10オクターブループ
	for (i = 0; i < 11; i++) {
		hh = ((128 - 12 * i) * lKeyZoom) * lLogPixelsY / 96;
		// 白鍵と白鍵の間の線
		pDC->MoveTo (0, hh - lKeyZoom * 0 * lLogPixelsY / 96);
		pDC->LineTo (w, hh - lKeyZoom * 0 * lLogPixelsY / 96);
		pDC->MoveTo (0, hh - lKeyZoom * 3 / 2 * lLogPixelsY / 96);
		pDC->LineTo (w, hh - lKeyZoom * 3 / 2 * lLogPixelsY / 96);
		pDC->MoveTo (0, hh - lKeyZoom * 7 / 2 * lLogPixelsY / 96);
		pDC->LineTo (w, hh - lKeyZoom * 7 / 2 * lLogPixelsY / 96);
		pDC->MoveTo (0, hh - lKeyZoom * 5 * lLogPixelsY / 96);
		pDC->LineTo (w, hh - lKeyZoom * 5 * lLogPixelsY / 96);
		pDC->MoveTo (0, hh - lKeyZoom * 13 / 2 * lLogPixelsY / 96);
		pDC->LineTo (w, hh - lKeyZoom * 13 / 2 * lLogPixelsY / 96);
		pDC->MoveTo (0, hh - lKeyZoom * 17 / 2 * lLogPixelsY / 96);
		pDC->LineTo (w, hh - lKeyZoom * 17 / 2 * lLogPixelsY / 96);
		pDC->MoveTo (0, hh - lKeyZoom * 21 / 2 * lLogPixelsY / 96);
		pDC->LineTo (w, hh - lKeyZoom * 21 / 2 * lLogPixelsY / 96);
		// 黒鍵部分塗りつぶし
		pDC->FillSolidRect 
			(0, hh - lKeyZoom * 2 * lLogPixelsY / 96, w * 2 / 3, lKeyZoom * lLogPixelsY / 96, ::GetSysColor (COLOR_BTNTEXT));
		pDC->FillSolidRect 
			(0, hh - lKeyZoom * 4 * lLogPixelsY / 96, w * 2 / 3, lKeyZoom * lLogPixelsY / 96, ::GetSysColor (COLOR_BTNTEXT));
		pDC->FillSolidRect 
			(0, hh - lKeyZoom * 7 * lLogPixelsY / 96, w * 2 / 3, lKeyZoom * lLogPixelsY / 96, ::GetSysColor (COLOR_BTNTEXT));
		pDC->FillSolidRect 
			(0, hh - lKeyZoom * 9 * lLogPixelsY / 96, w * 2 / 3, lKeyZoom * lLogPixelsY / 96, ::GetSysColor (COLOR_BTNTEXT));
		pDC->FillSolidRect 
			(0, hh - lKeyZoom * 11 * lLogPixelsY / 96, w * 2 / 3, lKeyZoom * lLogPixelsY / 96, ::GetSysColor (COLOR_BTNTEXT));
	}
	...
	if (lKeyZoom < 12) {
		...
		for (i = 0; i < 11; i++) {
			hh = (128 - 12 * i) * lKeyZoom * lLogPixelsY / 96;
			rcText.top = hh - lKeyZoom / 2 * lLogPixelsY / 96 - 6 * lLogPixelsY / 96;
			rcText.bottom = hh - lKeyZoom / 2 * lLogPixelsY / 96 + 6 * lLogPixelsY / 96;
			CString strText;
		...
		for (i = 0; i < 128; i ++) {
			...
			rcText.top = (127 - i) * lKeyZoom * lLogPixelsY / 96; //20220123修正
			rcText.bottom = (128 - i) * lKeyZoom * lLogPixelsY / 96; //20220123修正
	...
	// 選択部反転
	if (GetCapture () == this) {
		long lDownY = pPianoRollFrame->KeytoY (m_lDownKey, lLogPixelsX);
		long lCurY = pPianoRollFrame->KeytoY (m_lCurKey, lLogPixelsY);
...
void CPianoRollKeyScaleView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	m_lOldKey = pPianoRollFrame->YtoKey (m_ptMouseDown.y, lLogPixelsY);
	m_lDownKey = pPianoRollFrame->YtoKey (point.y, lLogPixelsY);
	m_lCurKey = pPianoRollFrame->YtoKey (point.y, lLogPixelsY);
...
void CPianoRollKeyScaleView::OnMouseMove (UINT nFlags, CPoint point) {
	...
	if (GetCapture () == this) {
		...
		m_lCurKey = pPianoRollFrame->YtoKey (point.y, lLogPixelsY);
		m_lDownKey = pPianoRollFrame->YtoKey (m_ptMouseDown.y, lLogPixelsY);
		m_lOldKey = pPianoRollFrame->YtoKey (m_ptMouseMove.y, lLogPixelsY);
		// 前回のキーと今回のキーが異なる場合のみ
...
// マウスホイールが回された時
BOOL CPianoRollKeyScaleView::OnMouseWheel (UINT nFlags, short zDelta, CPoint point) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
