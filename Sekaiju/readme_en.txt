/******************************************************************************/
/*                                                                            */
/*  Sekaiju8.1  read me first                             (C)2002-2024  kuzu  */
/*                                                                            */
/******************************************************************************/

Thank you for downloading Sekaiju8.1. This file includes basic information and
 ...
 * Both TPQN base at the resolution (time-base) from 1 to 1920 and 
...
### Requirement ###
  ...
  * Printer       : A4 or higher printer (option).
...
|-mfc140u.dll           MFC Runtime Library (for unicode) [Essential]
|-mfc140chs.dll         MFC Chinese Resource DLL [Essential]
|-mfc140enu.dll         MFC English Resource DLL [Essential]
|-mfc140jpn.dll         MFC Japanese Resource DLL [Essential]
|-vcruntime140.dll      Visual Studio 2022 C/C++ Runtime Library [Essential]
|-vcruntime140_1.dll    Visual Studio 2022 C/C++ Runtime Library [Essential]
...
|-IniData.dll           Ini File editing library DLLf [Essential]
|-Sekaiju.exe           Sekaiju Application [Essetial]
|-Sekaiju.ini           Sekaiju Configuration text [Essetial]
|-Sekaiju.sln           Sekaiju Solution file for VisualC Studio 2022 Community
|-Sekaiju.vcxproj       Sekaiju Project file for VisualC Studio 2022 Community
|-Sekaiju.vcxproj.filter Sekaiju Project file for VisualC Studio 2022 Community
|-Sekaiju.vcxproj.user  Sekaiju Project file for VisualC Studio 2022 Community
...
| |-SekaijuChs.rc       Sekaiju Resource script for Chinese
| |-SekaijuRes.rc       Sekaiju Resource script for All
| |-SekaijuRes.sln      Sekaiju Resource solution file for Visual Studio 2022 Community
| |-SekaijuRes.vcxproj  Sekaiju Resource project file for Visual Studio 2022 Community
| |-SekaijuRes.vcxpro.filterj  Sekaiju Resource project file for Visual Studio 2022 Community
| !-SekaijuRes.vcxproj.user  Sekaiju Resource project file for Visual Studio 2022 Community
...
| |-Sekaiju.odt         Japanese Documentation (for Libre Office)
| |-Sekaiju.pdf         Japanese Documentation (for Adobe Reader)
| |-SekaijuEn.odt       English Documentation (for Libre Office)
| |-SekaijuEn.pdf       English Documentation (for Adobe Reader)
| |-SekaijuCh.odt       Chinese Documentation (for Libre Office)
| !-SekaijuCh.pdf       Chinese Documentation (for Adobe Reader)
...
(1) Please right-click "Sekaiju8.1.zip" and select "Properties", and then check on
             "General" tab's "Security : This file came from another computer 
             and might be blocked to help protect this computer" 's "Unblock".


(2) Please extract "Sekaiju8.1.zip" on your hard disk or SSD.

  !WARNING!  If you forget to extract, Sekaiju will not be executed.

(3) Please double click "Sekaiju.exe" on "explorer" or "my computer"
  ...
  !WARNING!  In recent Windows, by the "SmartScreen" feature, 
             exe files downloaded from the Internet may show 
             "This PC is protected by Windows." message,
             and may not be able to be executed. 
             If you get this message, you can execute it by clicking "Detail Information" 
             and then clicking "Run". 
             Please be careful that you may have downloaded a fake on a dummy site. 
  !WARNING!  Be sure to extract the zip file before executing. 

(4) First you must select MIDI In device, MIDI Out device, 
...
(1) Delete "Sekaiju8.1" folder on "explorer" or "my computer".
...
(3) This software dynamically links to the following Dynamic Link Library (DLL).
 * SekaijuJpn.dll       (c)2024 kuzu / OpenMIDIProject  License = MPL2.0
 * SekaijuEnu.dll       (c)2024 kuzu / OpenMIDIProject  License = MPL2.0
 * SekaijuChs.dll       (c)2024 kuzu / OpenMIDIProject  License = MPL2.0
 * MIDIIO.dll           (c)2024 kuzu / OpenMIDIProject  License = GNU LGPL
 * MIDIData.dll         (c)2024 kuzu / OpenMIDIProject  License = GNU LGPL
 * MIDIClock.dll        (c)2024 kuzu / OpenMIDIProject  License = GNU LGPL
 * MIDIStatus.dll       (c)2024 kuzu / OpenMIDIProject  License = GNU LGPL
 * MIDIInstrument.dll   (c)2024 kuzu / OpenMIDIProject  License = GNU LGPL
 * IniData.dll          (c)2024 kuzu / OpenMIDIProject  License = GNU LGPL
 * vcruntime140.dll     (c)2022 Microsoft Corporation  License = Proprietary
 * vcruntime140_1.dll   (c)2022 Microsoft Corporation  License = Proprietary
 * mfc140u.dll          (c)2022 Microsoft Corporation  License = Proprietary
 * mfc140jpn.dll        (c)2022 Microsoft Corporation  License = Proprietary
 * mfc140enu.dll        (c)2022 Microsoft Corporation  License = Proprietary
 * mfc140chs.dll        (c)2022 Microsoft Corporation  License = Proprietary
 
  !Warning! These dlls are shipped with Sekaiju8.1.
  !Warning! If you lost dll, or if you use wrong version of DLL, Sekaiju will not be executed.
  !Warning! The other dlls (e.g. api-ms-crt-runtime-l1-1-0.dll) are provided by Windows Update automatically.
            Or, please download and install VisualC++ Redistributable Package (vc_redist.x64.exe) manually.
 ...
 * A bug that if multiple piano roll window or event list window or musical score window is opened 
   the track list's check box becomes all checked when OK button used is fixed.
 * In order to restore RPN/NRPN exactly when playing is started again,
   changed to remain RPN/NRPN's status independently from system exclusive data's one.
 ...
 * Windows SDK is migrated to 10.0.240.0 (c:\program files(x86)\Windows Kits\10\include\10.0.240.0\ucrt\).
 * Windows 64bit EXE Compliant.
 ...
 * A bug that in the event list window, if selecting tempo, time signature or key signature at the first track 
  Sekaiju causes freeze is fixed.

2022/02/13 Sekaiju7.4 Release
 * Support real-time view updating while real-time inputting at the track list window, piano roll window,
   event list window, musical score window.
 * A bug that in the piano roll window, if high DPI mode, the note key names are displayed wrongly is fixed.

2022/04/17 Sekaiju7.5 Relase
 ...
 * MIDIIOLibrary is updated to 8.0.
 * MIDIDataLibrary is updated to 8.0.
 * MIDIClockLibrary is updated to 8.0.
 * MIDIStatusLibrary is updated to 8.0.
 * MIDIInstrumentLibrary is updated to 8.0.

2024/03/24 Sekaiju8.1 Release
 * A bug that when invalid CC#0 and CC#32 were detected Sekaiju crashes is fixed.
 * IniDataLibrary is imported and executing and exiting get faster.
